...
import dml.runtime.test.GenerateTestMatrixBlockVerySparseLimited.SequenceOutMapper;
import dml.runtime.util.MapReduceTool;

public class GenerateTestMatrixBlockVerySparseLimited
{
	...
	public static JobConf runJob(int numMappers, long numRows, long numColumns, String outDir, 
			int replication, int blockRowSize, int blockColumnsSize,
			double minValue, double maxValue, long seed, double sparcity) 
	throws IOException
	{
		Path jobFile = new Path( "GenerateTestMatrixBlockVerySparseLimited-seeds-"
				+ Integer.toString(new Random().nextInt(Integer.MAX_VALUE)));
		Path outputDir = new Path(outDir);
		
		// create job
		JobConf job = new JobConf(GenerateTestMatrixBlockVerySparseLimited.class);
		try
		{ 
	...
	public static void main(String[] args) throws Exception
	{
		if(args.length < 11)
		{
			System.err.println("expect 11 paramers: numMappers numRows numColumns " +
					"outputDir replication blockRowSize blockColumnsSize " +
					"minValue maxValue seed sparcity");
			System.exit(-1);
		}
		...
		JobConf job = GenerateTestMatrixBlockVerySparseLimited.runJob(numMappers, numRows, numColumns, outDir, 
