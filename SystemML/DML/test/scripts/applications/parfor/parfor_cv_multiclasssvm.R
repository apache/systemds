...
scoreMultiClassSVM = function( X, y, W, intercept) 
{
   ...
   predicted_y = which(scores == max(scores));
...
y1 <- readMM(paste(args[1], "y.mtx", sep=""))
X <- as.matrix(X1);
y <- as.matrix(y1);
...
#CV
P = array(runif(m, min=0, max=1),dim=c(m,1));
P = round(0.5+P*k);
