...
public class ReaderBinaryBlockParFiles extends MatrixReader {
	...
	public ReaderBinaryBlockParFiles( boolean localFS )
	{
		_localFS = localFS;
	...
	{			
		...
		try 
		{
			...
			for( Path lpath : getSequenceFilePaths(fs, path) ){
				ReadMatrixPerPartfileTask t = new ReadMatrixPerPartfileTask(lpath, job, fs, dest, rlen, clen, brlen, bclen);
				tasks.add(t);
			}
	 ...
	 * 
	 */
	private static class ReadMatrixPerPartfileTask implements Callable<Object> 
	{
		...
		public ReadMatrixPerPartfileTask(Path path, JobConf job, FileSystem fs, MatrixBlock dest, long rlen, long clen, int brlen, int bclen)
		{
			_path = path;
			_fs = fs;
		...
		public Object call() throws Exception 
		{
			...
			try
			{
				...
				{	
					...
					{
						if (cols < _clen ) {
							synchronized( _dest ){ //sparse requires lock, when matrix is wider than one block-width
	 ...
	 * 
	 */
	private static class ReadMatrixBlockTask implements Callable<Object> 
	{
		private Path _path = null;
		private JobConf _job = null;
		private FileSystem _fs = null;
