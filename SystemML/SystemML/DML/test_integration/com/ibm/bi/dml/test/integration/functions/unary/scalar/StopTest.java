...
public class StopTest extends AutomatedTestBase 
{
	...
	String stopMessage = "M is all 0 matrix.";
	String printMessage = "Sum(M2) = 10.0";
	@Test
	...
	public void runStopTest(int val) {
		
		TestConfiguration config = availableTestConfigurations.get("StopTest");
		
		String STOP_HOME = SCRIPT_DIR + TEST_DIR;
		fullDMLScriptName = STOP_HOME + TEST_STOP + ".dml";
		programArgs = new String[]{"-args", Integer.toString(val), stopMessage};
		...
		if ( val == 0 )
			setExpectedStdErr(stopMessage);
		else 
			setExpectedStdOut(printMessage);
