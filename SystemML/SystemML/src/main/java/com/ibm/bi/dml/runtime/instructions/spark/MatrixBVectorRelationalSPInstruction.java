...
public class MatrixBVectorRelationalSPInstruction extends RelationalBinarySPInstruction {
	...
	public MatrixBVectorRelationalSPInstruction(Operator op, 
			   CPOperand in1, 
			   CPOperand in2, 
			   CPOperand out,
			   String opcode,
		...
		if(input1.getDataType() == DataType.MATRIX && input2.getDataType() == DataType.MATRIX) {
			...
			if ( opcode.equalsIgnoreCase("map==") || opcode.equalsIgnoreCase("map!=") || opcode.equalsIgnoreCase("map<")
				...
				if(mc1.getRowsPerBlock() != mc2.getRowsPerBlock() ||  mc1.getColsPerBlock() != mc2.getColsPerBlock()) {
					throw new DMLRuntimeException("MatrixBVectorRelationalSPInstruction is only supported for matrices with same blocksizes "
				...
				if(!isColumnVectorOperation && !isRowVectorOperation && !isOuter) {
					throw new DMLRuntimeException("Incorrect input dimensions for MatrixBVectorRelationalSPInstruction");
				}
				...
				if(!mcOut.dimsKnown()) {
					if(!mc1.dimsKnown())
						throw new DMLRuntimeException("The output dimensions are not specified for MatrixBVectorRelationalSPInstruction");
					else if(isOuter) {
			...
			throw new DMLRuntimeException("MatrixBVectorRelationalSPInstruction is only applicable for matrix inputs");
		}
	}
