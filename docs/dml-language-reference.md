...
svd() | Singular Value Decomposition of a matrix A (of size mxm), Which decomposes into three matrices U, V, S as A = U %*% S %*% t(V). Where U is an mxm unitary matrix (i.e., Orthogonal), V is an nxn unitary matrix (also Orthogonal), S is a mxn matrix with non-negative real numbers on the diagonal. | Input: matrix A<(mxn)> <br/> Output: matrices U<(mxm)>, matrix V<(nxn)>, matrix S<(mxn)> | [U, S, V] = svd(A)
