...
input_path = base_dir / "data" / "processed" / "diabetes_sample.csv"
...
def parse_args():
    ...
    parser.add_argument("--output-name", type = str, default = "diabetes_mnar.csv")
    parser.add_argument("--mask-name", type = str, default = "mask_diabetes_mnar.csv")
...
def generate_hybrid_missingness(df, target_configs, random_state = 100):
    ...
    for column, config in target_configs.items():
        print(f"\nGenerating missingness for column: {column}")
        ...
        print(f"Target missing rate: {config['missing_rate']:.3f}")
        print(f"Actual missing rate: {actual_rate:.3f}")
