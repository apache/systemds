...
CMD=${1:-"systemds"}
DATADIR=${2:-"temp/sharedworkers"}/als
NUMFED=${3:-6}
NUMCOORD=${4:-2}
NUMSPLIT=${5:-4}
...
startCoordinator () {
  ...
  ${CMD} -f ${BASEPATH}/scripts/${SCRIPT} \
    ...
    --stats \
    --nvargs data=$FED_DATA modelB=${TARGET_PREFIX}B${INDEX} modelM=${TARGET_PREFIX}M${INDEX} \
      maxiter=$MAXITER &> ${COORD_DIR}/output_${INDEX}.txt &
  pids+=" $!"
}
