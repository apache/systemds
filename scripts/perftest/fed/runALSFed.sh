...
${BASEPATH}/../genALS_FedData.sh systemds $DATADIR &>> ${BASEPATH}/../logs/genALS_FedData.out; # generate the data

# start the federated workers on localhost
"date" &>> ${BASEPATH}/../logs/runAllFed.out
${BASEPATH}/utils/startFedWorkers.sh systemds $DATADIR $NUMFED "localhost" &>> ${BASEPATH}/../logs/runAllFed.out;
...
do
  ...
  ${BASEPATH}/../runALS_CG.sh ${DATADIR}/X${d}_fed.json $MAXITR $DATADIR systemds 0.001 FALSE &> ${BASEPATH}/../logs/${BASEFILENAME}_${d}.out;
done
