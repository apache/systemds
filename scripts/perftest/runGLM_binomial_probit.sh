...
for i in 0 1 2; do
   ...
   #training
   tstart=$(date +%s.%N)
   # systemds -f scripts/algorithms/GLM.dml \
   systemds -f ${PERFTESTPATH}/scripts/GLM.dml \
   ...
   echo "GLM_binomial_probit train ict="$i" on "$1": "$ttrain >> ${PERFTESTPATH}/results/times.txt
   ...
   echo "GLM_binomial_probit predict ict="$i" on "$1": "$tpredict >> ${PERFTESTPATH}/results/times.txt
done
