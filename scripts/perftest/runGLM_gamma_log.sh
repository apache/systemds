...
for i in 0 1 2; do
   ...
   ${CMD} -f scripts/GLM.dml \
      ...
      ${DASH}-stats \
      ${DASH}-nvargs X=$1 Y=$2 B=${BASE}/b icpt=${i} fmt="csv" moi=$4 mii=5 dfam=1 vpow=2.0 link=1 lpow=0.0 tol=0.0001 reg=0.01
   ...
   ${CMD} -f ../algorithms/GLM-predict.dml \
      ${DASH}-config conf/SystemDS-config.xml \
      ${DASH}-stats \
      ${DASH}-nvargs dfam=1 vpow=2.0 link=1 lpow=0.0 fmt=csv X=$1_test B=${BASE}/b Y=$2_test M=${BASE}/m O=${BASE}/out.csv
