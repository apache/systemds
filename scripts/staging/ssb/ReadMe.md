...
The shell script covers the installation of the following points.
...
After that, it is copied into the database containers (SystemDS, PostgreSQL and DuckDB) where the queries are executed.
 ...
 `TODO`: DuckDB in Docker or locally?
...
```
./run_script.sh [QUERY_NAME] [SCALE] [DB-SYSTEM]
```
Examples
```
./run_script.sh all 0.1 all
./run_script.sh q4_3 0.1 systemds
./run_script.sh all 0.1 postgres
```
