		...
		if (o != ops.end()) {
			...
			if(output != nullptr) {
				...
				if (op->op_type == SpoofOperator::OpType::CW)
					num_blocks = std::ceil(((m*n) + NT * 2 - 1) / (NT * 2));
		    ...
		    case SpoofOperator::OpType::CW:
				launch_cw_kernel(op, d_in, d_out, d_sides, sides.size(), d_scalars, m, n, grix, input[0].row_ptr!=nullptr);
				break;
		    case SpoofOperator::OpType::RA:
				launch_ra_kernel(op, d_in, d_out, d_sides, sides.size(), d_scalars, m, n, grix, input[0].row_ptr!=nullptr);
		        break;
		    default:
