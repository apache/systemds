...
public class FunctionOp extends MultiThreadedHop
{
	...
	{		
		if ( getFunctionType() != FunctionType.MULTIRETURN_BUILTIN )
			...
			if ( getFunctionName().equalsIgnoreCase(Opcodes.QR.getName()) ) {
			...
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.LU.getName()) ) {
				// upper-triangular and lower-triangular matrices
			...
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.EIGEN.getName()) ) {
			...
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.FFT.getName()) ) {
			...
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.IFFT.getName()) ) {
				long outputRe = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(), getOutputs().get(0).getDim2(), 1.0);
				long outputIm = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(), getOutputs().get(1).getDim2(), 1.0);
				return outputRe+outputIm;
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.FFT_LINEARIZED.getName()) ) {
				long outputRe = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(), getOutputs().get(0).getDim2(), 1.0);
				long outputIm = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(), getOutputs().get(1).getDim2(), 1.0);
				return outputRe+outputIm;
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.IFFT_LINEARIZED.getName()) ) {
				long outputRe = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(), getOutputs().get(0).getDim2(), 1.0);
				long outputIm = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(), getOutputs().get(1).getDim2(), 1.0);
				return outputRe+outputIm;
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.STFT.getName()) ) {
				long outputRe = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(), getOutputs().get(0).getDim2(), 1.0);
				long outputIm = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(), getOutputs().get(1).getDim2(), 1.0);
				return outputRe+outputIm;
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.LSTM.getName()) || getFunctionName().equalsIgnoreCase(Opcodes.LSTM_BACKWARD.getName()) ) {
				// TODO: To allow for initial version to always run on the GPU
				return 0; 
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D.getName()) || getFunctionName().equalsIgnoreCase("batch_norm2d_train")) {
			...
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D_BACKWARD.getName()) ) {
				return OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(), getOutputs().get(0).getDim2(), 1.0) +
						OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(), getOutputs().get(1).getDim2(), 1.0) +
						OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(2).getDim1(), getOutputs().get(2).getDim2(), 1.0);
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.SVD.getName()) ) {
			...
			else if( getFunctionName().equalsIgnoreCase(Opcodes.RCM.getName()) ) {
	...
	protected double computeIntermediateMemEstimate( long dim1, long dim2, long nnz )
	{
		if ( getFunctionType() != FunctionType.MULTIRETURN_BUILTIN )
			...
			else if (getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D.getName()) || getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D_BACKWARD.getName()) ||
					getFunctionName().equalsIgnoreCase("batch_norm2d_train") || getFunctionName().equalsIgnoreCase("batch_norm2d_test")) {
				return 0; 
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.LSTM.getName())
					||  getFunctionName().equalsIgnoreCase(Opcodes.LSTM_BACKWARD.getName())
					|| getFunctionName().equalsIgnoreCase(Opcodes.RCM.getName())) {
				// TODO: To allow for initial version to always run on the GPU
				return 0; 
			}
			else if ( getFunctionName().equalsIgnoreCase(Opcodes.SVD.getName())) {
	...
	public boolean isGPUEnabled() {
		if(getFunctionName().equalsIgnoreCase(Opcodes.LSTM.getName()) || getFunctionName().equalsIgnoreCase(Opcodes.LSTM_BACKWARD.getName()) ||
			getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D.getName()) || getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D_BACKWARD.getName()) ||
			getFunctionName().equalsIgnoreCase("batch_norm2d_train") || getFunctionName().equalsIgnoreCase("batch_norm2d_test")) 
			return true;
		else
			return false;
	...
	protected void constructAndSetCompressionLopFunctionalIfRequired(ExecType et) {
		if((requiresCompression()) && ((FunctionCallCP) getLops()).getFunctionName().equalsIgnoreCase(Opcodes.TRANSFORMENCODE.getName())){ // xor
	...
	protected ExecType optFindExecType(boolean transitive) 
	{
		...
		if ( getFunctionType() == FunctionType.MULTIRETURN_BUILTIN ) {
			...
			if(isBuiltinFunction && getFunctionName().equalsIgnoreCase(Opcodes.TRANSFORMENCODE.getName()) ) {
			...
			else if(isBuiltinFunction && (getFunctionName().equalsIgnoreCase(Opcodes.LSTM.getName()) || getFunctionName().equalsIgnoreCase(Opcodes.LSTM_BACKWARD.getName()))) {
				_etype = DMLScript.USE_ACCELERATOR ? ExecType.GPU : ExecType.CP;
			}
			else if(isBuiltinFunction && (getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D.getName()) || getFunctionName().equalsIgnoreCase(Opcodes.BATCH_NORM2D_BACKWARD.getName()))) {
				_etype = DMLScript.USE_ACCELERATOR ? ExecType.GPU : ExecType.CP;
			}
