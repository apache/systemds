 ...
 * 
 * Note: Currently, we support expressions in function arguments along with function calls in expressions with single
 * outputs, leaving multiple outputs handling as it is.
 */
public class FunctionOp extends Hop {
	public enum FunctionType {
		DML, MULTIRETURN_BUILTIN, UNKNOWN
	}
	...
	public FunctionOp(FunctionType type, String fnamespace, String fname, String[] inputNames, List<Hop> inputs,
		String[] outputNames, ArrayList<Hop> outputHops) {
		this(type, fnamespace, fname, inputNames, inputs, outputNames, false);
		_outputHops = outputHops;
	}

	public FunctionOp(FunctionType type, String fnamespace, String fname, String[] inputNames, List<Hop> inputs,
		String[] outputNames, boolean singleOut) {
		this(type, fnamespace, fname, inputNames, inputs, outputNames, singleOut, false);
	}

	public FunctionOp(FunctionType type, String fnamespace, String fname, String[] inputNames, List<Hop> inputs,
		String[] outputNames, boolean singleOut, boolean pseudo) {
	...
	public boolean containsOutput(String varname) {
		return Arrays.stream(getOutputVariableNames()).anyMatch(outName -> outName.equals(varname));
	}
	...
	protected double computeOutputMemEstimate(long dim1, long dim2, long nnz) {
		if(getFunctionType() != FunctionType.MULTIRETURN_BUILTIN)
			...
			if(getFunctionName().equalsIgnoreCase("qr")) {
				...
				long outputH = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(),
					getOutputs().get(0).getDim2(), 0.5);
				long outputR = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(),
					getOutputs().get(1).getDim2(), 0.5);
				return outputH + outputR;
			}
			else if(getFunctionName().equalsIgnoreCase("lu")) {
				// upper-triangular and lower-triangular matrices
				long outputP = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(),
					getOutputs().get(1).getDim2(), 1.0 / getOutputs().get(1).getDim2());
				long outputL = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(),
					getOutputs().get(0).getDim2(), 0.5);
				long outputU = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(),
					getOutputs().get(1).getDim2(), 0.5);
				return outputL + outputU + outputP;
			}
			else if(getFunctionName().equalsIgnoreCase("eigen")) {
				long outputVectors = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(),
				...
				long outputRe = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(),
					getOutputs().get(0).getDim2(), 1.0);
				long outputIm = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(),
				...
				return OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(),
					getOutputs().get(0).getDim2(), 1.0) +
				...
				long outputU = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(0).getDim1(),
					getOutputs().get(0).getDim2(), 1.0);
				long outputSigma = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(1).getDim1(),
					getOutputs().get(1).getDim2(), 1.0);
				long outputV = OptimizerUtils.estimateSizeExactSparsity(getOutputs().get(2).getDim1(),
					getOutputs().get(2).getDim2(), 1.0);
	...
	protected double computeIntermediateMemEstimate(long dim1, long dim2, long nnz) {
		if(getFunctionType() != FunctionType.MULTIRETURN_BUILTIN)
			...
			if(getFunctionName().equalsIgnoreCase("qr")) {
				...
				return OptimizerUtils.estimateSizeExactSparsity(getInput().get(0).getDim1(),
				...
				return 2 * OptimizerUtils.estimateSizeExactSparsity(getInput().get(0).getDim1(),
					getInput().get(0).getDim2(), 1.0);
			}
			else if(getFunctionName().equalsIgnoreCase("ifft")) {
				// 2 matrices of size same as the input
	...
	public Lop constructLops() {
		...
		FunctionCallCP fcall = new FunctionCallCP(tmp, _fnamespace, _fname, _inputNames, _outputNames, _outputHops,
			_opt, et);
	...
	protected void constructAndSetCompressionLopFunctionalIfRequired(ExecType et) {
		if((requiresCompression()) &&
			((FunctionCallCP) getLops()).getFunctionName().equalsIgnoreCase("transformencode")) { // xor
	...
	protected ExecType optFindExecType(boolean transitive) {
		...
		if(getFunctionType() == FunctionType.MULTIRETURN_BUILTIN) {
			...
			if(isBuiltinFunction && getFunctionName().equalsIgnoreCase("transformencode")) {
				_etype = ((_etypeForced == ExecType.SPARK || (getMemEstimate() >= OptimizerUtils.getLocalMemBudget() &&
			...
			else if(isBuiltinFunction && (getFunctionName().equalsIgnoreCase("batch_norm2d") ||
				getFunctionName().equalsIgnoreCase("batch_norm2d_backward"))) {
