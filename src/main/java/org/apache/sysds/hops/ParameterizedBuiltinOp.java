			...
			if(spec instanceof LiteralOp && ((LiteralOp) spec).getStringValue().contains("word_embedding")){
				...
				if( memo.hasInputStatistics(this)) {
					...
					if (idc != null && edc != null && edc.dimsKnown() && idc.dimsKnown()) {
						DataCharacteristics wdc = new MatrixCharacteristics(idc.getRows(), edc.getCols(), -1, idc.getRows()*edc.getCols());
						_outputMemEstimate = DenseBlockFP64DEDUP.estimateMemory(wdc.getRows(), edc.getCols(), edc.getRows());
