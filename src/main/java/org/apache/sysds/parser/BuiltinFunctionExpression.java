	...
	public void validateExpression(MultiAssignmentStatement stmt, HashMap<String, DataIdentifier> ids,
		...
		switch (_opcode) {
			...
			case LSTM: {
				...
				// Output1 - out: If `return_sequences` is True, outputs for all timesteps, else
				// outputs for the final timestep.
			...
			case BATCH_NORM2D: {
				// Input: image, scale, bias, runningMean, runningVar, mode, epsilon,
	...
	public void validateExpression(HashMap<String, DataIdentifier> ids, HashMap<String, ConstIdentifier> constVars,
		...
		switch (getOpCode()) {
			...
			case CAST_AS_DOUBLE:
				...
				output.setDataType(DataType.SCALAR);
				// output.setDataType(id.getDataType()); //TODO whenever we support multiple
				// matrix value types, currently noop.
				output.setDimensions(0, 0);
				output.setBlocksize(0);
				output.setValueType(ValueType.FP64);
				break;
			...
			case SEQ:
				...
				if (isConstant(getFirstExpr()) && isConstant(getSecondExpr())
					...
					// For example, seq(0,1,0.5) produces (0.0 0.5 1.0) whereas seq(0,1,0.6)
					// produces only (0.0 0.6) but not (0.0 0.6 1.0)
			...
			case AVG_POOL_BACKWARD: {
				// At DML level:
				// output = conv2d(input, filter, input_shape=[1, 3, 2, 2], filter_shape=[1, 3,
				// 2, 2],
				...
				Expression input = _args[0]; // For conv2d_backward_filter, this is input and for conv2d_backward_data,
												// this is filter
