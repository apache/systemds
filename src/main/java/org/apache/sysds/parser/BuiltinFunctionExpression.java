...
public class BuiltinFunctionExpression extends DataIdentifier {
	...
	public void validateExpression(MultiAssignmentStatement stmt, HashMap<String, DataIdentifier> ids,
		...
		switch(_opcode) {
			...
			case LU:
				...
				if(inrows != incols) {
					raiseValidateError(
						"LU Decomposition requires a square matrix. Matrix " + getFirstExpr() + " is " + inrows + "x" +
							incols + ".", conditional);
				}
			...
			case RCM: {
				...
				checkMatrixParam(getFirstExpr());
				checkMatrixParam(getSecondExpr());
				long nr = Math.max(getFirstExpr().getOutput().getDim1(), getSecondExpr().getOutput().getDim1());
				long nc = Math.max(getFirstExpr().getOutput().getDim2(), getSecondExpr().getOutput().getDim2());
				for(int i = 0; i < 2; i++) {
			...
			case FFT: {
				...
				if(expressionOne == null) {
					...
					raiseValidateError("Too many arguments. This FFT implementation is only defined for real inputs.",
			...
			case IFFT: {
				...
				if(expressionTwo != null && expressionOne != null) {
					...
					if(expressionOne.getOutput().getDim1() != expressionTwo.getOutput().getDim1() ||
						...
						raiseValidateError(
							"The real and imaginary part of the provided matrix are of different dimensions.", false);
			...
			case FFT_LINEARIZED: {
				...
				if(expressionOne == null) {
					...
					raiseValidateError(
						"Too many arguments. This FFT_LINEARIZED implementation is only defined for real inputs.",
						false, LanguageErrorCodes.INVALID_PARAMETERS);
				}
			...
			case STFT: {
				...
				if((getFirstExpr() == null || getSecondExpr() == null || getThirdExpr() == null) && _args.length > 0) {
					...
					raiseValidateError(
						"Invalid number of arguments for function " + this.getOpCode().toString().toLowerCase() +
					...
					if(!isPowerOfTwo(((ConstIdentifier) getSecondExpr().getOutput()).getLongValue())) {
						raiseValidateError(
							"This FFT implementation is only defined for matrices with dimensions that are powers of 2." +
								"The window size (2nd argument) is not a power of two", false,
							LanguageErrorCodes.INVALID_PARAMETERS);
					}
					else if(((ConstIdentifier) getSecondExpr().getOutput()).getLongValue() <=
						((ConstIdentifier) getThirdExpr().getOutput()).getLongValue()) {
					...
					if(!isPowerOfTwo(((ConstIdentifier) getThirdExpr().getOutput()).getLongValue())) {
						raiseValidateError(
							"This FFT implementation is only defined for matrices with dimensions that are powers of 2." +
								"The window size (3rd argument) is not a power of two", false,
							LanguageErrorCodes.INVALID_PARAMETERS);
					}
					...
					else if(((ConstIdentifier) getThirdExpr().getOutput()).getLongValue() <=
						((ConstIdentifier) getFourthExpr().getOutput()).getLongValue()) {
						raiseValidateError("Overlap can't be larger than or equal to the window size.", false,
							LanguageErrorCodes.INVALID_PARAMETERS);
					}
				}
			...
			case REMOVE: {
				...
				long nrow = getFirstExpr().getOutput().getDim1() > 0 ? getFirstExpr().getOutput().getDim1() + 1 : -1;
		...
		switch(getOpCode()) {
			...
			case TRACE:
				if(getFirstExpr().getOutput().dimsKnown() &&
					...
					raiseValidateError(
						"Trace is only defined on squared matrices but found [" + getFirstExpr().getOutput().getDim1() +
							"x" + getFirstExpr().getOutput().getDim2() + "].", conditional);
				}
			case SUM:
			case PROD:
			case SD:
							...
							if(m1rlen >= 0 && m2rlen >= 0 && m1rlen != m2rlen) {
								raiseValidateError(
									"inputs to cbind must have same number of rows: input 1 rows: " + m1rlen +
										", input 2 rows: " + m2rlen, conditional,
									LanguageErrorCodes.INVALID_PARAMETERS);
							}
							...
							if(m1clen >= 0 && m2clen >= 0 && m1clen != m2clen) {
								raiseValidateError(
									"inputs to rbind must have same number of columns: input 1 columns: " + m1clen +
										", input 2 columns: " + m2clen, conditional,
									LanguageErrorCodes.INVALID_PARAMETERS);
							}
						...
						if(id.getDim1() != id.getDim2()) {
							raiseValidateError(
								"diag can either: (1) create diagonal matrix from (n x 1) matrix, or (2) take diagonal from a square matrix. " +
									...
									id.getDim2() + ") in " + this.toString(), conditional,
								LanguageErrorCodes.INVALID_PARAMETERS);
						}
						//diag M2V
						output.setDimensions(id.getDim1(), 1);
			...
			case DET:
				...
				if(id.getDim2() == -1 || id.getDim1() != id.getDim2()) {
					raiseValidateError("det requires a square matrix as first argument.", conditional,
						LanguageErrorCodes.INVALID_PARAMETERS);
				}
				output.setDataType(DataType.SCALAR);
				output.setDimensions(0, 0);
			...
			case INVERSE:
				...
				if(in.dimsKnown() && in.getDim1() != in.getDim2())
					raiseValidateError(
						"Input to inv() must be square matrix -- given: a " + in.getDim1() + "x" + in.getDim2() +
							" matrix.", conditional);
			...
			case SQRT_MATRIX_JAVA:
				...
				if(sqrt.dimsKnown() && sqrt.getDim1() != sqrt.getDim2())
					raiseValidateError(
						"Input to sqrtMatrix() must be square matrix -- given: a " + sqrt.getDim1() + "x" +
							sqrt.getDim2() + " matrix.", conditional);
			...
			case CHOLESKY: {
				...
				if(inA.dimsKnown() && inA.getDim1() != inA.getDim2())
					raiseValidateError(
						"Input to cholesky() must be square matrix -- given: a " + inA.getDim1() + "x" + inA.getDim2() +
							" matrix.", conditional);
			...
			case OUTER:
				...
				if(id.getDim2() > 1 || id2.getDim1() > 1) {
					raiseValidateError(
						"Outer vector operations require a common dimension of one: " + id.getDim1() + "x" +
							id.getDim2() + " o " + id2.getDim1() + "x" + id2.getDim2() + ".", false);
				}
			...
			case AVG_POOL_BACKWARD: {
				...
				if(this.getOpCode() == Builtins.MAX_POOL_BACKWARD || this.getOpCode() == Builtins.AVG_POOL_BACKWARD) {
					...
					try {
						...
						output.setDimensions(-1,
							-1); // To make sure that output dimensions are not incorrect even if getDoubleValue doesnot return value
					}
				}
				checkMatrixParam(input);
				if(input2 != null)
	...
	private void validateEinsum(DataIdentifier output) {
		...
		if(equationString.length() == 0)
			raiseValidateError("Einsum: equation str too short", false, LanguageErrorCodes.INVALID_PARAMETERS);
		if(equationString.charAt(0) == '-' || equationString.charAt(0) == ',')
			raiseValidateError("Einsum: equation str invalid", false, LanguageErrorCodes.INVALID_PARAMETERS);
		...
		if(allDimsKnown) {
			...
			DataType dataType = EinsumEquationValidator.validateEinsumEquationNoDimensions(equationString,
				_args.length - 1);
