...
public class DMLTranslator {
	...
	public void rewriteHopsDAG(DMLProgram dmlp) {
		...
		if(ConfigurationManager.isCodegenEnabled()) {
			SpoofCompiler.PLAN_CACHE_POLICY = PlanCachePolicy.get(dmlconf.getBooleanValue(DMLConfig.CODEGEN_PLANCACHE),
	...
	private void prepareAndAddFunctionProgramBlock(Program rtprog, DMLConfig config, String fnamespace, String fname,
		FunctionStatementBlock fsb, boolean opt) {
	...
	private static Instruction deriveExitInstruction(StatementBlock sb) {
		Set<String> rmVars = VariableSet
			.union(VariableSet.minus(sb.liveIn(), sb.liveOut()), VariableSet.minus(sb.getKill(), sb.liveOut()))
			.getVariableNames();
	...
	public void constructHops(StatementBlock sb) {
		...
		if(!liveIn.getVariables().values().isEmpty()) {
			...
			for(String varName : liveIn.getVariables().keySet()) {
				...
				if(updated.containsVariable(varName) || gen.containsVariable(varName)) {
					...
					long actualDim1 = (var instanceof IndexedIdentifier) ? ((IndexedIdentifier) var).getOrigDim1() : var
						.getDim1();
					long actualDim2 = (var instanceof IndexedIdentifier) ? ((IndexedIdentifier) var).getOrigDim2() : var
						.getDim2();
					DataOp read = new DataOp(var.getName(), var.getDataType(), var.getValueType(),
						OpOpData.TRANSIENTREAD, null, actualDim1, actualDim2, var.getNnz(), var.getBlocksize());
		...
		for(int i = 0; i < sb.getNumStatements(); i++) {
			...
			if(current instanceof OutputStatement) {
				...
				if(target instanceof IndexedIdentifier) {
					throw new LanguageException(
						source.printErrorLocation() + ": Unsupported indexing expression in write statement. "
				...
				ae.setFileFormat((fmtExpr instanceof StringIdentifier) ? Expression
					.convertFormatType(fmtExpr.toString()) : FileFormat.UNKNOWN);
			...
			if(current instanceof AssignmentStatement) {
				...
				// CASE: regular assignment statement -- source is DML expression that is NOT user-defined or external
				// function
				if(!(source instanceof FunctionCallIdentifier)) {
					...
					if(!(target instanceof IndexedIdentifier)) {
						...
						if((statementId != null) && (statementId.intValue() == i)) {
							DataOp transientwrite = new DataOp(target.getName(), target.getDataType(),
								target.getValueType(), ae, OpOpData.TRANSIENTWRITE, null);
							transientwrite.setOutputParams(ae.getDim1(), ae.getDim2(), ae.getNnz(), ae.getUpdateType(),
								ae.getBlocksize());
					...
					// CASE: target is indexed identifier (left-hand side indexed expression)
						...
						if((statementId != null) && (statementId.intValue() == i)) {
							DataOp transientwrite = new DataOp(target.getName(), target.getDataType(),
								target.getValueType(), ae, OpOpData.TRANSIENTWRITE, null);
							transientwrite.setOutputParams(origDim1, origDim2, ae.getNnz(), ae.getUpdateType(),
								ae.getBlocksize());
							transientwrite.setParseInfo(target);
							updatedLiveOut.addVariable(target.getName(), target);
							output.add(transientwrite);
					...
					FunctionStatementBlock fsb = this._dmlProg.getFunctionStatementBlock(fci.getNamespace(),
						fci.getName());

					// error handling missing function
					if(fsb == null) {
						throw new LanguageException(source.printErrorLocation() + "function " + fci.getName()
					...
					List<String> inputNames = new ArrayList<>(
						fci.getParamExprs().stream().map(e -> e.getName()).collect(Collectors.toList()));
					...
					FunctionOp fcall = (target == null) ? new FunctionOp(ftype, fci.getNamespace(), fci.getName(),
						inputNames2, finputs, new String[] {}, false) : new FunctionOp(ftype, fci.getNamespace(),
							fci.getName(), inputNames2, finputs, new String[] {target.getName()}, false);
				...
				if(source instanceof FunctionCallIdentifier) {
					...
					((BuiltinFunctionExpression) source).multipleReturns()) {
					// construct input hops
					Hop fcall = processMultipleReturnBuiltinFunctionExpression((BuiltinFunctionExpression) source,
						mas.getTargetList(), ids);
					output.add(fcall);
				}
				else if(source instanceof ParameterizedBuiltinFunctionExpression &&
					((ParameterizedBuiltinFunctionExpression) source).multipleReturns()) {
					// construct input hops
					Hop fcall = processMultipleReturnParameterizedBuiltinFunctionExpression(
						(ParameterizedBuiltinFunctionExpression) source, mas.getTargetList(), ids);
					output.add(fcall);
				}
				else
					throw new LanguageException(
						"Class \"" + source.getClass() + "\" is not supported in Multiple Assignment statements");
			}
	...
	private static DataIdentifier getAccumulatorData(VariableSet liveIn, String varname) {
		...
		if(accum == null)
			throw new LanguageException(
				"Invalid accumulator assignment " + "to non-existing variable " + varname + ".");
		return accum;
	}

	private void appendDefaultArguments(FunctionStatement fstmt, List<String> inputNames, List<Hop> inputs,
		HashMap<String, Hop> ids) {
		...
		for(DataIdentifier di : fstmt.getInputParams()) {
			if(probeNames.contains(di.getName()))
				continue;
			Expression exp = fstmt.getInputDefault(di.getName());
			if(exp == null) {
				throw new LanguageException("Missing default expression for unspecified " + "function argument '"
					+ di.getName() + "' in call to function '" + fstmt.getName() + "'.");
			}
	...
	public void constructHopsForConditionalPredicate(StatementBlock passedSB) {
		...
		for(String varName : varsRead.getVariables().keySet()) {
			...
			if(var == null) {
				...
				read = new DataOp(var.getName(), var.getDataType(), var.getValueType(), OpOpData.TRANSIENTREAD, null,
					actualDim1, actualDim2, var.getNnz(), var.getBlocksize());
				read.setParseInfo(var);
			}
		...
		predicateHops = HopRewriteUtils.createDataOp(ProgramBlock.PRED_VAR, predicateHops, OpOpData.TRANSIENTWRITE);
	...
	/**
	 * Constructs all predicate Hops (for FROM, TO, INCREMENT) of an iterable predicate and assigns these Hops to the
	 * passed statement block.
	 * 
	...
	public void constructHopsForIterablePredicate(ForStatementBlock fsb) {
		...
		for(int i = 0; i < 3; i++) {
			Expression expr = (i == 0) ? ip.getFromExpr() : (i == 1) ? ip
				.getToExpr() : (ip.getIncrementExpr() != null) ? ip.getIncrementExpr() : null;
			...
			if(varsRead != null) {
				for(String varName : varsRead.getVariables().keySet()) {
					...
					if(var == null) {
						...
						long actualDim1 = (var instanceof IndexedIdentifier) ? ((IndexedIdentifier) var)
							.getOrigDim1() : var.getDim1();
						long actualDim2 = (var instanceof IndexedIdentifier) ? ((IndexedIdentifier) var)
							.getOrigDim2() : var.getDim2();
			...
			if(predicateHops != null)
				predicateHops = HopRewriteUtils.createDataOp(ProgramBlock.PRED_VAR, predicateHops,
	...
	/**
	 * Construct Hops from parse tree : Process Expression in an assignment statement
	 * 
	...
	private Hop processLeftIndexedExpression(Expression source, IndexedIdentifier target, HashMap<String, Hop> hops) {
		...
		if(targetOp == null) {
			throw new ParseException(target.printErrorLocation() + " must define matrix " + target.getName()
				+ " before indexing operations are allowed ");
		}

		if(sourceOp.getDataType().isMatrix() && source.getOutput().getDataType().isScalar())
			sourceOp.setDataType(DataType.SCALAR);

		Hop leftIndexOp = new LeftIndexingOp(target.getName(), target.getDataType(), ValueType.FP64, targetOp, sourceOp,
			ixRange[0], ixRange[1], ixRange[2], ixRange[3], target.getRowLowerEqualsUpper(),
	...
	private Hop processIndexingExpression(IndexedIdentifier source, DataIdentifier target, HashMap<String, Hop> hops) {
		...
		Hop indexOp = new IndexingOp(target.getName(), target.getDataType(), target.getValueType(),
			hops.get(source.getName()), ixRange[0], ixRange[1], ixRange[2], ixRange[3], source.getRowLowerEqualsUpper(),
	...
	private Hop[] getIndexingBounds(IndexedIdentifier ix, HashMap<String, Hop> hops, boolean lix) {
		Hop rowLowerHops = (ix.getRowLowerBound() != null) ? processExpression(ix.getRowLowerBound(), null,
			hops) : new LiteralOp(1);
		Hop colLowerHops = (ix.getColLowerBound() != null) ? processExpression(ix.getColLowerBound(), null,
			hops) : new LiteralOp(1);
		...
		if(ix.getRowUpperBound() != null)
			...
			rowUpperHops = ((lix ? ix.getDim1() : ix.getOrigDim1()) != -1) ? new LiteralOp(
				ix.getOrigDim1()) : new UnaryOp(ix.getName(), DataType.SCALAR, ValueType.INT64, OpOp1.NROW,
					hops.get(ix.getName()));
		...
		if(ix.getColUpperBound() != null)
			...
			colUpperHops = ((lix ? ix.getDim2() : ix.getOrigDim2()) != -1) ? new LiteralOp(
				ix.getOrigDim2()) : new UnaryOp(ix.getName(), DataType.SCALAR, ValueType.INT64, OpOp1.NCOL,
					hops.get(ix.getName()));
	...
	/**
	 * Construct Hops from parse tree : Process Binary Expression in an assignment statement
	 * 
	...
	private Hop processBinaryExpression(BinaryExpression source, DataIdentifier target, HashMap<String, Hop> hops) {
		...
		switch(source.getOpCode()) {
			...
			case POW:
			case INTDIV:
				currBop = new BinaryOp(target.getName(), target.getDataType(), target.getValueType(),
		...
		if(source.getRight() == null) {
			...
			if(source.getOpCode() == Expression.BooleanOp.LOGICALAND) {
				...
				throw new RuntimeException(
					source.printErrorLocation() + "Unknown boolean operation " + source.getOpCode());
			}
			currBop = new BinaryOp(target.getName(), target.getDataType(), target.getValueType(), op, left, right);
			currBop.setParseInfo(source);
			// setIdentifierParams(currBop,source.getOutput());
	...
	private static Hop constructDfHop(String name, DataType dt, ValueType vt, Builtins op,
		LinkedHashMap<String, Hop> paramHops) {
		...
		return new ParameterizedBuiltinOp(name, dt, vt, ParameterizedBuiltinFunctionExpression.pbHopMap.get(op),
			paramHops);
	}

	private Hop processMultipleReturnParameterizedBuiltinFunctionExpression(
		ParameterizedBuiltinFunctionExpression source, ArrayList<DataIdentifier> targetList,
		HashMap<String, Hop> hops) {
		...
		switch(source.getOpCode()) {
			case TRANSFORMENCODE:
				...
				outputs.add(new DataOp(outputNames[0], DataType.MATRIX, ValueType.FP64, inputs.get(0),
					OpOpData.FUNCTIONOUTPUT, inputs.get(0).getFilename()));
				outputs.add(new DataOp(outputNames[1], DataType.FRAME, ValueType.STRING, inputs.get(0),
					OpOpData.FUNCTIONOUTPUT, inputs.get(0).getFilename()));

				currBuiltinOp = new FunctionOp(ftype, nameSpace, source.getOpCode().toString(), null, inputs,
					outputNames, outputs);
				break;

			default:
				throw new ParseException(
					"Invaid Opcode in DMLTranslator:processMultipleReturnParameterizedBuiltinFunctionExpression(): "
						+ source.getOpCode());
		}
	...
	/**
	 * Construct Hops from parse tree : Process ParameterizedBuiltinFunction Expression in an assignment statement
	 * 
	 ...
	 * @return high-level operator
	 */
	private Hop processParameterizedBuiltinFunctionExpression(ParameterizedBuiltinFunctionExpression source,
		...
		switch(source.getOpCode()) {
			...
			case PCHISQ:
			case PEXP:
				currBuiltinOp = constructDfHop(target.getName(), target.getDataType(), target.getValueType(),
			...
			case ORDER:
				...
				currBuiltinOp = new ReorgOp(target.getName(), target.getDataType(), target.getValueType(), ReOrgOp.SORT,
					inputs);
				break;
			...
			case TOSTRING:
				...
				currBuiltinOp = !paramHops.get("target").getDataType().isScalar() ? new ParameterizedBuiltinOp(
					target.getName(), target.getDataType(), target.getValueType(), ParamBuiltinOp.TOSTRING,
					paramHops) : HopRewriteUtils.createBinary(paramHops.get("target"), new LiteralOp(""), OpOp2.PLUS);
				break;
	...
	/**
	 * Construct Hops from parse tree : Process ParameterizedExpression in a read/write/rand statement
	 * 
	...
	private Hop processDataExpression(DataExpression source, DataIdentifier target, HashMap<String, Hop> hops) {
		...
		switch(source.getOpCode()) {
			case READ:
				currBuiltinOp = new DataOp(target.getName(), target.getDataType(), target.getValueType(),
					OpOpData.PERSISTENTREAD, paramHops);
				((DataOp) currBuiltinOp)
					.setFileName(((StringIdentifier) source.getVarParam(DataExpression.IO_FILENAME)).getValue());
				break;
			...
			case MATRIX:
				...
				tmpMatrix.add(3, !paramHops.containsKey(DataExpression.RAND_DIMS) ? new LiteralOp("-1") : paramHops
					.get(DataExpression.RAND_DIMS));
			...
			case SQL:
				currBuiltinOp = new DataOp(target.getName(), target.getDataType(), target.getValueType(),
					OpOpData.SQLREAD, paramHops);
				break;

			case FEDERATED:
				currBuiltinOp = new DataOp(target.getName(), target.getDataType(), target.getValueType(),
					OpOpData.FEDERATED, paramHops);
				break;

			default:
				throw new ParseException(source.printErrorLocation() + "processDataExpression():: Unknown operation:  "
					+ source.getOpCode());
		}
		...
		if(source.getOpCode() == DataExpression.DataOp.READ)
			...
			if(source.getVarParam(DataExpression.ROWBLOCKCOUNTPARAM) != null)
				currBuiltinOp
					.setBlocksize(Integer.parseInt(source.getVarParam(DataExpression.ROWBLOCKCOUNTPARAM).toString()));
		}
		currBuiltinOp.setParseInfo(source);
	...
	/**
	 * Construct HOps from parse tree: process BuiltinFunction Expressions in MultiAssignment Statements. For all other
	 * builtin function expressions, <code>processBuiltinFunctionExpression()</code> is used.
	 * 
	 ...
	 * @return high-level operator
	 */
	private Hop processMultipleReturnBuiltinFunctionExpression(BuiltinFunctionExpression source,
		...
		switch(source.getOpCode()) {
			...
			case SVD:
				...
				for(int i = 0; i < targetList.size(); i++) {
					...
					Hop output = new DataOp(outputNames[i], DataType.MATRIX, ValueType.FP64, inputs.get(0),
						OpOpData.FUNCTIONOUTPUT, inputs.get(0).getFilename());
					outputs.add(output);
				}
			...
			case COMPRESS:
				...
				outputs.add(new DataOp(outputNamesCompress[0], DataType.MATRIX, ValueType.FP64, inputs.get(0),
					OpOpData.FUNCTIONOUTPUT, inputs.get(0).getFilename()));
				outputs.add(new DataOp(outputNamesCompress[1], DataType.FRAME, ValueType.STRING, inputs.get(0),
					OpOpData.FUNCTIONOUTPUT, inputs.get(0).getFilename()));

				// Create the hop for current function call
				currBuiltinOp = new FunctionOp(ftype, nameSpace, source.getOpCode().toString(), null, inputs,
					outputNamesCompress, outputs);
				break;
			default:
				throw new ParseException(
					"Invaid Opcode in DMLTranslator:processMultipleReturnBuiltinFunctionExpression(): "
						+ source.getOpCode());
		}
	...
	/**
	 * Construct Hops from parse tree : Process BuiltinFunction Expression in an assignment statement
	 * 
	 * @param source built-in function expression
	 * @param target data identifier
	 * @param hops   map of high-level operators
	...
	private Hop processBuiltinFunctionExpression(BuiltinFunctionExpression source, DataIdentifier target,
		...
		switch(source.getOpCode()) {
			...
			case EVAL:
			case EVALLIST:
				currBuiltinOp = new NaryOp(target.getName(), target.getDataType(), target.getValueType(), OpOpN.EVAL,
			...
			case COLSD:
				...
				currBuiltinOp = new AggUnaryOp(target.getName(), DataType.MATRIX, target.getValueType(), AggOp.VAR,
					Direction.Col, expr);
				currBuiltinOp = new UnaryOp(target.getName(), DataType.MATRIX, target.getValueType(), OpOp1.SQRT,
					currBuiltinOp);
				break;
			...
			case LENGTH:
				// If the dimensions are available at compile time, then create a LiteralOp (constant propagation)
				// Else create a UnaryOp so that a control program instruction is generated
				currBuiltinOp = (expr.getDim1() == -1 || expr.getDim2() == -1) ? new UnaryOp(target.getName(),
					target.getDataType(), target.getValueType(), OpOp1.LENGTH,
					expr) : new LiteralOp(expr.getDim1() * expr.getDim2());
				break;
			...
			case LIST:
				currBuiltinOp = new NaryOp(target.getName(), DataType.LIST, ValueType.UNKNOWN, OpOpN.LIST,
					processAllExpressions(source.getAllExpr(), hops));
				break;

			case EXISTS:
				currBuiltinOp = new UnaryOp(target.getName(), DataType.SCALAR, target.getValueType(), OpOp1.EXISTS,
					expr);
				break;
			...
			case MEAN:
				if(expr2 == null) {
					...
					currBuiltinOp = new TernaryOp(target.getName(), DataType.SCALAR, target.getValueType(),
						OpOp3.MOMENT, expr, expr2, orderHop);
				}
				break;

			case SD:
				// stdDev = sqrt(variance)
				currBuiltinOp = new AggUnaryOp(target.getName(), DataType.SCALAR, target.getValueType(), AggOp.VAR,
					Direction.RowCol, expr);
				HopRewriteUtils.setOutputParametersForScalar(currBuiltinOp);
				currBuiltinOp = new UnaryOp(target.getName(), DataType.SCALAR, target.getValueType(), OpOp1.SQRT,
					currBuiltinOp);
				break;
			...
			case MAX:
				...
				currBuiltinOp = (expr2 == null) ? new AggUnaryOp(target.getName(), DataType.SCALAR,
					target.getValueType(), AggOp.valueOf(source.getOpCode().name()), Direction.RowCol,
					expr) : (source.getAllExpr().length == 2) ? new BinaryOp(target.getName(), target.getDataType(),
						target.getValueType(), OpOp2.valueOf(source.getOpCode().name()), expr,
						expr2) : new NaryOp(target.getName(), target.getDataType(), target.getValueType(),
			...
			case PPRED:
				...
				currBuiltinOp = new BinaryOp(target.getName(), target.getDataType(), target.getValueType(), operation,
					expr, expr2);
				break;
			...
			case DIAG:
			case REV:
				currBuiltinOp = new ReorgOp(target.getName(), DataType.MATRIX, target.getValueType(),
			...
			case RBIND:
				...
				currBuiltinOp = (source.getAllExpr().length == 2) ? new BinaryOp(target.getName(), target.getDataType(),
					target.getValueType(), appendOp2, expr, expr2) : new NaryOp(target.getName(), target.getDataType(),
						target.getValueType(), appendOpN, processAllExpressions(source.getAllExpr(), hops));
				break;
				...
				switch(numTableArgs) {
					...
					case 4:
						...
						if(numTableArgs == 2)
							currBuiltinOp = new TernaryOp(target.getName(), target.getDataType(), target.getValueType(),
								OpOp3.CTABLE, expr, expr2, weightHop);
						else {
					...
					case 6:
						...
						if(numTableArgs == 3)
							...
							Hop outputEmptyBlocks = numTableArgs == 6 ? processExpression(source._args[5], null,
								hops) : new LiteralOp(true);
							currBuiltinOp = new TernaryOp(target.getName(), target.getDataType(), target.getValueType(),
					...
					default:
						throw new ParseException(
							"Invalid number of arguments " + numTableArgs + " to table() function.");
				}
				break;

			// data type casts
			case CAST_AS_SCALAR:
				currBuiltinOp = new UnaryOp(target.getName(), DataType.SCALAR, target.getValueType(),
					OpOp1.CAST_AS_SCALAR, expr);
				break;
			case CAST_AS_MATRIX:
				currBuiltinOp = new UnaryOp(target.getName(), DataType.MATRIX, target.getValueType(),
					OpOp1.CAST_AS_MATRIX, expr);
				break;
			case CAST_AS_FRAME:
				if(expr2 != null)
					currBuiltinOp = new BinaryOp(target.getName(), DataType.FRAME, target.getValueType(),
						OpOp2.CAST_AS_FRAME, expr, expr2);
				else
					currBuiltinOp = new UnaryOp(target.getName(), DataType.FRAME, target.getValueType(),
						OpOp1.CAST_AS_FRAME, expr);
				break;
			case CAST_AS_LIST:
				currBuiltinOp = new UnaryOp(target.getName(), DataType.LIST, target.getValueType(), OpOp1.CAST_AS_LIST,
					expr);
				break;

			// value type casts
			case CAST_AS_DOUBLE:
				currBuiltinOp = new UnaryOp(target.getName(), target.getDataType(), ValueType.FP64,
					OpOp1.CAST_AS_DOUBLE, expr);
				break;
			case CAST_AS_INT:
				currBuiltinOp = new UnaryOp(target.getName(), target.getDataType(), ValueType.INT64, OpOp1.CAST_AS_INT,
					expr);
				break;
			case CAST_AS_BOOLEAN:
				currBuiltinOp = new UnaryOp(target.getName(), target.getDataType(), ValueType.BOOLEAN,
					OpOp1.CAST_AS_BOOLEAN, expr);
				break;
			case LOCAL:
				currBuiltinOp = new UnaryOp(target.getName(), target.getDataType(), ValueType.FP64, OpOp1.LOCAL, expr);
				break;
			case COMPRESS:
				currBuiltinOp = new UnaryOp(target.getName(), target.getDataType(), ValueType.FP64, OpOp1.COMPRESS,
					expr);
				break;
			case DECOMPRESS:
				currBuiltinOp = new UnaryOp(target.getName(), target.getDataType(), ValueType.FP64, OpOp1.DECOMPRESS,
					expr);
				break;
			...
			case MAP:
				currBuiltinOp = new TernaryOp(target.getName(), target.getDataType(), target.getValueType(),
					OpOp3.valueOf(source.getOpCode().name()), expr, expr2, (expr3 == null) ? new LiteralOp(0L) : expr3);
				break;
			...
			case QUANTILE:
			case INTERQUANTILE:
				currBuiltinOp = (expr3 == null) ? new BinaryOp(target.getName(), target.getDataType(),
					target.getValueType(), OpOp2.valueOf(source.getOpCode().name()), expr,
					expr2) : new TernaryOp(target.getName(), target.getDataType(), target.getValueType(),
						OpOp3.valueOf(source.getOpCode().name()), expr, expr2, expr3);
				break;

			case IQM:
			case MEDIAN:
				currBuiltinOp = (expr2 == null) ? new UnaryOp(target.getName(), target.getDataType(),
					target.getValueType(), OpOp1.valueOf(source.getOpCode().name()),
					expr) : new BinaryOp(target.getName(), target.getDataType(), target.getValueType(),
						OpOp2.valueOf(source.getOpCode().name()), expr, expr2);
				break;

			case IFELSE:
				currBuiltinOp = new TernaryOp(target.getName(), target.getDataType(), target.getValueType(),
					OpOp3.IFELSE, expr, expr2, expr3);
				break;
			...
			case SOLVE:
				currBuiltinOp = new BinaryOp(target.getName(), target.getDataType(), target.getValueType(), OpOp2.SOLVE,
					expr, expr2);
				break;
			...
			case OUTER:
				...
				if(op == null)
					throw new HopsException(
						"Unsupported outer vector binary operation: " + ((LiteralOp) expr3).getStringValue());
				...
				currBuiltinOp.refreshSizeInformation(); // force size reevaluation according to 'outer' flag otherwise
														// danger of incorrect dims
				break;
	...
	private ArrayList<Hop> getALHopsForConvOpPoolingCOL2IM(Hop first, BuiltinFunctionExpression source, int skip,
		HashMap<String, Hop> hops) {
		...
		for(int i = skip; i < allExpr.length; i++) {
			if(i == 11) {
				ret.add(processExpression(allExpr[7], null, hops)); // Make number of channels of images and filter the
																	// same
			}
			else
	...
	private ArrayList<Hop> getALHopsForPoolingForwardIM2COL(Hop first, BuiltinFunctionExpression source, int skip,
		HashMap<String, Hop> hops) {
		ArrayList<Hop> ret = new ArrayList<>();
		ret.add(first);
		Expression[] allExpr = source.getAllExpr();
	...
	private ArrayList<Hop> getALHopsForConvOpPoolingIM2COL(Hop first, BuiltinFunctionExpression source, int skip,
		HashMap<String, Hop> hops) {
	...
	private ArrayList<Hop> getALHopsForConvOp(Hop first, BuiltinFunctionExpression source, int skip,
		HashMap<String, Hop> hops) {
		ArrayList<Hop> ret = new ArrayList<>();
		ret.add(first);
		Expression[] allExpr = source.getAllExpr();
	...
	private boolean prepareReadAfterWrite(DMLProgram prog, HashMap<String, DataIdentifier> pWrites) {
		...
		// process functions
		/*
		 * MB: for the moment we only support read-after-write in the main program for( FunctionStatementBlock fsb :
		 * prog.getFunctionStatementBlocks() ) ret |= prepareReadAfterWrite(fsb, pWrites);
		 */
