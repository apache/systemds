...
public class DMLTranslator {
	...
	private static Instruction deriveExitInstruction(StatementBlock sb) {
		Set<String> rmVars = VariableSet.union(VariableSet.minus(sb.liveIn(), sb.liveOut()),
	...
	public void constructHops(StatementBlock sb) {
		...
		for(int i = 0; i < sb.getNumStatements(); i++) {
			...
			if(current instanceof OutputStatement) {
				...
				ae.setFileFormat((fmtExpr instanceof StringIdentifier) ? Expression.convertFormatType(
					fmtExpr.toString()) : FileFormat.UNKNOWN);
			...
			if(current instanceof AssignmentStatement) {
				...
				if(!(source instanceof FunctionCallIdentifier)) {
					...
					if(fsb == null) {
						throw new LanguageException(
							source.printErrorLocation() + "function " + fci.getName() + " is undefined in namespace " +
								fci.getNamespace());
					}
					...
					List<Hop> finputs = new ArrayList<>(
						fci.getParamExprs().stream().map(e -> processExpression(e.getExpr(), null, ids))
	...
	private void appendDefaultArguments(FunctionStatement fstmt, List<String> inputNames, List<Hop> inputs,
		...
		for(DataIdentifier di : fstmt.getInputParams()) {
			...
			if(exp == null) {
				throw new LanguageException(
					"Missing default expression for unspecified " + "function argument '" + di.getName() +
	...
	public void constructHopsForIterablePredicate(ForStatementBlock fsb) {
		...
		for(int i = 0; i < 3; i++) {
			Expression expr = (i == 0) ? ip.getFromExpr() : (i == 1) ? ip.getToExpr() : (ip.getIncrementExpr() !=
				null) ? ip.getIncrementExpr() : null;
	...
	private Hop processIndexingExpression(IndexedIdentifier source, DataIdentifier target, HashMap<String, Hop> hops) {
		...
		Hop indexOp = new IndexingOp(target.getName(), dt, target.getValueType(), hops.get(source.getName()),
			ixRange[0], ixRange[1], ixRange[2], ixRange[3], source.getRowLowerEqualsUpper(),
	...
	private Hop processBooleanExpression(BooleanExpression source, DataIdentifier target, HashMap<String, Hop> hops) {
		...
		Hop left = !constLeft ? processExpression(source.getLeft(), null, hops) : new LiteralOp(
			Boolean.valueOf(source.getLeft().getText().toLowerCase()));
		Hop right = null;
		if(source.getRight() != null) {
			right = !constRight ? processExpression(source.getRight(), null, hops) : new LiteralOp(
				Boolean.valueOf(source.getRight().getText().toLowerCase()));
		}
	...
	private Hop processMultipleReturnParameterizedBuiltinFunctionExpression(
		...
		switch(source.getOpCode()) {
			case TRANSFORMENCODE:
				...
				outputs.add(
					new DataOp(outputNames[0], DataType.MATRIX, ValueType.FP64, inputs.get(0), OpOpData.FUNCTIONOUTPUT,
						inputs.get(0).getFilename()));
				outputs.add(
					new DataOp(outputNames[1], DataType.FRAME, ValueType.STRING, inputs.get(0), OpOpData.FUNCTIONOUTPUT,
						inputs.get(0).getFilename()));
	...
	private Hop processDataExpression(DataExpression source, DataIdentifier target, HashMap<String, Hop> hops) {
		...
		switch(source.getOpCode()) {
			case READ:
				...
				((DataOp) currBuiltinOp).setFileName(
					((StringIdentifier) source.getVarParam(DataExpression.IO_FILENAME)).getValue());
				break;
			...
			case MATRIX:
				...
				tmpMatrix.add(3, !paramHops.containsKey(DataExpression.RAND_DIMS) ? new LiteralOp("-1") : paramHops.get(
					DataExpression.RAND_DIMS));
			...
			default:
				throw new ParseException(
					source.printErrorLocation() + "processDataExpression():: Unknown operation:  " +
						source.getOpCode());
		}
		...
		if(source.getOpCode() == DataExpression.DataOp.READ)
			...
			if(source.getVarParam(DataExpression.ROWBLOCKCOUNTPARAM) != null)
				currBuiltinOp.setBlocksize(
					Integer.parseInt(source.getVarParam(DataExpression.ROWBLOCKCOUNTPARAM).toString()));
		}
		currBuiltinOp.setParseInfo(source);
	...
	private Hop processBuiltinFunctionExpression(BuiltinFunctionExpression source, DataIdentifier target,
		...
		switch(source.getOpCode()) {
			...
			case LENGTH:
				...
				currBuiltinOp = (expr.getDim1() == -1 || expr.getDim2() == -1) ? new UnaryOp(target.getName(),
					target.getDataType(), target.getValueType(), OpOp1.LENGTH, expr) : new LiteralOp(
					expr.getDim1() * expr.getDim2());
				break;
			...
			case MAX:
				...
				currBuiltinOp = (expr2 == null) ? new AggUnaryOp(target.getName(), DataType.SCALAR,
					target.getValueType(), AggOp.valueOf(source.getOpCode().name()), Direction.RowCol, expr) : (
					source.getAllExpr().length == 2) ? new BinaryOp(target.getName(), target.getDataType(),
					target.getValueType(), OpOp2.valueOf(source.getOpCode().name()), expr, expr2) : new NaryOp(
				...
				switch(numTableArgs) {
					...
					case 6:
						...
						if(numTableArgs == 3)
							...
							Hop outputEmptyBlocks =
								numTableArgs == 6 ? processExpression(source._args[5], null, hops) : new LiteralOp(
									true);
							currBuiltinOp = new TernaryOp(target.getName(), target.getDataType(), target.getValueType(),
			...
			case INTERQUANTILE:
				currBuiltinOp = (expr3 == null) ? new BinaryOp(target.getName(), target.getDataType(),
					target.getValueType(), OpOp2.valueOf(source.getOpCode().name()), expr, expr2) : new TernaryOp(
					target.getName(), target.getDataType(), target.getValueType(),
			...
			case MEDIAN:
				currBuiltinOp = (expr2 == null) ? new UnaryOp(target.getName(), target.getDataType(),
					target.getValueType(), OpOp1.valueOf(source.getOpCode().name()), expr) : new BinaryOp(
					target.getName(), target.getDataType(), target.getValueType(),
					OpOp2.valueOf(source.getOpCode().name()), expr, expr2);
				break;
			...
			case GET_CATEGORICAL_MASK:
				currBuiltinOp = new BinaryOp(target.getName(), DataType.MATRIX, ValueType.FP64,
					OpOp2.GET_CATEGORICAL_MASK, expr, expr2);
				break;
			default:
		...
		boolean isConvolution =
			source.getOpCode() == Builtins.CONV2D || source.getOpCode() == Builtins.CONV2D_BACKWARD_DATA ||
	...
	private ArrayList<Hop> getALHopsForConvOpPoolingCOL2IM(Hop first, BuiltinFunctionExpression source, int skip,
		...
		for(int i = skip; i < allExpr.length; i++) {
			if(i == 11) {
				ret.add(
					processExpression(allExpr[7], null, hops)); // Make number of channels of images and filter the same
			}
			else
