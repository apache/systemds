...
public class DataExpression extends DataIdentifier {
  ...
  public static final Set<String> READ_VALID_MTD_PARAM_NAMES = new HashSet<>(Arrays
    .asList(IO_FILENAME, READROWPARAM, READCOLPARAM, READNNZPARAM, FORMAT_TYPE, ROWBLOCKCOUNTPARAM,
      COLUMNBLOCKCOUNTPARAM, DATATYPEPARAM, VALUETYPEPARAM, SCHEMAPARAM, DESCRIPTIONPARAM, AUTHORPARAM, CREATEDPARAM,
  ...
  public static DataExpression getDataExpression(String functionName, ArrayList<ParameterExpression> passedParamExprs,
    ...
    if(LOG.isDebugEnabled()) {
      LOG.debug("getDataExpression: " + functionName + " " + passedParamExprs + " " + parseInfo + " " + errorListener);
    }
  ...
  private static DataExpression processReadDataExpression(String functionName,
    ...
    if(functionName.equals("read.csv"))
      dataExpr.addVarParam(DataExpression.FORMAT_TYPE, new StringIdentifier(FileFormat.CSV.toString(), parseInfo));

    if(functionName.equals("read.libsvm"))
      dataExpr.addVarParam(DataExpression.FORMAT_TYPE, new StringIdentifier(FileFormat.LIBSVM.toString(), parseInfo));
    ...
    for(int i = 1; i < passedParamExprs.size(); i++) {
      ...
      if(dataExpr.getVarParam(currName) != null) {
        errorListener
          .validationError(parseInfo, "attempted to add IOStatement parameter " + currName + " more than once");
        return null;
      }
  ...
  private static DataExpression processMatrixExpression(String functionName, List<ParameterExpression> passedParamExprs,
    ...
    if(passedParamExprs.size() < 3) {
      errorListener
        .validationError(parseInfo, "for matrix statement, must specify at least 3 arguments: data, rows, cols");
      return null;
    }
    ...
    if(unnamedParamCount > 1) {
      ...
      if(firstParam.getName() != null && !firstParam.getName().equals(DataExpression.RAND_DATA)) {
        errorListener
          .validationError(parseInfo, "matrix method must have data parameter as first parameter or unnamed parameter");
        return null;
      }
      else {
        dataExpr.addMatrixExprParam(DataExpression.RAND_DATA, passedParamExprs.get(0).getExpr());
  ...
  private static DataExpression processFrameExpression(String functionName, List<ParameterExpression> passedParamExprs,
    CustomErrorListener errorListener, ParseInfo parseInfo) {
    ...
    if(passedParamExprs.size() < 3) { // it will generate a frame with string schema
      errorListener
        .validationError(parseInfo, "for frame statement, must specify at least 3 arguments: data, rows and cols");
      return null;
    }
    ...
    if(unnamedParamCount > 1) {
      ...
      if(unnamedParamCount > 3) {
        errorListener
          .validationError(parseInfo, "for frame  statement, at most 4 arguments supported: data, rows, cols, schema");
        return null;
      }
    }
    else {
      ...
      if(firstParam.getName() != null && !firstParam.getName().equals(DataExpression.RAND_DATA)) {
        errorListener
          .validationError(parseInfo, "frame method must have data parameter as first parameter or unnamed parameter");
        return null;
      }
      else {
        dataExpr.addFrameExprParam(DataExpression.RAND_DATA, passedParamExprs.get(0).getExpr());
  ...
  private static DataExpression processTensorExpression(String functionName, List<ParameterExpression> passedParamExprs,
    CustomErrorListener errorListener, ParseInfo parseInfo) {
    ...
    if(unnamedParamCount > 1) {
      ...
      if(firstParam.getName() != null && !firstParam.getName().equals(DataExpression.RAND_DATA)) {
        errorListener
          .validationError(parseInfo, "tensor method must have data parameter as first parameter or unnamed parameter");
        return null;
      }
      else {
        dataExpr.addTensorExprParam(DataExpression.RAND_DATA, passedParamExprs.get(0).getExpr());
  ...
  private static DataExpression processFederatedExpression(String functionName,
    ...
    if(passedParamExprs.size() < 2) {
      errorListener
        .validationError(parseInfo, "for federated statement, must specify at least 2 arguments: addresses, ranges");
      return null;
    }
    if(unnamedParamCount > 0) {
      if(namedParamCount > 0) {
    ...
    if(fileNameExpr instanceof ConstIdentifier) {
      ...
      switch(op) {
        case PLUS:
          ...
          break;
        default:
          raiseValidateError("for read method, parameter " + IO_FILENAME + " can only be const string concatenations. ",
            conditional);
      }
    }
    else {
  ...
  @Override public void validateExpression(HashMap<String, DataIdentifier> ids,
    ...
    for(Entry<String, Expression> e : getVarParams().entrySet()) {
      ...
      if(s != null && !s.equals(RAND_DATA) && !s.equals(RAND_DIMS) && !s.equals(FED_ADDRESSES) && !s
        .equals(FED_RANGES) && !s.equals(DELIM_NA_STRINGS) && !s.equals(SCHEMAPARAM) && getVarParam(s).getOutput()
      ...
      case READ:
        ...
        if(dataTypeString != null && dataTypeString.equalsIgnoreCase(Statement.SCALAR_DATA_TYPE)) {
          if(getVarParam(READROWPARAM) != null || getVarParam(READCOLPARAM) != null || getVarParam(
            ...
            DELIM_FILL) != null || getVarParam(DELIM_FILL_VALUE) != null || getVarParam(DELIM_NA_STRINGS) != null) {
            raiseValidateError(
        ...
        if(shouldReadMTD //skip check for jmlc/mlcontext
          ...
          raiseValidateError("Read input file does not exist on " + fsext + ": " + inputFileName, conditional);
        }
        ...
        if(isCSV) {
          ...
          if(getVarParam(DELIM_DELIMITER) == null) {
            ...
            if((getVarParam(DELIM_DELIMITER) instanceof ConstIdentifier) && (!(getVarParam(
              ...
              raiseValidateError("For delimited file '" + getVarParam(DELIM_DELIMITER) + "' must be a string value ",
                conditional);
            }
          }
          ...
          if(getVarParam(DELIM_FILL_VALUE) == null) {
            ...
            if((getVarParam(DELIM_FILL_VALUE) instanceof ConstIdentifier) && (!(getVarParam(
              ...
              raiseValidateError("For delimited file '" + getVarParam(DELIM_FILL_VALUE) + "' must be a numeric value ",
                conditional);
            }
          }
          ...
          if(getVarParam(DELIM_HAS_HEADER_ROW) == null) {
            ...
            if((getVarParam(DELIM_HAS_HEADER_ROW) instanceof ConstIdentifier) && (!(getVarParam(
              ...
              raiseValidateError(
                "For delimited file '" + getVarParam(DELIM_HAS_HEADER_ROW) + "' must be a boolean value ", conditional);
            }
          }
          ...
          if(getVarParam(DELIM_FILL) == null) {
            ...
            if((getVarParam(DELIM_FILL) instanceof ConstIdentifier) && (!(getVarParam(
              ...
              raiseValidateError("For delimited file '" + getVarParam(DELIM_FILL) + "' must be a boolean value ",
                conditional);
            }
          }
          ...
          if(getVarParam(DELIM_NA_STRINGS) == null) {
            ...
            if((getVarParam(DELIM_NA_STRINGS) instanceof ConstIdentifier) && (!(getVarParam(
              ...
              raiseValidateError("For delimited file '" + getVarParam(DELIM_NA_STRINGS) + "' must be a string value ",
                conditional);
            }
        ...
        if(isLIBSVM) {
          ...
          if(!inferredFormatType) {
            for(String key : _varParams.keySet()) {
              if(!(key.equals(IO_FILENAME) || key.equals(FORMAT_TYPE) || key.equals(READROWPARAM) || key
                ...
                .equals(VALUETYPEPARAM) || key.equals(DELIM_DELIMITER) || key.equals(LIBSVM_INDEX_DELIM))) {
          ...
          if(getVarParam(LIBSVM_INDEX_DELIM) == null) {
            ...
            if((getVarParam(LIBSVM_INDEX_DELIM) instanceof ConstIdentifier) && (!(getVarParam(
              ...
              raiseValidateError("For delimited file '" + getVarParam(LIBSVM_INDEX_DELIM) + "' must be a string value ",
                conditional);
            }
          }
        }
        ...
        if(dataTypeString == null || dataTypeString.equalsIgnoreCase(Statement.MATRIX_DATA_TYPE) || dataTypeString
          ...
          if(!isCSV && !isLIBSVM && ConfigurationManager.getCompilerConfig()
            ...
            raiseValidateError("Missing or incomplete dimension information in read statement: " + mtdFileName,
              conditional, LanguageErrorCodes.INVALID_PARAMETERS);
          }
          ...
          if(isLIBSVM) {
            Long dim2 = (getVarParam(READCOLPARAM) == null) ? null : Long.valueOf(getVarParam(READCOLPARAM).toString());
            if(dim2 < 0 && ConfigurationManager.getCompilerConfig().getBool(ConfigType.REJECT_READ_WRITE_UNKNOWNS)) {
              raiseValidateError("Invalid dimension information in read statement", conditional,
                LanguageErrorCodes.INVALID_PARAMETERS);
            }
          ...
          String fmt = (getVarParam(FORMAT_TYPE) == null ? FileFormat.defaultFormatString() : getVarParam(FORMAT_TYPE)
            .toString());
          try {
        ...
        if(getOutput() instanceof IndexedIdentifier) {
          ...
          if(targetAsSeen == null) {
            raiseValidateError("cannot assign value to indexed identifier " + ((DataIdentifier) getOutput())
              .getName() + " without first initializing " + ((DataIdentifier) getOutput()).getName(), conditional);
          }
  ...
  private String fileNameCat(BinaryExpression expr, HashMap<String, ConstIdentifier> currConstVars, String filename,
    ...
    if(expr.getLeft() instanceof BinaryExpression && ((BinaryExpression) expr.getLeft()).getOpCode() == BinaryOp.PLUS) {
      ...
      raiseValidateError("Parameter " + IO_FILENAME + " only supports a const string or const string concatenations.",
        conditional);
    }
    ...
    // DRB: CHANGE
      ...
      .getDataType() == DataType.SCALAR && ((DataIdentifier) expr.getRight()).getValueType() == ValueType.STRING) {
      ...
      if(getVarParam(key.toString()) != null && (getVarParam(
        ...
        if(getVarParam(key.toString()) == null) {
          if((!key.toString().equalsIgnoreCase(DESCRIPTIONPARAM)) && (!key.toString()
            ...
            else if(key.toString().equalsIgnoreCase(DELIM_NA_STRINGS) || key.toString().equalsIgnoreCase(PRIVACY) || key
              .toString().equalsIgnoreCase(FINE_GRAINED_PRIVACY)) {
  ...
  public JSONObject readMetadataFile(String filename, boolean conditional) {
    ...
    if(exists && isDir) {
      ...
      for(FileStatus stat : HDFSTool.getDirectoryListing(filename)) {
        ...
        try(BufferedReader br = new BufferedReader(
          ...
          raiseValidateError(
            "for MTD file in directory, error parting part of MTD file with path " + childPath.toString() + ": " + e
              .getMessage(), conditional);
        }
      }
    }
    // CASE: filename points to a file
      ...
      try(
        BufferedReader br = new BufferedReader(new InputStreamReader(IOUtilFunctions.getFileSystem(path).open(path)))) {
  ...
  public boolean checkHasMatrixMarketFormat(String inputFileName, String mtdFileName, boolean conditional) {
    ...
    if(HDFSTool.existsFileOnHDFS(inputFileName) && !HDFSTool.isDirectory(inputFileName)) {
      ...
      try(
        BufferedReader in = new BufferedReader(new InputStreamReader(IOUtilFunctions.getFileSystem(path).open(path)))) {
