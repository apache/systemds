...
public class DataExpression extends DataIdentifier {
	...
	public static final Set<String> RAND_VALID_PARAM_NAMES = new HashSet<>(Arrays
		.asList(RAND_ROWS, RAND_COLS, RAND_DIMS, RAND_MIN, RAND_MAX, RAND_SPARSITY, RAND_SEED, RAND_PDF, RAND_LAMBDA));
	 ...
	 */
	public static final Set<String> READ_VALID_MTD_PARAM_NAMES = new HashSet<>(Arrays
		.asList(IO_FILENAME, READROWPARAM, READCOLPARAM, READNNZPARAM, FORMAT_TYPE, ROWBLOCKCOUNTPARAM,
			COLUMNBLOCKCOUNTPARAM, DATATYPEPARAM, VALUETYPEPARAM, SCHEMAPARAM, DESCRIPTIONPARAM, AUTHORPARAM,
			CREATEDPARAM,
	 ...
	 */
	public static final Set<String> READ_VALID_PARAM_NAMES = new HashSet<>(Arrays
		.asList(IO_FILENAME, READROWPARAM, READCOLPARAM, FORMAT_TYPE, DATATYPEPARAM, VALUETYPEPARAM, SCHEMAPARAM,
	...
	public static DataExpression getDataExpression(String functionName, ArrayList<ParameterExpression> passedParamExprs,
		...
		if(LOG.isDebugEnabled()) {
			LOG.debug(
				"getDataExpression: " + functionName + " " + passedParamExprs + " " + parseInfo + " " + errorListener);
		}
	...
	private static DataExpression processReadDataExpression(String functionName,
		...
		if(functionName.equals("readMM"))
			dataExpr.addVarParam(DataExpression.FORMAT_TYPE, new StringIdentifier(FileFormat.MM.toString(), parseInfo));

		if(functionName.equals("read.csv"))
			dataExpr
				.addVarParam(DataExpression.FORMAT_TYPE, new StringIdentifier(FileFormat.CSV.toString(), parseInfo));

		if(functionName.equals("read.libsvm"))
			dataExpr
				.addVarParam(DataExpression.FORMAT_TYPE, new StringIdentifier(FileFormat.LIBSVM.toString(), parseInfo));
		...
		for(int i = 1; i < passedParamExprs.size(); i++) {
			...
			if(!isValidName) {
				errorListener
					.validationError(parseInfo, "attempted to add invalid read statement parameter " + currName);
				return null;
			}
	...
	private static DataExpression processMatrixExpression(String functionName,
		...
		if(unnamedParamCount > 1) {
			if(namedParamCount > 0) {
				errorListener
					.validationError(parseInfo, "for matrix statement, cannot mix named and unnamed parameters");
				return null;
			}
			...
			if(unnamedParamCount > 5) {
				errorListener.validationError(parseInfo,
					"for matrix statement, at most 5 arguments supported: data, rows, cols, byrow, dimname");
				return null;
			}
		}
		else {
			...
			if(firstParam.getName() != null && !firstParam.getName().equals(DataExpression.RAND_DATA)) {
				errorListener.validationError(parseInfo,
					"matrix method must have data parameter as first parameter or unnamed parameter");
				return null;
			}
			else {
				dataExpr.addMatrixExprParam(DataExpression.RAND_DATA, passedParamExprs.get(0).getExpr());
			...
			for(int i = 1; i < passedParamExprs.size(); i++) {
				if(passedParamExprs.get(i).getName() == null) {
					errorListener.validationError(parseInfo,
						"for matrix statement, cannot mix named and unnamed parameters, only data parameter can be unnammed");
					return null;
				}
				else {
	...
	private static DataExpression processFrameExpression(String functionName,
		...
		if(passedParamExprs.size() < 3) { // it will generate a frame with string schema
			errorListener.validationError(parseInfo,
				"for frame statement, must specify at least 3 arguments: data, rows and cols");
			return null;
		}
		...
		if(unnamedParamCount > 1) {
			if(namedParamCount > 0) {
				errorListener
					.validationError(parseInfo, "for frame statement, cannot mix named and unnamed parameters");
				return null;
			}
			if(unnamedParamCount < 3) {
				errorListener
					.validationError(parseInfo, "for frame statement, must specify at least 3 arguments: rows, cols");
				return null;
			}
			...
			if(unnamedParamCount > 3) {
				errorListener.validationError(parseInfo,
					"for frame  statement, at most 4 arguments supported: data, rows, cols, schema");
				return null;
			}
		}
		else {
			...
			if(firstParam.getName() != null && !firstParam.getName().equals(DataExpression.RAND_DATA)) {
				errorListener.validationError(parseInfo,
					"frame method must have data parameter as first parameter or unnamed parameter");
				return null;
			}
			else {
				dataExpr.addFrameExprParam(DataExpression.RAND_DATA, passedParamExprs.get(0).getExpr());
			...
			for(int i = 1; i < passedParamExprs.size(); i++) {
				if(passedParamExprs.get(i).getName() == null) {
					errorListener.validationError(parseInfo,
						"for frame statement, cannot mix named and unnamed parameters, only data parameter can be unnammed");
					return null;
				}
				else {
	...
	private static DataExpression processTensorExpression(String functionName,
		...
		if(passedParamExprs.size() < 2) {
			errorListener
				.validationError(parseInfo, "for tensor statement, must specify at least 2 arguments: data, dims[]");
			return null;
		}
		if(unnamedParamCount > 1) {
			if(namedParamCount > 0) {
				errorListener
					.validationError(parseInfo, "for tensor statement, cannot mix named and unnamed parameters");
				return null;
			}
			...
			if(unnamedParamCount > 4) {
				errorListener.validationError(parseInfo,
					"for tensor statement, at most 4 arguments supported: data, dims, byrow, dimname");
				return null;
			}
		}
		else {
			...
			if(firstParam.getName() != null && !firstParam.getName().equals(DataExpression.RAND_DATA)) {
				errorListener.validationError(parseInfo,
					"tensor method must have data parameter as first parameter or unnamed parameter");
				return null;
			}
			else {
				dataExpr.addTensorExprParam(DataExpression.RAND_DATA, passedParamExprs.get(0).getExpr());
			...
			for(int i = 1; i < passedParamExprs.size(); i++) {
				if(passedParamExprs.get(i).getName() == null) {
					errorListener.validationError(parseInfo,
						"for tensor statement, cannot mix named and unnamed parameters, only data parameter can be unnammed");
					return null;
				}
				else {
	...
	private static DataExpression processSQLExpression(String functionName, List<ParameterExpression> passedParamExprs,
		CustomErrorListener errorListener, ParseInfo parseInfo) {
		...
		if(passedParamExprs.size() < 2) {
			errorListener
				.validationError(parseInfo, "for sql statement, must specify at least 2 arguments: conn, query");
			return null;
		}
		if(unnamedParamCount > 0) {
			...
			if(unnamedParamCount == 2 || unnamedParamCount == 4) {
				...
				errorListener.validationError(parseInfo,
					"for sql statement, " + "at most 4 arguments supported: conn, user, password, query");
				return null;
			}
		}
		else {
	...
	private static DataExpression processFederatedExpression(String functionName,
		...
		if(unnamedParamCount > 0) {
			if(namedParamCount > 0) {
				errorListener
					.validationError(parseInfo, "for federated statement, cannot mix named and unnamed parameters");
				return null;
			}
	...
	public void addRandExprParam(String paramName, Expression paramValue) {
		...
		if(!found) {
			raiseValidateError(
				"unexpected parameter \"" + paramName + "\". Legal parameters for Rand statement are " + "(capitalization-sensitive): " + RAND_ROWS + ", " + RAND_COLS + ", " + RAND_MIN + ", " + RAND_MAX + ", " + RAND_SPARSITY + ", " + RAND_SEED + ", " + RAND_PDF + ", " + RAND_LAMBDA);
		}
	...
	public void addMatrixExprParam(String paramName, Expression paramValue) {
		...
		if(!found) {
			raiseValidateError(
				"unexpected parameter \"" + paramName + "\". Legal parameters for  matrix statement are " + "(capitalization-sensitive): " + RAND_DATA + ", " + RAND_ROWS + ", " + RAND_COLS + ", " + RAND_BY_ROW);
		}
		if(getVarParam(paramName) != null) {
	...
	public void addFrameExprParam(String paramName, Expression paramValue) {
		...
		if(!found) {
			raiseValidateError(
				"unexpected parameter \"" + paramName + "\". Legal parameters for  frame statement are " + "(capitalization-sensitive): " + RAND_DATA + ", " + RAND_ROWS + ", " + RAND_COLS + ", " + SCHEMAPARAM);
		}
		if(getVarParam(paramName) != null) {
	...
	public void addTensorExprParam(String paramName, Expression paramValue) {
		...
		if(!found) {
			raiseValidateError(
				"unexpected parameter \"" + paramName + "\". Legal parameters for tensor statement are " + "(capitalization-sensitive): " + RAND_DATA + ", " + RAND_DIMS + ", " + RAND_BY_ROW + ", " + RAND_DIMNAMES);
		}
		if(getVarParam(paramName) != null) {
	...
	public void addSqlExprParam(String paramName, Expression paramValue) {
		...
		if(!found) {
			raiseValidateError(
				"unexpected parameter \"" + paramName + "\". Legal parameters for sql statement are " + "(capitalization-sensitive): " + SQL_CONN + ", " + SQL_USER + ", " + SQL_PASS + ", " + SQL_QUERY);
		}
		if(getVarParam(paramName) != null) {
	...
	public void addFederatedExprParam(String paramName, Expression paramValue) {
		...
		if(!found)
			raiseValidateError(
				"unexpected parameter \"" + paramName + "\". Legal parameters for federated statement are " + "(capitalization-sensitive): " + FED_ADDRESSES + ", " + FED_RANGES + ", " + FED_TYPE);
		if(getVarParam(paramName) != null)
	...
	public DataExpression(ParserRuleContext ctx, DataOp op, HashMap<String, Expression> varParams, String filename) {
		_opcode = op;
		_varParams = varParams;
		setCtxValuesAndFilename(ctx, filename);
	}

	@Override public Expression rewriteExpression(String prefix) {
	...
	private String getInputFileName(HashMap<String, ConstIdentifier> currConstVars, boolean conditional) {
		...
		if(fileNameExpr instanceof ConstIdentifier) {
			...
			switch(op) {
				...
				default:
					raiseValidateError(
						"for read method, parameter " + IO_FILENAME + " can only be const string concatenations. ",
						conditional);
			}
		}
		else {
			raiseValidateError(
				"for read method, parameter " + IO_FILENAME + " can only be a const string or const string concatenations. ",
				conditional);
		}
	 ...
	 * statement
	 */
	@Override public void validateExpression(HashMap<String, DataIdentifier> ids,
		...
		for(Entry<String, Expression> e : getVarParams().entrySet()) {
			...
			if(s != null && !s.equals(RAND_DATA) && !s.equals(RAND_DIMS) && !s.equals(FED_ADDRESSES) && !s
				.equals(FED_RANGES) && !s.equals(DELIM_NA_STRINGS) && !s.equals(SCHEMAPARAM) && getVarParam(s)
				.getOutput().getDataType() != DataType.SCALAR) {
				raiseValidateError("Non-scalar data types are not supported for data expression.", conditional,
					LanguageErrorCodes.INVALID_PARAMETERS);
			}
		}
		...
		if(dataParam1 == null && (getOpCode().equals(DataOp.MATRIX) || getOpCode().equals(DataOp.TENSOR))) {
			raiseValidateError("for matrix, frame or tensor, must defined data parameter", conditional,
				LanguageErrorCodes.INVALID_PARAMETERS);
		}
		// We need to remember the operation if we replace the OpCode by rand so we have the correct output
		if(dataParam1 != null && dataParam1.getOutput() != null && dataParam1.getOutput()
			.getDataType() == DataType.SCALAR && (_opcode == DataOp.MATRIX || _opcode == DataOp.TENSOR)/*&& dataParam instanceof ConstIdentifier*/) {
		...
		switch(this.getOpCode()) {
			...
			case READ:
				if(getVarParam(DATATYPEPARAM) != null && !(getVarParam(DATATYPEPARAM) instanceof StringIdentifier)) {
					raiseValidateError(
						"for read statement, parameter " + DATATYPEPARAM + " can only be a string. " + "Valid values are: " + Statement.MATRIX_DATA_TYPE + ", " + Statement.SCALAR_DATA_TYPE,
						conditional);
				}

				String dataTypeString = (getVarParam(DATATYPEPARAM) == null) ? null : getVarParam(DATATYPEPARAM)
					.toString();
				...
				if(dataTypeString != null && dataTypeString.equalsIgnoreCase(Statement.SCALAR_DATA_TYPE)) {
					if(getVarParam(READROWPARAM) != null || getVarParam(READCOLPARAM) != null || getVarParam(
						ROWBLOCKCOUNTPARAM) != null || getVarParam(COLUMNBLOCKCOUNTPARAM) != null || getVarParam(
						FORMAT_TYPE) != null || getVarParam(DELIM_DELIMITER) != null || getVarParam(
						LIBSVM_INDEX_DELIM) != null || getVarParam(DELIM_HAS_HEADER_ROW) != null || getVarParam(
						DELIM_FILL) != null || getVarParam(DELIM_FILL_VALUE) != null || getVarParam(
						DELIM_NA_STRINGS) != null) {
						raiseValidateError(
							"Invalid parameters in read statement of a scalar: " + toString() + ". Only " + VALUETYPEPARAM + " is allowed.",
				...
				boolean shouldReadMTD = _checkMetadata && (!ConfigurationManager
					.getCompilerConfigFlag(ConfigType.IGNORE_READ_WRITE_METADATA) || HDFSTool
					.existsFileOnHDFS(mtdFileName)); // existing mtd file
				...
				if(shouldReadMTD //skip check for jmlc/mlcontext
					...
					raiseValidateError("Read input file does not exist on " + fsext + ": " + inputFileName,
						conditional);
				}
				...
				String formatTypeString = (getVarParam(FORMAT_TYPE) == null) ? null : getVarParam(FORMAT_TYPE)
					.toString();
				...
				if(formatTypeString != null && formatTypeString.equalsIgnoreCase(FileFormat.MM.toString())) {
					...
					if(headerLines != null && headerLines.length >= 2) {
						...
						if(sizeInfo.length != 3) {
							raiseValidateError(
								"Unsupported size line in MatrixMarket file: " + headerLines[1] + ". Only supported format in MatrixMarket file has size line: <NUM ROWS> <NUM COLS> <NUM NON-ZEROS>, where each value is an integer.",
								conditional);
						}
						...
						if(rowsCount < 0)
							...
							if(rowsCount2 != rowsCount)
								raiseValidateError(
									"MM file: invalid specified number of rows: " + rowsCount2 + " vs " + rowsCount);
						}
						...
						if(colsCount < 0)
							...
							if(colsCount2 != colsCount)
								raiseValidateError(
									"MM file: invalid specified number of columns: " + colsCount2 + " vs " + colsCount);
						}
						...
						if(nnzCount < 0)
							...
							if(nnzCount2 != nnzCount)
								raiseValidateError(
									"MM file: invalid specified number of non-zeros: " + nnzCount2 + " vs " + nnzCount);
						}
						addVarParam(READNNZPARAM, new IntIdentifier(nnzCount, this));
					}
				}

				boolean isCSV = (formatTypeString != null && formatTypeString
					.equalsIgnoreCase(FileFormat.CSV.toString()));
				...
				if(isCSV) {
					...
					if(getVarParam(DELIM_DELIMITER) == null) {
						...
						if((getVarParam(DELIM_DELIMITER) instanceof ConstIdentifier) && (!(getVarParam(
							DELIM_DELIMITER) instanceof StringIdentifier))) {
							raiseValidateError(
								"For delimited file '" + getVarParam(DELIM_DELIMITER) + "' must be a string value ",
								conditional);
						}
					}
					...
					if(getVarParam(DELIM_FILL_VALUE) == null) {
						...
						if((getVarParam(DELIM_FILL_VALUE) instanceof ConstIdentifier) && (!(getVarParam(
							DELIM_FILL_VALUE) instanceof IntIdentifier || getVarParam(
							DELIM_FILL_VALUE) instanceof DoubleIdentifier))) {
							raiseValidateError(
								"For delimited file '" + getVarParam(DELIM_FILL_VALUE) + "' must be a numeric value ",
								conditional);
						}
					}
					...
					if(getVarParam(DELIM_HAS_HEADER_ROW) == null) {
						...
						if((getVarParam(DELIM_HAS_HEADER_ROW) instanceof ConstIdentifier) && (!(getVarParam(
							DELIM_HAS_HEADER_ROW) instanceof BooleanIdentifier))) {
							raiseValidateError("For delimited file '" + getVarParam(
								DELIM_HAS_HEADER_ROW) + "' must be a boolean value ", conditional);
						}
					}
					...
					if(getVarParam(DELIM_FILL) == null) {
						...
						if((getVarParam(DELIM_FILL) instanceof ConstIdentifier) && (!(getVarParam(
							DELIM_FILL) instanceof BooleanIdentifier))) {
							raiseValidateError(
								"For delimited file '" + getVarParam(DELIM_FILL) + "' must be a boolean value ",
								conditional);
						}
					}
					...
					if(getVarParam(DELIM_NA_STRINGS) == null) {
						...
						if((getVarParam(DELIM_NA_STRINGS) instanceof ConstIdentifier) && (!(getVarParam(
							DELIM_NA_STRINGS) instanceof StringIdentifier))) {
							raiseValidateError(
								"For delimited file '" + getVarParam(DELIM_NA_STRINGS) + "' must be a string value ",
								conditional);
						}
				...
				isLIBSVM = (formatTypeString != null && formatTypeString
					.equalsIgnoreCase(FileFormat.LIBSVM.toString()));
				if(isLIBSVM) {
					...
					if(!inferredFormatType) {
						for(String key : _varParams.keySet()) {
							if(!(key.equals(IO_FILENAME) || key.equals(FORMAT_TYPE) || key.equals(READROWPARAM) || key
								.equals(READCOLPARAM) || key.equals(READNNZPARAM) || key.equals(DATATYPEPARAM) || key
								.equals(VALUETYPEPARAM) || key.equals(DELIM_DELIMITER) || key
								.equals(LIBSVM_INDEX_DELIM))) {
								String msg = "Only parameters allowed are: " + IO_FILENAME + "," + READROWPARAM + "," + READCOLPARAM + "," + DELIM_DELIMITER + "," + LIBSVM_INDEX_DELIM;
					...
					if(getVarParam(LIBSVM_INDEX_DELIM) == null) {
						...
						if((getVarParam(LIBSVM_INDEX_DELIM) instanceof ConstIdentifier) && (!(getVarParam(
							LIBSVM_INDEX_DELIM) instanceof StringIdentifier))) {
							raiseValidateError(
								"For delimited file '" + getVarParam(LIBSVM_INDEX_DELIM) + "' must be a string value ",
								conditional);
						}
					}
				}
				...
				if(dataTypeString == null || dataTypeString
					.equalsIgnoreCase(Statement.MATRIX_DATA_TYPE) || dataTypeString
					.equalsIgnoreCase(Statement.FRAME_DATA_TYPE)) {
					...
					if(!isCSV && !isLIBSVM && ConfigurationManager.getCompilerConfig()
						...
						raiseValidateError(
							"Missing or incomplete dimension information in read statement: " + mtdFileName,
							conditional, LanguageErrorCodes.INVALID_PARAMETERS);
					}

					if(getVarParam(READROWPARAM) instanceof ConstIdentifier && getVarParam(
						READCOLPARAM) instanceof ConstIdentifier) {
						// these are strings that are long values
						Long dim1 = (getVarParam(READROWPARAM) == null) ? null : Long
							.valueOf(getVarParam(READROWPARAM).toString());
						Long dim2 = (getVarParam(READCOLPARAM) == null) ? null : Long
							.valueOf(getVarParam(READCOLPARAM).toString());
						if(!isCSV && (dim1 < 0 || dim2 < 0) && ConfigurationManager.getCompilerConfig()
							.getBool(ConfigType.REJECT_READ_WRITE_UNKNOWNS)) {
							raiseValidateError("Invalid dimension information in read statement", conditional,
								LanguageErrorCodes.INVALID_PARAMETERS);
						}
						...
						if(dim1 != null && dim2 != null) {
							...
							raiseValidateError("Partial dimension information in read statement", conditional,
								LanguageErrorCodes.INVALID_PARAMETERS);
						}
					}
					...
					String fmt = (getVarParam(FORMAT_TYPE) == null ? FileFormat.defaultFormatString() : getVarParam(
					...
					if(getVarParam(ROWBLOCKCOUNTPARAM) instanceof ConstIdentifier && getVarParam(
						COLUMNBLOCKCOUNTPARAM) instanceof ConstIdentifier) {
						Integer rowBlockCount = (getVarParam(ROWBLOCKCOUNTPARAM) == null) ? null : Integer
							.valueOf(getVarParam(ROWBLOCKCOUNTPARAM).toString());
					...
					if((getOutput().getFileFormat().isTextFormat() || !isMatrix) && getOutput().getBlocksize() != -1) {
						raiseValidateError(
							"Invalid block dimensions (" + getOutput().getBlocksize() + ") when format=" + getVarParam(
								FORMAT_TYPE) + " in \"" + this.toString() + "\".", conditional);
					}
					...
					raiseValidateError(
						"Unknown Data Type " + dataTypeString + ". Valid  values: " + Statement.SCALAR_DATA_TYPE + ", " + Statement.MATRIX_DATA_TYPE,
						conditional, LanguageErrorCodes.INVALID_PARAMETERS);
				}
				...
				if(getVarParam(VALUETYPEPARAM) != null && !(getVarParam(VALUETYPEPARAM) instanceof StringIdentifier)) {
					raiseValidateError(
						"for read method, parameter " + VALUETYPEPARAM + " can only be a string. " + "Valid values are: " + Statement.DOUBLE_VALUE_TYPE + ", " + Statement.INT_VALUE_TYPE + ", " + Statement.BOOLEAN_VALUE_TYPE + ", " + Statement.STRING_VALUE_TYPE,
						conditional);
				}
				// Identify the value type (used only for read method)
				String valueTypeString = getVarParam(VALUETYPEPARAM) == null ? null : getVarParam(VALUETYPEPARAM)
					.toString();
				if(valueTypeString != null) {
					if(valueTypeString.equalsIgnoreCase(Statement.DOUBLE_VALUE_TYPE)) {
						...
						raiseValidateError(
							"Unknown Value Type " + valueTypeString + ". Valid values are: " + Statement.DOUBLE_VALUE_TYPE + ", " + Statement.INT_VALUE_TYPE + ", " + Statement.BOOLEAN_VALUE_TYPE + ", " + Statement.STRING_VALUE_TYPE,
							conditional);
					}
				}
				else {
			...
			case WRITE:
				...
				if(getVarParam(FORMAT_TYPE) == null || getVarParam(FORMAT_TYPE).toString()
					.equalsIgnoreCase(FileFormat.CSV.toString())) {
					if(getVarParam(DELIM_DELIMITER) == null) {
						addVarParam(DELIM_DELIMITER, new StringIdentifier(DEFAULT_DELIM_DELIMITER, this));
					}
			...
			case RAND:
				...
				validateParams(conditional, RAND_VALID_PARAM_NAMES,
					"Legal parameters for Rand statement are " + "(capitalization-sensitive): " + RAND_ROWS + ", " + RAND_COLS + ", " + RAND_DIMS + ", " + RAND_MIN + ", " + RAND_MAX + ", " + RAND_SPARSITY + ", " + RAND_SEED + ", " + RAND_PDF + ", " + RAND_LAMBDA);

				//parameters w/ support for variable inputs
				if(getVarParam(RAND_ROWS) instanceof StringIdentifier || getVarParam(
					RAND_ROWS) instanceof BooleanIdentifier) {
					raiseValidateError("for Rand statement " + RAND_ROWS + " has incorrect value type", conditional);
				}

				if(getVarParam(RAND_COLS) instanceof StringIdentifier || getVarParam(
					RAND_COLS) instanceof BooleanIdentifier) {
					raiseValidateError("for Rand statement " + RAND_COLS + " has incorrect value type", conditional);
				}

				if(getVarParam(RAND_DIMS) instanceof IntIdentifier || getVarParam(
					RAND_DIMS) instanceof DoubleIdentifier || getVarParam(RAND_DIMS) instanceof BooleanIdentifier) {
					raiseValidateError("for Rand statement " + RAND_DIMS + " has incorrect value type", conditional);
				}

				if(getVarParam(RAND_SEED) instanceof StringIdentifier || getVarParam(
					RAND_SEED) instanceof BooleanIdentifier) {
				...
				if((getVarParam(RAND_MAX) instanceof StringIdentifier && !_strInit) || (getVarParam(
					RAND_MAX) instanceof BooleanIdentifier && !isTensorOperation)) {
					raiseValidateError("for Rand statement " + RAND_MAX + " has incorrect value type", conditional);
				}

				if((getVarParam(RAND_MIN) instanceof StringIdentifier && !_strInit) || getVarParam(
					RAND_MIN) instanceof BooleanIdentifier && !isTensorOperation)
				...
				if(!((lambda instanceof DataIdentifier || lambda instanceof ConstIdentifier) && (lambda.getOutput()
					.getValueType() == ValueType.FP64 || lambda.getOutput().getValueType() == ValueType.INT64))) {
				...
				if(!isTensorOperation) {
					...
					if(rowsExpr instanceof IntIdentifier) {
						if(((IntIdentifier) rowsExpr).getValue() < 0) {
							raiseValidateError(
								"In rand statement, can only assign rows a long " + "(integer) value >= 0 -- attempted to assign value: " + ((IntIdentifier) rowsExpr)
						...
						if(((DoubleIdentifier) rowsExpr).getValue() < 0) {
							raiseValidateError(
								"In rand statement, can only assign rows a long " + "(integer) value >= 0 -- attempted to assign value: " + rowsExpr
									.toString(), conditional);
						}
						...
						if(currConstVars.containsKey(identifierName)) {
							...
							if(constValue instanceof IntIdentifier) {
								...
								if(((IntIdentifier) constValue).getValue() < 0) {
									raiseValidateError(
										"In rand statement, can only assign rows a long " + "(integer) value >= 0 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
							...
							// handle double constant
								...
								if(((DoubleIdentifier) constValue).getValue() < 0) {
									raiseValidateError(
										"In rand statement, can only assign rows a long " + "(double) value >= 0 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
								// update row expr with new IntIdentifier (rounded down)
								long roundedValue = Double
									.valueOf(Math.floor(((DoubleIdentifier) constValue).getValue())).longValue();
								rowsExpr = new IntIdentifier(roundedValue, this);
								addVarParam(RAND_ROWS, rowsExpr);
								rowsLong = roundedValue;
							}
					...
					if(colsExpr instanceof IntIdentifier) {
						if(((IntIdentifier) colsExpr).getValue() < 0) {
							raiseValidateError(
								"In rand statement, can only assign cols a long " + "(integer) value >= 0 -- attempted to assign value: " + colsExpr
									.toString(), conditional);
						}
						...
						if(currConstVars.containsKey(identifierName)) {
							...
							if(constValue instanceof IntIdentifier) {
								if(((IntIdentifier) constValue).getValue() < 0) {
									raiseValidateError(
										"In rand statement, can only assign cols a long " + "(integer) value >= 0 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
							...
							// handle double constant
								...
								if(((DoubleIdentifier) constValue).getValue() < 0) {
									raiseValidateError(
										"In rand statement, can only assign cols a long " + "(double) value >= 0 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
				...
				if(minExpr instanceof DataIdentifier && !(minExpr instanceof IndexedIdentifier)) {
					...
					if(currConstVars.containsKey(identifierName)) {
						...
						// handle double constant
							...
							// exception -- rows must be integer or double constant
							raiseValidateError(
								"In rand statement, can only assign min a numerical " + "value -- attempted to assign: " + constValue
									.toString(), conditional);
						}
					}
					else {
				...
				if(maxExpr instanceof DataIdentifier && !(maxExpr instanceof IndexedIdentifier)) {
					...
					if(currConstVars.containsKey(identifierName)) {
						...
						// handle double constant
							...
							// exception -- rows must be integer or double constant
							raiseValidateError(
								"In rand statement, can only assign max a numerical " + "value -- attempted to assign: " + constValue
									.toString(), conditional);
						}
					}
					else {
				...
				if(getOutput() instanceof IndexedIdentifier) {
					...
					if(targetAsSeen == null) {
						raiseValidateError("cannot assign value to indexed identifier " + ((DataIdentifier) getOutput())
								.getName() + " without first initializing " + ((DataIdentifier) getOutput()).getName(),
							conditional);
					}
					//_output.setProperties(targetAsSeen);
					((IndexedIdentifier) getOutput())
						.setOriginalDimensions(targetAsSeen.getDim1(), targetAsSeen.getDim2());
					//((IndexedIdentifier) getOutput()).setOriginalDimensions(getOutput().getDim1(), getOutput().getDim2());
				}
				if(getOutput() instanceof IndexedIdentifier) {
					LOG.warn(
						this.printWarningLocation() + "Output for Rand Statement may have incorrect size information");
				}
			...
			case MATRIX:
				...
				validateParams(conditional, RESHAPE_VALID_PARAM_NAMES,
					"Legal parameters for matrix statement are (case-sensitive): " + RAND_DATA + ", " + RAND_ROWS + ", " + RAND_COLS + ", " + RAND_BY_ROW);
				...
				if(getVarParam(RAND_ROWS) != null && (getVarParam(RAND_ROWS) instanceof StringIdentifier || getVarParam(
					RAND_ROWS) instanceof BooleanIdentifier)) {
					raiseValidateError("for matrix statement " + RAND_ROWS + " has incorrect value type", conditional);
				}
				if(getVarParam(RAND_COLS) != null && (getVarParam(RAND_COLS) instanceof StringIdentifier || getVarParam(
					RAND_COLS) instanceof BooleanIdentifier)) {
				...
				if(!(getVarParam(RAND_BY_ROW) instanceof BooleanIdentifier)) {
					raiseValidateError("for matrix statement " + RAND_BY_ROW + " has incorrect value type",
						conditional);
				}
				...
				if(rowsExpr != null) {
					if(rowsExpr instanceof IntIdentifier) {
						if(((IntIdentifier) rowsExpr).getValue() >= 1) {
							...
							raiseValidateError(
								"In matrix statement, can only assign rows a long " + "(integer) value >= 1 -- attempted to assign value: " + ((IntIdentifier) rowsExpr)
									.getValue(), conditional);
						}
					}
					else if(rowsExpr instanceof DoubleIdentifier) {
						if(((DoubleIdentifier) rowsExpr).getValue() >= 1) {
							rowsLong = Double.valueOf((Math.floor(((DoubleIdentifier) rowsExpr).getValue())))
								.longValue();
						}
						else {
							raiseValidateError(
								"In matrix statement, can only assign rows a long " + "(integer) value >= 1 -- attempted to assign value: " + rowsExpr
									.toString(), conditional);
						}
					}
					else if(rowsExpr instanceof DataIdentifier && !(rowsExpr instanceof IndexedIdentifier)) {
						...
						if(currConstVars.containsKey(identifierName)) {
							...
							if(constValue instanceof IntIdentifier) {
								...
								if(((IntIdentifier) constValue).getValue() < 1) {
									raiseValidateError(
										"In matrix statement, can only assign rows a long " + "(integer) value >= 1 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
								// update row expr with new IntIdentifier
								long roundedValue = ((IntIdentifier) constValue).getValue();
				...
				if(colsExpr != null) {
					if(colsExpr instanceof IntIdentifier) {
						if(((IntIdentifier) colsExpr).getValue() >= 1) {
							...
							raiseValidateError(
								"In matrix statement, can only assign cols a long " + "(integer) value >= 1 -- attempted to assign value: " + colsExpr
									.toString(), conditional);
						}
					}
					else if(colsExpr instanceof DoubleIdentifier) {
						if(((DoubleIdentifier) colsExpr).getValue() >= 1) {
							colsLong = Double.valueOf((Math.floor(((DoubleIdentifier) colsExpr).getValue())))
								.longValue();
						}
						else {
							raiseValidateError(
								"In matrix statement, can only assign rows a long " + "(integer) value >= 1 -- attempted to assign value: " + colsExpr
									.toString(), conditional);
						}
					}
					else if(colsExpr instanceof DataIdentifier && !(colsExpr instanceof IndexedIdentifier)) {
						...
						if(currConstVars.containsKey(identifierName)) {
							...
							if(constValue instanceof IntIdentifier) {
								...
								if(((IntIdentifier) constValue).getValue() < 1) {
									raiseValidateError(
										"In matrix statement, can only assign cols a long " + "(integer) value >= 1 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
								// update col expr with new IntIdentifier
								long roundedValue = ((IntIdentifier) constValue).getValue();
				...
				if(getOutput() instanceof IndexedIdentifier) {
					((IndexedIdentifier) getOutput())
						.setOriginalDimensions(getOutput().getDim1(), getOutput().getDim2());
					LOG.warn(
						this.printWarningLocation() + "Output for matrix Statement may have incorrect size information");
				}
			...
			case FRAME:
				...
				validateParams(conditional, FRAME_VALID_PARAM_NAMES,
					"Legal parameters for frame statement are (case-sensitive): " + RAND_DATA + ", " + RAND_ROWS + ", " + RAND_COLS + ", " + SCHEMAPARAM);
				...
				if(rowsExpr != null) {
					if(rowsExpr instanceof IntIdentifier) {
						if(((IntIdentifier) rowsExpr).getValue() >= 1)
							...
							raiseValidateError(
								"In frame statement, can only assign rows a long " + "(integer) value >= 1 -- attempted to assign value: " + ((IntIdentifier) rowsExpr)
									.getValue(), conditional);
					}
					else if(rowsExpr instanceof DoubleIdentifier) {
						if(((DoubleIdentifier) rowsExpr).getValue() >= 1)
							...
							raiseValidateError(
								"In frame statement, can only assign rows a long " + "(integer) value >= 1 -- attempted to assign value: " + rowsExpr
									.toString(), conditional);
					}
					else if(rowsExpr instanceof DataIdentifier && !(rowsExpr instanceof IndexedIdentifier)) {
						// check if the DataIdentifier variable is a ConstIdentifier
						...
						if(currConstVars.containsKey(identifierName)) {
							...
							if(constValue instanceof IntIdentifier) {
								...
								if(((IntIdentifier) constValue).getValue() < 1) {
									raiseValidateError(
										"In frame statement, can only assign rows a long " + "(integer) value >= 1 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
								// update row expr with new IntIdentifier
								long roundedValue = ((IntIdentifier) constValue).getValue();
					...
					if(colsExpr instanceof IntIdentifier) {
						...
						if(currConstVars.containsKey(identifierName)) {
							...
							if(constValue instanceof IntIdentifier) {
								...
								if(((IntIdentifier) constValue).getValue() < 1) {
									raiseValidateError(
										"In frame statement, can only assign cols a long " + "(integer) value >= 1 -- attempted to assign value: " + constValue
											.toString(), conditional);
								}
								// update col expr with new IntIdentifier
								long roundedValue = ((IntIdentifier) constValue).getValue();
				...
				if(getOutput() instanceof IndexedIdentifier) {
					((IndexedIdentifier) getOutput())
						.setOriginalDimensions(getOutput().getDim1(), getOutput().getDim2());
					LOG.warn(
						this.printWarningLocation() + "Output for frame Statement may have incorrect size information");
				}
				break;
			...
			case TENSOR:
				...
				setTensorDefault();
				validateParams(conditional, RESHAPE_VALID_PARAM_NAMES,
					"Legal parameters for tensor statement are (case-sensitive): " + RAND_DATA + ", " + RAND_DIMS + ", " + RAND_BY_ROW);
			...
			/*if (getVarParam(RAND_DATA) != null && (getVarParam(RAND_DATA) instanceof BooleanIdentifier)){
				...
				if(!(getVarParam(RAND_BY_ROW) instanceof BooleanIdentifier)) {
					raiseValidateError("for tensor statement " + RAND_BY_ROW + " has incorrect value type",
						conditional);
				}
				...
				if(getOutput() instanceof IndexedIdentifier) {
					LOG.warn(
						this.printWarningLocation() + "Output for tensor Statement may have incorrect size information");
				}
				break;
			...
			case SQL:
				...
				validateParams(conditional, SQL_VALID_PARAM_NAMES,
					"Legal parameters for tensor statement are (case-sensitive): " + SQL_CONN + ", " + SQL_USER + ", " + SQL_PASS + ", " + SQL_QUERY);
				...
				if(getOutput() instanceof IndexedIdentifier) {
					LOG.warn(
						this.printWarningLocation() + "Output for sql statement may have incorrect size information");
				}
				break;

			case FEDERATED:
				validateParams(conditional, FEDERATED_VALID_PARAM_NAMES,
					"Legal parameters for federated statement are (case-sensitive): " + FED_TYPE + ", " + FED_ADDRESSES + ", " + FED_RANGES);
				exp = getVarParam(FED_ADDRESSES);
				if(!(exp instanceof DataIdentifier)) {
					raiseValidateError("for federated statement " + FED_ADDRESSES + " has incorrect value type",
						conditional);
				}
				...
				if(!(exp instanceof StringIdentifier)) {
					raiseValidateError("for federated statement " + FED_TYPE + " has incorrect value type",
						conditional);
				}
			...
			default:
				raiseValidateError("Unsupported Data expression " + this.getOpCode(), false,
					LanguageErrorCodes.INVALID_PARAMETERS); //always unconditional
		}
	}
	...
	private void validateParams(boolean conditional, Set<String> validParamNames, String legalMessage) {
		for(String key : _varParams.keySet()) {
			...
			if(!found) {
				raiseValidateError("unexpected parameter \"" + key + "\". " + legalMessage, conditional);
			}
		}
	}
	...
	private void performConstantPropagation(HashMap<String, ConstIdentifier> currConstVars, String[] paramNames) {
		for(String paramName : paramNames) {
			...
			if(paramExp != null && paramExp instanceof DataIdentifier && !(paramExp instanceof IndexedIdentifier) && currConstVars
				.containsKey(((DataIdentifier) paramExp).getName())) {
				addVarParam(paramName, currConstVars.get(((DataIdentifier) paramExp).getName()));
			}
		}
	}

	private String fileNameCat(BinaryExpression expr, HashMap<String, ConstIdentifier> currConstVars, String filename,
		boolean conditional) {
		// Processing the left node first
		if(expr.getLeft() instanceof BinaryExpression && ((BinaryExpression) expr.getLeft())
			.getOpCode() == BinaryOp.PLUS) {
		...
		else if(expr.getLeft() instanceof DataIdentifier && ((DataIdentifier) expr.getLeft())
			...
			raiseValidateError(
				"Parameter " + IO_FILENAME + " only supports a const string or const string concatenations.",
				conditional);
		}
		// Now process the right node
		if(expr.getRight() instanceof BinaryExpression && ((BinaryExpression) expr.getRight())
			.getOpCode() == BinaryOp.PLUS) {
		...
		else if(expr.getRight() instanceof DataIdentifier && ((DataIdentifier) expr.getRight())
			.getDataType() == DataType.SCALAR && ((DataIdentifier) expr.getRight())
	...
	@Override public VariableSet variablesRead() {
	...
	@Override public VariableSet variablesUpdated() {
		VariableSet result = new VariableSet();
		for(Expression expr : _varParams.values()) {
	...
	@SuppressWarnings("unchecked") private void parseMetaDataFileParameters(String mtdFileName, JSONObject configObject,
		boolean conditional) {
		for(Object obj : configObject.entrySet()) {
			...
			if(getVarParam(key.toString()) != null && (getVarParam(
				key.toString()) instanceof ConstIdentifier) && !getVarParam(key.toString()).toString()
				.equalsIgnoreCase(val.toString())) {
				raiseValidateError("Parameter '" + key
					.toString() + "' has conflicting values in metadata and read statement. MTD file value: '" + val
					.toString() + "'. Read statement value: '" + getVarParam(key.toString()) + "'.", conditional);
			}
			else {
				...
				if(getVarParam(key.toString()) == null) {
					if((!key.toString().equalsIgnoreCase(DESCRIPTIONPARAM)) && (!key.toString()
						.equalsIgnoreCase(AUTHORPARAM)) && (!key.toString().equalsIgnoreCase(CREATEDPARAM))) {
						StringIdentifier strId = new StringIdentifier(val.toString(), this);

						if(key.toString().equalsIgnoreCase(DELIM_HAS_HEADER_ROW) || key.toString()
							.equalsIgnoreCase(DELIM_FILL) || key.toString().equalsIgnoreCase(DELIM_SPARSE)) {
							...
							if(strId.toString().equalsIgnoreCase("true")) {
								...
								raiseValidateError(
									"Invalid value provided for '" + DELIM_HAS_HEADER_ROW + "' in metadata file '" + mtdFileName + "'. " + "Must be either TRUE or FALSE.",
									conditional);
							}
						...
						else if(key.toString().equalsIgnoreCase(DELIM_NA_STRINGS) || key.toString()
							.equalsIgnoreCase(PRIVACY) || key.toString().equalsIgnoreCase(FINE_GRAINED_PRIVACY)) {
							...
							if(val instanceof String) {
								...
								throw new ParseException(
									"Type of value " + val + " from metadata not recognized by parser.");
							}
	...
	public JSONObject readMetadataFile(String filename, boolean conditional) {
		...
		if(exists && isDir) {
			...
			for(FileStatus stat : HDFSTool.getDirectoryListing(filename)) {
				...
				try(BufferedReader br = new BufferedReader(
					new InputStreamReader(IOUtilFunctions.getFileSystem(childPath).open(childPath)))) {
					...
					raiseValidateError(
						"for MTD file in directory, error parting part of MTD file with path " + childPath
							.toString() + ": " + e.getMessage(), conditional);
				}
			}
		}
		// CASE: filename points to a file
			...
			try(BufferedReader br = new BufferedReader(
				new InputStreamReader(IOUtilFunctions.getFileSystem(path).open(path)))) {
	...
	public boolean isCSVReadWithUnknownSize() {
		...
		if(_opcode == DataOp.READ && format != null && format.toString().equalsIgnoreCase(FileFormat.CSV.toString())) {
			...
			return (rows == null || Long.parseLong(rows.toString()) < 0) || (cols == null || Long
				.parseLong(cols.toString()) < 0);
		}
		return false;
	}
	...
	public boolean isLIBSVMReadWithUnknownSize() {
		Expression format = getVarParam(FORMAT_TYPE);
		if(_opcode == DataOp.READ && format != null && format.toString()
			.equalsIgnoreCase(FileFormat.LIBSVM.toString())) {
			Expression rows = getVarParam(READROWPARAM);
			Expression cols = getVarParam(READCOLPARAM);
			return (rows == null || Long.parseLong(rows.toString()) < 0) || (cols == null || Long
