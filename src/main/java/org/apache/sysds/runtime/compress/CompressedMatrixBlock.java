...
public class CompressedMatrixBlock extends AbstractCompressedMatrixBlock {
		...
		for(int b = 0; b <= db.numBlocks(); b++) {
			...
			if(k == 1) {
				...
				try {
					...
					ArrayList<ColGroup>[] parts = createStaticTaskPartitioningForMatrixMult(colGroups, k, false);
					for(ArrayList<ColGroup> part : parts) {
						...
						// }
						// else {
						tasks.add(new LeftMatrixMatrixMultTask(part, thatV, retV, that.getNumRows(), numColumns, blockL,
							blockU));
	...
	@SuppressWarnings("unchecked")
	private static ArrayList<ColGroup>[] createStaticTaskPartitioningForMatrixMult(List<ColGroup> colGroups, int k,
		boolean inclUncompressed) {
		int numTasks = Math.min(k, colGroups.size());
		ArrayList<ColGroup>[] grpParts = new ArrayList[numTasks];
		int pos = 0;
	...
	private static class LeftMatrixMatrixMultTask implements Callable<Object> {
		private final ArrayList<ColGroup> _group;
		private final double[] _that;
		private final double[] _ret;
		// private final int _numVals;
		// private final double[] _values;
		// private final byte[] _valuesB;
		private final int _numRows;
		private final int _numCols;
		private final int _rl;
		private final int _ru;

		protected LeftMatrixMatrixMultTask(ArrayList<ColGroup> group, double[] that, double[] ret, int numRows,
			int numCols, int rl, int ru) {
			_group = group;
			_that = that;
			_ret = ret;
			// _numVals = numVals;
		...
		// protected LeftMatrixMatrixMultTask(ColGroup group, MatrixBlock that, MatrixBlock ret, int numVals,
		// byte[] valuesB) {
		// _group = group;
		// _that = that;
		// _ret = ret;
		...
		public Object call() {
			...
			try {
				...
				// for(int j = 0; j < colGroups.size(); j++)
				// colGroups.get(j).leftMultByRowVector(rowVector.getDenseBlockValues(),
				// tmp.getDenseBlockValues(),
				// v.getRight().get(j),
				// materialized[j]);

				// if(_values == null) {
				// _group.leftMultByMatrix(_that, _ret.getDenseBlockValues(), _numVals, _valuesB);
				// }
				// else {
