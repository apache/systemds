...
public abstract class ColGroupDDC extends ColGroupValue {
	...
	// protected final void postScaling(byte[] values, double[] vals, double[] c, int numVals, int i, int totalCols) {
	// 	final int ncol = getNumCols();
	...
	// 			}
	// 		}
	// 		c[colIx] = c[colIx] * ((QDictionary) _dict)._scale;
	// 	}
	// }
	...
	@Override
	public void leftMultByMatrix(double[] a, double[] c, int numVals, double[] values, int numRows, int numCols, int rl, int ru) {
	...
	// public void leftMultByMatrix(double[] a, double[] c, int numVals, byte[] values, int numRows, int numCols) {
	...
	// 			// iterative over codes and pre-aggregate inputs per code (guaranteed <=255)
	// 			// temporary array also avoids false sharing in multi-threaded environments
	// 			double[] vals = preAggregate(a, numVals, i);
	// 			postScaling(values, vals, c, numVals, i, numCols);
	// 		// }
	// 		// else {
	// 		// 	// iterate over codes, compute all, and add to the result
