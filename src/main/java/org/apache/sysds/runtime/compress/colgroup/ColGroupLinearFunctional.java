...
public class ColGroupLinearFunctional extends AColGroupCompressed {
	 ...
	 * @param coefficents  Array where the first `colIndices.length` entries are the intercepts and the
	 *                     next `colIndices.length` entries are the slopes
		...
		if(op.fn instanceof Plus || op.fn instanceof Minus) {
			...
			if(left) {
				for(int col = 0; col < getNumCols(); col++) {
					...
					coefficients_new[col] = op.fn.execute(v[_colIndexes[col]], _coefficents[col]);
					// update slope
					coefficients_new[col + getNumCols()] = op.fn.execute(v[_colIndexes[col]], _coefficents[col + getNumCols()]);
				}
			} else {
				for(int col = 0; col < getNumCols(); col++) {
					// update intercept
					coefficients_new[col] = op.fn.execute(_coefficents[col], v[_colIndexes[col]]);
					// update slope
					coefficients_new[col + getNumCols()] = op.fn.execute(_coefficents[col + getNumCols()], v[_colIndexes[col]]);
				}
			}
