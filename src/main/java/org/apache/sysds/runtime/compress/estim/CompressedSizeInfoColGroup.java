...
public class CompressedSizeInfoColGroup {
	...
	private Map<CompressionType, Long> calculateCompressionSizes(CompressedSizeEstimationFactors fact,
		List<CompressionType> validCompressionTypes) {
	...
	private Long getCompressionSize(CompressionType ct, CompressedSizeEstimationFactors fact) {
		long size = 0;
		switch(ct) {
			case DDC:
				if(fact.numVals < 256) {
