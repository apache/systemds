			...
			for(int wrapIter = 0; wrapIter < numLines; wrapIter++) {
				...
				if(wrapIter == 0) {
					...
					sb.append(String.format(
						...
						(counter + 1), instStr, timeSString, count));
				}
				else {
					sb.append(String.format(
	...
	private static FedStatsCollection collectFedStats() {
		...
		for(Future<FederatedResponse> res : responses) {
			try {
				...
				System.out.println("Exception of type " + e.getClass().toString() + " thrown while "
					+ "getting the federated stats of the federated response: " + e.toString());
			}
		}
		return aggFedStats;
	}
	...
	private static Future<FederatedResponse>[] getFederatedResponses() {
		...
		for(Pair<String, Integer> fedAddr : _fedWorkerAdresses) {
			...
			try {
				...
				System.out.println("SSLException while getting the federated stats from " + isa.toString()
					+ ": " + ssle.toString());
			} catch(DMLRuntimeException dre) {
				// silently ignore this exception --> caused by offline federated workers
			} catch (Exception e) {
				System.out.println("Exeption of type " + e.getClass().getName() + " thrown while getting "
					+ "stats from federated worker: " + e.toString());
			}
		}
		@SuppressWarnings("unchecked")
		...
		private void aggregate(FedStatsCollection that) {
			...
			that.heavyHitters.forEach(
				(key, value) -> heavyHitters.merge(key, value, (v1, v2) ->
					new ImmutablePair<Long, Double>(v1.getLeft() + v2.getLeft(), v1.getRight() + v2.getRight()))
			);
		}
