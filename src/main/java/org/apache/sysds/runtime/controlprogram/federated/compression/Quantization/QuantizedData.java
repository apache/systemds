...
public class QuantizedData implements Serializable {
	...
	public QuantizedData(byte[] quantizedValues, double min, double max,
					     int levels, int bitsPerValue, int numRows, int numCols) {
	...
	public double reconstructValue(byte levelIndex) {
		if (max - min < 1e-10) return min;  // Constant matrix
	...
	public String toString() {
		return String.format("QuantizedData[%dx%d, levels=%d, bits=%d, min=%.4f, max=%.4f]",
			numRows, numCols, levels, bitsPerValue, min, max);
	}
}
