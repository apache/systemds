...
public class InstructionUtils {
	...
	public static int checkNumFields(String str, int expected) {
		...
		if(numFields != expected)
			throw new DMLRuntimeException("checkNumFields() for (" + str + ") -- expected number (" + expected +
	...
	public static int checkNumFields(String[] parts, int expected) {
		...
		if(numFields != expected)
			throw new DMLRuntimeException(
				"checkNumFields() -- expected number (" + expected + ") != is not equal to actual number (" +
					numFields + ").");
	...
	public static int checkNumFields(String[] parts, int expected1, int expected2) {
		...
		if(numFields != expected1 && numFields != expected2)
			throw new DMLRuntimeException("checkNumFields() -- expected number (" + expected1 + " or " + expected2 +
				") != is not equal to actual number (" + numFields + ").");
	...
	public static int checkNumFields(String str, int expected1, int expected2) {
		...
		if(numFields != expected1 && numFields != expected2)
			throw new DMLRuntimeException(
				"checkNumFields() for (" + str + ") -- expected number (" + expected1 + " or " + expected2 +
					") != is not equal to actual number (" + numFields + ").");
		return numFields;
	}

	/**
	 * Given an instruction string, strip-off the execution type and return opcode and all input/output operands WITHOUT
	 * their data/value type. i.e., ret.length = parts.length-1 (-1 for execution type)
	 *
	...
	/**
	 * Given an instruction string, this function strips-off the execution type (CP or SPARK) and returns the remaining
	 * parts, which include the opcode as well as the input and output operands. Each returned part will have the
	 * datatype and valuetype associated with the operand.
	 *
	...
	public static AggregateUnaryOperator parseBasicAggregateUnaryOperator(String opcode, int numThreads) {
		...
		if(opcode.equalsIgnoreCase(Opcodes.UAKP.toString())) {
			...
			AggregateOperator agg = new AggregateOperator(0, KahanPlusSq.getKahanPlusSqFnObject(),
				CorrectionLocationType.LASTCOLUMN);
			aggun = new AggregateUnaryOperator(agg, ReduceAll.getReduceAllFnObject(), numThreads);
		}
			...
			// Mean
			AggregateOperator agg = new AggregateOperator(0, Mean.getMeanFnObject(),
			...
			AggregateOperator agg = new AggregateOperator(Double.NEGATIVE_INFINITY,
			...
			AggregateOperator agg = new AggregateOperator(Double.POSITIVE_INFINITY,
			...
			aggun = new CountDistinctOperator(AggregateUnaryCPInstruction.AUType.COUNT_DISTINCT, Direction.RowCol,
				ReduceAll.getReduceAllFnObject());
		}
		else if(opcode.equalsIgnoreCase(Opcodes.UACDR.toString())) {
			aggun = new CountDistinctOperator(AggregateUnaryCPInstruction.AUType.COUNT_DISTINCT, Direction.Row,
				ReduceCol.getReduceColFnObject());
		}
		else if(opcode.equalsIgnoreCase(Opcodes.UACDC.toString())) {
			aggun = new CountDistinctOperator(AggregateUnaryCPInstruction.AUType.COUNT_DISTINCT, Direction.Col,
			...
			aggun = new CountDistinctOperator(AggregateUnaryCPInstruction.AUType.COUNT_DISTINCT_APPROX, Direction.Row,
				ReduceCol.getReduceColFnObject());
		}
		else if(opcode.equalsIgnoreCase(Opcodes.UACDAPC.toString())) {
			aggun = new CountDistinctOperator(AggregateUnaryCPInstruction.AUType.COUNT_DISTINCT_APPROX, Direction.Col,
				ReduceRow.getReduceRowFnObject());
		}
	...
	public static AggregateUnaryOperator parseAggregateUnaryRowIndexOperator(String opcode, int numOutputs,
		int numThreads) {
		AggregateUnaryOperator aggun = null;
	...
	public static AggregateTernaryOperator parseAggregateTernaryOperator(String opcode, int numThreads) {
		CorrectionLocationType corr = opcode.equalsIgnoreCase(
			Opcodes.TAKPM.toString()) ? CorrectionLocationType.LASTCOLUMN : CorrectionLocationType.LASTROW;
		AggregateOperator agg = new AggregateOperator(0, KahanPlus.getKahanPlusFnObject(), corr);
		IndexFunction ixfun = opcode.equalsIgnoreCase(
			Opcodes.TAKPM.toString()) ? ReduceAll.getReduceAllFnObject() : ReduceRow.getReduceRowFnObject();
	...
	public static AggregateOperator parseAggregateOperator(String opcode, String corrLoc) {
		...
		if(opcode.equalsIgnoreCase("ak+") || opcode.equalsIgnoreCase("aktrace")) {
			CorrectionLocationType lcorrLoc = (corrLoc ==
				null) ? CorrectionLocationType.LASTCOLUMN : CorrectionLocationType.valueOf(corrLoc);
			...
			CorrectionLocationType lcorrLoc = (corrLoc ==
				null) ? CorrectionLocationType.LASTTWOCOLUMNS : CorrectionLocationType.valueOf(corrLoc);
			agg = new AggregateOperator(0, KahanPlus.getKahanPlusFnObject(), lcorrLoc);
		}
		else if(opcode.equalsIgnoreCase("avar")) {
			CorrectionLocationType lcorrLoc = (corrLoc ==
				null) ? CorrectionLocationType.LASTFOURCOLUMNS : CorrectionLocationType.valueOf(corrLoc);
			CM varFn = CM.getCMFnObject(AggregateOperationTypes.VARIANCE);
	...
	public static UnaryOperator parseUnaryOperator(String opcode) {
		return opcode.equals(Opcodes.NOT.toString()) ? new UnaryOperator(Not.getNotFnObject()) : new UnaryOperator(
			Builtin.getBuiltinFnObject(opcode));
	}

	public static UnaryOperator parseUnaryOperator(String opcode, int k) {
		return opcode.equals(Opcodes.NOT.toString()) ? new UnaryOperator(Not.getNotFnObject(), k) : new UnaryOperator(
			Builtin.getBuiltinFnObject(opcode), k);
	}
	...
	public static MultiThreadedOperator parseBinaryOrBuiltinOperator(String opcode, CPOperand in1, CPOperand in2) {
		...
		return Builtin.isBuiltinFnObject(opcode) ? (matrixScalar ? new RightScalarOperator(
			Builtin.getBuiltinFnObject(opcode), 0) : new BinaryOperator(
			Builtin.getBuiltinFnObject(opcode))) : (matrixScalar ? parseScalarBinaryOperator(opcode,
			in1.getDataType().isScalar()) : parseBinaryOperator(opcode));
	}

	public static Operator parseExtendedBinaryOrBuiltinOperator(String opcode, CPOperand in1, CPOperand in2) {
		boolean matrixScalar = (in1.getDataType() != in2.getDataType() &&
			(in1.getDataType() != Types.DataType.FRAME && in2.getDataType() != Types.DataType.FRAME));
		return Builtin.isBuiltinFnObject(opcode) ? (matrixScalar ? new RightScalarOperator(
			Builtin.getBuiltinFnObject(opcode), 0) : new BinaryOperator(
			Builtin.getBuiltinFnObject(opcode))) : (matrixScalar ? parseScalarBinaryOperator(opcode,
			in1.getDataType().isScalar()) : parseExtendedBinaryOperator(opcode));
	...
	public static TernaryOperator parseTernaryOperator(String opcode, int numThreads) {
		return new TernaryOperator(opcode.equals(Opcodes.PM.toString()) ? PlusMultiply.getFnObject() : opcode.equals(
			Opcodes.MINUSMULT.toString()) ? MinusMultiply.getFnObject() : IfElse.getFnObject(), numThreads);
	}
	...
	public static ScalarOperator parseScalarBinaryOperator(String opcode, boolean arg1IsScalar, double constant) {
		...
		//non-commutative operators
			...
			return arg1IsScalar ? new LeftScalarOperator(And.getAndFnObject(), constant) : new RightScalarOperator(
				And.getAndFnObject(), constant);
		}
		else if(opcode.equalsIgnoreCase(Opcodes.OR.toString())) {
			return arg1IsScalar ? new LeftScalarOperator(Or.getOrFnObject(), constant) : new RightScalarOperator(
				Or.getOrFnObject(), constant);
		}
		else if(opcode.equalsIgnoreCase(Opcodes.XOR.toString())) {
			return arg1IsScalar ? new LeftScalarOperator(Xor.getXorFnObject(), constant) : new RightScalarOperator(
				Xor.getXorFnObject(), constant);
		}
		else if(opcode.equalsIgnoreCase(Opcodes.BITWAND.toString())) {
			return arg1IsScalar ? new LeftScalarOperator(BitwAnd.getBitwAndFnObject(),
				constant) : new RightScalarOperator(BitwAnd.getBitwAndFnObject(), constant);
		}
		else if(opcode.equalsIgnoreCase(Opcodes.BITWOR.toString())) {
			return arg1IsScalar ? new LeftScalarOperator(BitwOr.getBitwOrFnObject(),
				constant) : new RightScalarOperator(BitwOr.getBitwOrFnObject(), constant);
		}
		else if(opcode.equalsIgnoreCase(Opcodes.BITWXOR.toString())) {
			return arg1IsScalar ? new LeftScalarOperator(BitwXor.getBitwXorFnObject(),
				constant) : new RightScalarOperator(BitwXor.getBitwXorFnObject(), constant);
		}
		else if(opcode.equalsIgnoreCase(Opcodes.BITWSHIFTL.toString())) {
			return arg1IsScalar ? new LeftScalarOperator(BitwShiftL.getBitwShiftLFnObject(),
				constant) : new RightScalarOperator(BitwShiftL.getBitwShiftLFnObject(), constant);
		}
		else if(opcode.equalsIgnoreCase(Opcodes.BITWSHIFTR.toString())) {
			return arg1IsScalar ? new LeftScalarOperator(BitwShiftR.getBitwShiftRFnObject(),
				constant) : new RightScalarOperator(BitwShiftR.getBitwShiftRFnObject(), constant);
		}
	...
	public static BinaryOperator parseExtendedBinaryOperator(String opcode) {
		if(opcode.equalsIgnoreCase(Opcodes.EQUAL.toString()) || opcode.equalsIgnoreCase(Opcodes.MAPEQ.toString()))
			...
			opcode.equalsIgnoreCase(Opcodes.MAPNEQ.toString()))
			return new BinaryOperator(NotEquals.getNotEqualsFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.LESS.toString()) || opcode.equalsIgnoreCase(Opcodes.MAPLT.toString()))
			return new BinaryOperator(LessThan.getLessThanFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.GREATER.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPGT.toString()))
			return new BinaryOperator(GreaterThan.getGreaterThanFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.LESSEQUAL.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPLE.toString()))
			return new BinaryOperator(LessThanEquals.getLessThanEqualsFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.GREATEREQUAL.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPGE.toString()))
			return new BinaryOperator(GreaterThanEquals.getGreaterThanEqualsFnObject());
			...
			opcode.equalsIgnoreCase(Opcodes.MAPBITWAND.toString()))
			return new BinaryOperator(BitwAnd.getBitwAndFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.BITWOR.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPBITWOR.toString()))
			return new BinaryOperator(BitwOr.getBitwOrFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.BITWXOR.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPBITWXOR.toString()))
			return new BinaryOperator(BitwXor.getBitwXorFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.BITWSHIFTL.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPBITWSHIFTL.toString()))
			return new BinaryOperator(BitwShiftL.getBitwShiftLFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.BITWSHIFTR.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPBITWSHIFTR.toString()))
			return new BinaryOperator(BitwShiftR.getBitwShiftRFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.PLUS.toString()) || opcode.equalsIgnoreCase(Opcodes.MAPPLUS.toString()))
			return new BinaryOperator(Plus.getPlusFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.MINUS.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPMINUS.toString()))
			return new BinaryOperator(Minus.getMinusFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.MULT.toString()) || opcode.equalsIgnoreCase(Opcodes.MAPMULT.toString()))
			return new BinaryOperator(Multiply.getMultiplyFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.MINUS1_MULT.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPMINUS1_MULT.toString()))
			return new BinaryOperator(Minus1Multiply.getMinus1MultiplyFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.MULT2.toString()))
			return new BinaryOperator(Multiply2.getMultiply2FnObject());
			...
			opcode.equalsIgnoreCase(Opcodes.MAPMOD.toString()))
			return new BinaryOperator(Modulus.getFnObject());
		else if(opcode.equalsIgnoreCase(Opcodes.INTDIV.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.MAPINTDIV.toString()))
			return new BinaryOperator(IntegerDivide.getFnObject());
			...
			opcode.equalsIgnoreCase(Opcodes.MAPDROPINVALIDLENGTH.toString()))
			return new BinaryOperator(Builtin.getBuiltinFnObject("dropInvalidLength"));
	...
	public static AggOp getAggOp(String opcode) {
		switch(opcode) {
			...
			case "uacmin":
				return AggOp.MIN;
			case "uarimax":
				return AggOp.MAXINDEX;
			case "uarimin":
				return AggOp.MININDEX;
		}
		return null;
	}
	...
	public static CorrectionLocationType deriveAggregateOperatorCorrectionLocation(String opcode) {
		if(opcode.equalsIgnoreCase(Opcodes.UAKP.toString()) || opcode.equalsIgnoreCase(Opcodes.UARKP.toString()) ||
			...
			opcode.equalsIgnoreCase(Opcodes.UACSQKP.toString()))
			return CorrectionLocationType.LASTROW;
		else if(opcode.equalsIgnoreCase(Opcodes.UAMEAN.toString()) ||
			opcode.equalsIgnoreCase(Opcodes.UARMEAN.toString()))
			...
			opcode.equalsIgnoreCase(Opcodes.UARIMIN.toString()))
			return CorrectionLocationType.LASTCOLUMN;
	...
	public static String createOperand(CPOperand operand) {
		return InstructionUtils.concatOperandParts(operand.getName(), operand.getDataType().name(),
			operand.getValueType().name());
	}
	...
	/**
	 * Concat the inputs as operands to generate the instruction string. The inputs are separated by the operand
	 * delimiter and appended using a ThreadLocal StringBuilder.
	 *
	...
	public static String constructTernaryString(String instString, CPOperand op1, CPOperand op2, CPOperand op3,
		CPOperand out) {
		return concatOperands(constructBinaryInstString(instString, "ifelse", op1, op2, op3), createOperand(out));
	}

	public static String constructBinaryInstString(String instString, String opcode, CPOperand op1, CPOperand op2,
		CPOperand out) {
		String[] parts = instString.split(Lop.OPERAND_DELIMITOR);
		return InstructionUtils.concatOperands(parts[0], opcode, createOperand(op1), createOperand(op2),
			createOperand(out));
	}
	...
	/**
	 * Prepare instruction string for sending in a FederatedRequest as a CP instruction. This involves replacing the
	 * coordinator operand names with the worker operand names, changing the execution type, and removing the federated
	 * output flag if necessary.
	 *
	 ...
	 */
	public static String instructionStringFEDPrepare(String inst, CPOperand varOldOut, long id, CPOperand[] varOldIn,
		long[] varNewIn, boolean rmFederatedOutput) {
	...
	private static String replaceOperand(String linst, CPOperand oldOperand, String newOperandName) {
		return linst.replace(Lop.OPERAND_DELIMITOR + oldOperand.getName() + Lop.DATATYPE_PREFIX,
