...
public class DPBuiltinOps {
	 ...
	 * actually invoked: L1 for dp_laplace, L2 for dp_gaussian. The two only coincide when the release is scalar.
	 *
	...
	private static MatrixBlock generateNoise(String opcode, int rows, int cols, double sensitivity, double epsilon,
		...
		if(opcode.equalsIgnoreCase(Opcodes.DP_LAPLACE.toString())) {
			// Laplace mechanism
			// For a given epsilon, noise is drawn from the Laplace distribution at scale b = sensitivity / epsilon
			noise = fillLaplaceNoise(rows, cols, sensitivity / epsilon);
		}
		else {
			// Gaussian mechanism
			// For a given epsilon and delta, noise is drawn from the Gaussian distribution N(0, sigma^2)
	...
	private static MatrixBlock fillLaplaceNoise(int rows, int cols, double scale) {
		MatrixBlock noise = new MatrixBlock(rows, cols, false);
