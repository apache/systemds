...
public class ParameterizedBuiltinCPInstruction extends ComputationCPInstruction {
  ...
  public static ParameterizedBuiltinCPInstruction parseInstruction(String str) {
    ...
    if(opcode.equalsIgnoreCase("cdf")) {
      ...
      .equalsIgnoreCase("rexpand") || opcode.equalsIgnoreCase("lowertri") || opcode.equalsIgnoreCase("uppertri")) {
  ...
  @Override public void processInstruction(ExecutionContext ec) {
    ...
    if(opcode.equalsIgnoreCase("cdf")) {
      ...
      if(cacheData instanceof MatrixObject) {
        ...
        out = DataConverter
          .toString((ListObject) cacheData, rows, cols, sparse, separator, lineSeparator, rows, cols, decimal);
      }
      else {
      ...
      List<Data> data = params.values().stream()
        .map(d -> ec.containsVariable(d) ? ec.getVariable(d) : new StringObject(d)).collect(Collectors.toList());
  ...
  @Override public Pair<String, LineageItem> getLineageItem(ExecutionContext ec) {
    ...
    if(opcode.equalsIgnoreCase("groupedagg")) {
      ...
      return Pair
        .of(output.getName(), new LineageItem(getOpcode(), LineageItemUtils.getLineage(ec, target, margin, select)));
    }
    else if(opcode.equalsIgnoreCase("replace")) {
      CPOperand target = getTargetOperand();
      ...
      return Pair
        .of(output.getName(), new LineageItem(getOpcode(), LineageItemUtils.getLineage(ec, target, pattern, replace)));
    }
    else if(opcode.equalsIgnoreCase("rexpand")) {
      CPOperand target = getTargetOperand();
      ...
      return Pair
        .of(output.getName(), new LineageItem(getOpcode(), LineageItemUtils.getLineage(ec, target, meta, spec)));
    }
    else {
