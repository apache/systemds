...
public class VariableCPInstruction extends CPInstruction implements LineageTraceable {
  ...
  // This version of the constructor is used only in case of CreateVariable
  private VariableCPInstruction(VariableOperationCode op, CPOperand in1, CPOperand in2, CPOperand in3, MetaData md,
    UpdateType updateType, FileFormatProperties formatProperties, String schema, String sopcode, String istr) {
    ...
    switch(voc) {
      ...
      case CreateVariable:
        ...
        if(fmt.equalsIgnoreCase(
          "csv")) {                    // Cretevar instructions for CSV format either has 13 or 14 inputs.
      ...
      case RemoveVariable:
        VariableCPInstruction rminst = new VariableCPInstruction(getVariableOperationCode(opcode), null, null, null,
          out, opcode, str);
      ...
      case Write:
        ...
        VariableCPInstruction inst = new VariableCPInstruction(getVariableOperationCode(opcode), in1, in2, in3, out,
          null, fprops, null, null, opcode, str);
        inst.addInput(in4);
   ...
   */
  @SuppressWarnings("rawtypes") private void processMoveInstruction(ExecutionContext ec) {
    ...
    if(getInput3() == null) {
      ...
      if(srcData == null) {
        throw new DMLRuntimeException(
          "Unexpected error: could not find a data object " + "for variable name:" + getInput1()
            .getName() + ", while processing instruction ");
      }
      ...
      if(ec.getVariable(getInput1().getName()) == null)
        throw new DMLRuntimeException("Unexpected error: could not find a data object for variable name:" + getInput1()
          .getName() + ", while processing instruction " + this.toString());
      ...
      if(getInput3().getName().equalsIgnoreCase("binaryblock")) {
        ...
        if(!success) {
          throw new DMLRuntimeException(
            "Failed to move var " + getInput1().getName() + " to file " + getInput2().getName() + ".");
        }
      }
      else if(object instanceof MatrixObject)
        throw new DMLRuntimeException("Unexpected formats while copying: from matrix blocks [" + ((MatrixObject) object)
          .getBlocksize() + "] to " + getInput3().getName());
      else if(object instanceof FrameObject)
        throw new DMLRuntimeException("Unexpected formats while copying: from fram object [" + ((FrameObject) object)
          .getNumColumns() + "," + ((FrameObject) object).getNumColumns() + "] to " + getInput3().getName());
    }
  }
  ...
  private void processRemoveVariableAndFileInstruction(ExecutionContext ec) {
    ...
    boolean del = ((BooleanObject) ec.getScalarInput(getInput2().getName(), getInput2().getValueType(), true))
      .getBooleanValue();
    ...
    switch(getInput1().getDataType()) {
      case MATRIX: {
        ...
        if(mBlock.getNumRows() != 1 || mBlock.getNumColumns() != 1)
          throw new DMLRuntimeException(
            "Dimension mismatch - unable to cast matrix '" + getInput1().getName() + "' of dimension (" + mBlock
              .getNumRows() + " x " + mBlock.getNumColumns() + ") to scalar.");
      ...
      case FRAME: {
        ...
        if(fBlock.getNumRows() != 1 || fBlock.getNumColumns() != 1)
          throw new DMLRuntimeException(
            "Dimension mismatch - unable to cast frame '" + getInput1().getName() + "' of dimension (" + fBlock
              .getNumRows() + " x " + fBlock.getNumColumns() + ") to scalar.");
      ...
      case TENSOR: {
        ...
        if(tBlock.getNumDims() != 2 || tBlock.getNumRows() != 1 || tBlock.getNumColumns() != 1)
          throw new DMLRuntimeException(
            "Dimension mismatch - unable to cast tensor '" + getInput1().getName() + "' to scalar.");
  ...
  private void processCastAsMatrixVariableInstruction(ExecutionContext ec) {
    switch(getInput1().getDataType()) {
      ...
      case SCALAR: {
        ScalarObject scalarInput = ec
          .getScalarInput(getInput1().getName(), getInput1().getValueType(), getInput1().isLiteral());
  ...
  private void processReadInstruction(ExecutionContext ec) {
    ...
    try {
      switch(getInput1().getValueType()) {
        ...
        default:
          throw new DMLRuntimeException(
            "Invalid value type (" + getInput1().getValueType() + ") while processing readScalar instruction.");
      }
    }
    catch(IOException e) {
  ...
  private void processWriteInstruction(ExecutionContext ec) {
    ...
    if(fmt != FileFormat.LIBSVM) {
      String desc = ec.getScalarInput(getInput4().getName(), ValueType.STRING, getInput4().isLiteral())
        .getStringValue();
  ...
  private void processSetFileNameInstruction(ExecutionContext ec) {
    ...
    if(data.getDataType() == DataType.MATRIX) {
      ...
      throw new DMLRuntimeException(
        "Invalid data type (" + getInput1().getDataType() + ") in SetFileName instruction: " + instString);
  }
  ...
  private void writeCSVFile(ExecutionContext ec, String fname) {
    ...
    if(mo.isDirty()) {
      ...
      try {
        ...
        if(fmt == FileFormat.CSV && !getInput1().getName().startsWith(org.apache.sysds.lops.Data.PREAD_PREFIX)) {
    ...
    if(mo.isDirty()) {
      ...
      try {
        ...
        if(fmt == FileFormat.TEXT && !getInput1().getName().startsWith(org.apache.sysds.lops.Data.PREAD_PREFIX)) {
          WriterMatrixMarket
            .mergeTextcellToMatrixMarket(mo.getFileName(), fname, dc.getRows(), dc.getCols(), dc.getNonZeros());
        }
        else {
          mo.exportData(fname, outFmt);
        }
  ...
  private static String getBasicCreatevarString(String varName, String fileName, boolean fNameOverride, DataType dt,
    String format) {
    ...
    sb.append(Lop.OPERAND_DELIMITOR);
    sb.append(
      fileName);        // Constant CREATEVAR_FILE_NAME_VAR_POS is used to find a position of filename within a string generated through this function.
  ...
  public static Instruction prepCreatevarInstruction(String varName, String fileName, boolean fNameOverride,
    String format) {
    return parseInstruction(getBasicCreatevarString(varName, fileName, fNameOverride, DataType.MATRIX, format));
  }

  public static Instruction prepCreatevarInstruction(String varName, String fileName, boolean fNameOverride,
    DataType dt, String format, DataCharacteristics mc, UpdateType update) {
    StringBuilder sb = new StringBuilder();
  ...
  public static Instruction prepCreatevarInstruction(String varName, String fileName, boolean fNameOverride,
    DataType dt, String format, DataCharacteristics mc, UpdateType update, boolean hasHeader, String delim,
    boolean sparse) {
    StringBuilder sb = new StringBuilder();
    sb.append(getBasicCreatevarString(varName, fileName, fNameOverride, dt, format));
  ...
  @Override public void updateInstructionThreadID(String pattern, String replace) {
    if(opcode == VariableOperationCode.CreateVariable || opcode == VariableOperationCode.SetFileName) {
      ...
      sb.append(
        ProgramConverter.saveReplaceFilenameThreadID(instString.substring(iPos + 1, iPos2 + 1), pattern, replace));
  ...
  @Override public Pair<String, LineageItem> getLineageItem(ExecutionContext ec) {
    ...
    switch(getVariableOpcode()) {
      ...
      case Read: {
        ...
        li = new LineageItem(toString().replace(getInput1().getName(), org.apache.sysds.lops.Data.PREAD_PREFIX + "xxx"),
          getOpcode());
        break;
      }
      case AssignVariable: {
      ...
      case Write: {
        ...
        if(_formatProperties != null && _formatProperties.getDescription() != null && !_formatProperties
