...
public class AggregateBinaryFEDInstruction extends BinaryFEDInstruction {
	...
	public AggregateBinaryFEDInstruction(Operator op, CPOperand in1, CPOperand in2, CPOperand out,
		...
		super(FEDType.AggregateBinary, op, in1, in2, out, opcode, istr, federatedOutput);
	}
	...
	public static AggregateBinaryFEDInstruction parseInstruction(String str) {
		...
		boolean federatedOutput = parts.length == 6 && Boolean.parseBoolean(parts[5]);
		return new AggregateBinaryFEDInstruction(
			InstructionUtils.getMatMultOperator(k), in1, in2, out, opcode, str, federatedOutput);
	}
	...
	public void processInstruction(ExecutionContext ec) {
		...
		if(mo1.isFederated(FType.COL) && mo2.isFederated(FType.ROW)
			...
			FederatedRequest fr1 = FederationUtils.callInstruction(instString, output, new CPOperand[]{input1, input2},
			...
			if( mo2.getNumColumns() == 1 && mo2.getNumRows() != mo1.getNumColumns()) { //MV
				...
				//execute federated operations and aggregate
				if ( _federatedOutput || !OptimizerUtils.FEDERATED_COMPILATION){
					FederatedRequest fr3 = mo1.getFedMapping().cleanup(getTID(), fr1.getID());
		...
		//#2 vector - federated matrix multiplication
			...
			FederatedRequest fr2 = FederationUtils.callInstruction(instString, output, new CPOperand[]{input1, input2},
				new long[]{fr1[0].getID(), mo2.getFedMapping().getID()}, _federatedOutput);
			if ( _federatedOutput ){
	...
	private void setPartialOutput(FederationMap federationMap, MatrixObject mo1, MatrixObject mo2,
		...
		FederationMap outputFedMap =
			federationMap.copyWithNewIDAndRange(mo1.getNumRows(), mo2.getNumColumns(), outputID);
