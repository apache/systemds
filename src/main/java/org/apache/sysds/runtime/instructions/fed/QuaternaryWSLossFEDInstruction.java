...
public class QuaternaryWSLossFEDInstruction extends QuaternaryFEDInstruction
{
	...
	public void processInstruction(ExecutionContext ec)
	{
		...
		if(!(X.isFederated() && !U.isFederated() && !V.isFederated() && (W == null || !W.isFederated())))
			throw new DMLRuntimeException("Unsupported federated inputs (X, U, V, W) = ("
				+X.isFederated()+", "+U.isFederated()+", "+V.isFederated() + (W != null ? W.isFederated() : "none") +")");
		...
		if(frInit3 != null)
		{
			...
			response = fedMap.execute(getTID(), true, frInit3,
				frCompute1, frGet1,
		...
		{
			// execute federated instructions
			response = fedMap.execute(getTID(), true, frInit1, frInit2,
				frCompute1, frGet1,
