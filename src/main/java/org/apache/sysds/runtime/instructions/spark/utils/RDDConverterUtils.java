...
public class RDDConverterUtils {
  ...
  public static JavaPairRDD<MatrixIndexes, MatrixBlock> textCellToBinaryBlock(JavaSparkContext sc,
    JavaPairRDD<LongWritable, Text> input, DataCharacteristics mcOut, boolean outputEmptyBlocks,
  ...
  public static JavaPairRDD<MatrixIndexes, MatrixBlock> binaryCellToBinaryBlock(JavaSparkContext sc,
    ...
    JavaPairRDD<MatrixIndexes, MatrixBlock> out = input.mapPartitionsToPair(new BinaryCellToBinaryBlockFunction(mcOut));
  ...
  public static JavaRDD<LabeledPoint> binaryBlockToLabeledPoints(JavaPairRDD<MatrixIndexes, MatrixBlock> in) {
    ...
    JavaRDD<LabeledPoint> pointrdd = in.values().flatMap(new PrepareBinaryBlockFunction());

    return pointrdd;
  }

  public static JavaRDD<String> binaryBlockToTextCell(JavaPairRDD<MatrixIndexes, MatrixBlock> in,
    DataCharacteristics mc) {
    return in.flatMap(new ConvertMatrixBlockToIJVLines(mc.getBlocksize()));
  }

  public static JavaRDD<String> binaryBlockToCsv(JavaPairRDD<MatrixIndexes, MatrixBlock> in, DataCharacteristics mcIn,
    FileFormatPropertiesCSV props, boolean strict) {
  ...
  public static JavaPairRDD<MatrixIndexes, MatrixBlock> csvToBinaryBlock(JavaSparkContext sc,
    JavaPairRDD<LongWritable, Text> input, DataCharacteristics mc, boolean hasHeader, String delim, boolean fill,
    double fillValue, Set<String> naStrings) {
    ...
    JavaPairRDD<MatrixIndexes, MatrixBlock> out = prepinput
      .mapPartitionsToPair(new CSVToBinaryBlockFunction(mc, sparse, hasHeader, delim, fill, fillValue, naStrings));
  ...
  public static JavaPairRDD<MatrixIndexes, MatrixBlock> csvToBinaryBlock(JavaSparkContext sc, JavaRDD<String> input,
    DataCharacteristics mcOut, boolean hasHeader, String delim, boolean fill, double fillValue, Set<String> naStrings) {
  ...
  public static JavaPairRDD<MatrixIndexes, MatrixBlock> dataFrameToBinaryBlock(JavaSparkContext sc, Dataset<Row> df,
    DataCharacteristics mc, boolean containsID, boolean isVector) {
    //determine unknown dimensions and sparsity if required
    if(!mc.dimsKnown(true)) {
      LongAccumulator aNnz = sc.sc().longAccumulator("nnz");
      JavaRDD<Row> tmp = df.javaRDD().map(new DataFrameAnalysisFunction(aNnz, containsID, isVector));
      long rlen = tmp.count();
      long clen = !isVector ? df.columns().length - (containsID ? 1 : 0) : ((Vector) tmp.first()
        .get(containsID ? 1 : 0)).size();
    ...
    JavaPairRDD<Row, Long> prepinput = containsID ? df.javaRDD()
      .mapToPair(new DataFrameExtractIDFunction(df.schema().fieldIndex(DF_ID_COLUMN))) : df.javaRDD()
      .zipWithIndex(); //zip row index

    //convert csv rdd to binary block rdd (w/ partial blocks)
    boolean sparse = requiresSparseAllocation(prepinput, mc);
    JavaPairRDD<MatrixIndexes, MatrixBlock> out = prepinput
      .mapPartitionsToPair(new DataFrameToBinaryBlockFunction(mc, sparse, containsID, isVector));
  ...
  public static Dataset<Row> binaryBlockToDataFrame(SparkSession sparkSession,
    ...
    JavaRDD<Row> rowsRDD = in.flatMapToPair(new SliceBinaryBlockToRowsFunction(mc.getBlocksize())).groupByKey()
      .map(new ConvertRowBlocksToRows((int) mc.getCols(), mc.getBlocksize(), toVector));
  ...
  @Deprecated public static Dataset<Row> binaryBlockToDataFrame(SQLContext sqlContext,
    JavaPairRDD<MatrixIndexes, MatrixBlock> in, DataCharacteristics mc, boolean toVector) {
   ...
   */
  public static void libsvmToBinaryBlock(JavaSparkContext sc, String pathIn, String pathX, String pathY,
    ...
    try {
      ...
      JavaRDD<org.apache.spark.mllib.regression.LabeledPoint> lpoints = MLUtils
        .loadLibSVMFile(sc.sc(), pathIn, numFeatures, numPartitions).toJavaRDD();

      //append row index and best-effort caching to avoid repeated text parsing
      JavaPairRDD<org.apache.spark.mllib.regression.LabeledPoint, Long> ilpoints = lpoints.zipWithIndex()
      ...
      DataCharacteristics mc2 = new MatrixCharacteristics(mcOutX.getRows(), mcOutX.getCols(), mcOutX.getBlocksize(),
        -1);
  ...
  private static boolean requiresSparseAllocation(JavaPairRDD<?, ?> in, DataCharacteristics mc) {
    ...
    if(!mc.nnzKnown() || (mc.nnzKnown() && MatrixBlock
      .evalSparseFormatInMemory(mc.getRows(), mc.getCols(), mc.getNonZeros()))) {
      return true;
    }
    ...
    double rowsize = OptimizerUtils.estimatePartitionedSizeExactSparsity(1, mc.getCols(), mc.getNumRowBlocks(),
  ...
  private static Vector createVector(MatrixBlock row) {
    if(row.isEmptyBlock(false)) //EMPTY SPARSE ROW
      ...
      return Vectors.sparse(row.getNumColumns(), row.getSparseBlock().indexes(0), row.getSparseBlock().values(0));
    else // DENSE ROW
  ...
  private static class PrepareBinaryBlockFunction implements FlatMapFunction<MatrixBlock, LabeledPoint> {
    ...
    @Override public Iterator<LabeledPoint> call(MatrixBlock arg0) throws Exception {
  ...
  private static class TextToBinaryBlockFunction extends CellToBinaryBlockFunction
    ...
    @Override public Iterator<Tuple2<MatrixIndexes, MatrixBlock>> call(Iterator<Text> arg0) throws Exception {
      ...
      while(arg0.hasNext()) {
        ...
        double val = (_mmProps == null) ? st.nextDouble() : _mmProps.isPatternField() ? 1 : _mmProps.isIntField() ? st
          .nextLong() : st.nextDouble();
  ...
  private static class TextToSerTextFunction implements PairFunction<Tuple2<Long, String>, LongWritable, Text> {
    ...
    @Override public Tuple2<LongWritable, Text> call(Tuple2<Long, String> arg0) throws Exception {
  ...
  private static class StringToSerTextFunction implements PairFunction<String, LongWritable, Text> {
    ...
    @Override public Tuple2<LongWritable, Text> call(String arg0) throws Exception {
  ...
  public static class BinaryCellToBinaryBlockFunction extends CellToBinaryBlockFunction
    ...
    @Override public Iterator<Tuple2<MatrixIndexes, MatrixBlock>> call(Iterator<Tuple2<MatrixIndexes, MatrixCell>> arg0)
      throws Exception {
      ArrayList<Tuple2<MatrixIndexes, MatrixBlock>> ret = new ArrayList<>();
      ReblockBuffer rbuff = new ReblockBuffer(_bufflen, _rlen, _clen, _blen);
  ...
  private static class CSVToBinaryBlockFunction
    ...
    public CSVToBinaryBlockFunction(DataCharacteristics mc, boolean sparse, boolean hasHeader, String delim,
      boolean fill, double fillValue, Set<String> naStrings) {
      _rlen = mc.getRows();
      _clen = mc.getCols();
      _blen = mc.getBlocksize();
      ...
      while(arg0.hasNext()) {
        ...
        for(int cix = 1, pix = 0; cix <= ncblks; cix++) {
          ...
          for(int j = 0; j < lclen; j++) {
            ...
            double val = (part.isEmpty() && _fill) ? _fillValue : UtilFunctions.parseToDouble(part, _naStrings);
    ...
    private static void flushBlocksToList(MatrixIndexes[] ix, MatrixBlock[] mb,
  ...
  private static class LabeledPointToBinaryBlockFunction implements
    ...
    public LabeledPointToBinaryBlockFunction(DataCharacteristics mc, boolean labels, LongAccumulator aNnz) {
      ...
      _clen = mc.getCols();
      _blen = mc.getBlocksize();
      _sparseX = MatrixBlock.evalSparseFormatInMemory(mc.getRows(), mc.getCols(), mc.getNonZeros());
      _labels = labels;
      _aNnz = aNnz;
    }

    @Override public Iterator<Tuple2<MatrixIndexes, MatrixBlock>> call(
      Iterator<Tuple2<org.apache.spark.mllib.regression.LabeledPoint, Long>> arg0) throws Exception {
      ArrayList<Tuple2<MatrixIndexes, MatrixBlock>> ret = new ArrayList<>();
      ...
      while(arg0.hasNext()) {
        ...
        boolean lsparse = _sparseX || (!_labels && row
          .features() instanceof org.apache.spark.mllib.linalg.SparseVector);
        long rowix = tmp._2() + 1;
        ...
        if(_labels) {
          ...
          if(row.features() instanceof org.apache.spark.mllib.linalg.SparseVector) {
            org.apache.spark.mllib.linalg.SparseVector srow = (org.apache.spark.mllib.linalg.SparseVector) row
              .features();
  ...
  private static class SliceBinaryBlockToRowsFunction
    ...
    @Override public Iterator<Tuple2<Long, Tuple2<Long, MatrixBlock>>> call(Tuple2<MatrixIndexes, MatrixBlock> arg0)
      throws Exception {
  ...
  private static class ConcatenateBlocksFunction
    ...
    @Override public Tuple2<MatrixIndexes, MatrixBlock> call(Tuple2<Long, Iterable<Tuple2<Long, MatrixBlock>>> arg0)
      throws Exception {
  ...
  private static class DataFrameToBinaryBlockFunction
    ...
    public DataFrameToBinaryBlockFunction(DataCharacteristics mc, boolean sparse, boolean containsID,
      boolean isVector) {
      _rlen = mc.getRows();
      _clen = mc.getCols();
      _blen = mc.getBlocksize();
    ...
    @Override public Iterator<Tuple2<MatrixIndexes, MatrixBlock>> call(Iterator<Tuple2<Row, Long>> arg0)
      throws Exception {
      ArrayList<Tuple2<MatrixIndexes, MatrixBlock>> ret = new ArrayList<>();
  ...
  private static class DataFrameAnalysisFunction implements Function<Row, Row> {
    ...
    @Override public Row call(Row arg0) throws Exception {
  ...
  public static class DataFrameExtractIDFunction implements PairFunction<Row, Row, Long> {
    ...
    @Override public Tuple2<Row, Long> call(Row arg0) throws Exception {
      ...
      if(id <= 0) {
        throw new DMLRuntimeException(
          "ID Column '" + DF_ID_COLUMN + "' expected to be 1-based, but found value: " + id);
      }
  ...
  private static class ConvertRowBlocksToRows
    ...
    @Override public Row call(Tuple2<Long, Iterable<Tuple2<Long, MatrixBlock>>> arg0) throws Exception {
