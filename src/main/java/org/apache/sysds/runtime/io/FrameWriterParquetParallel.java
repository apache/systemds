...
public class FrameWriterParquetParallel extends FrameWriterParquet {
     ...
     */
    @Override
    protected void writeParquetFrameToHDFS(Path path, Configuration conf, FrameBlock src) throws IOException, DMLRuntimeException {
        ...
        try {
            ...
            for (int i = 0; i < numThreads; i++) {
                ...
                if (startRow < endRow) {
                    ...
                    tasks.add(new WriteFileTask(newPath, conf, fs, src.slice(startRow, endRow - 1)));
                }
            }
    ...
    private class WriteFileTask implements Callable<Object> {
        ...
        public WriteFileTask(Path path, Configuration conf, FileSystem fs, FrameBlock src) {
            this.path = path;
            this.conf = conf;
            this.fs = fs;
            this.src = src;
