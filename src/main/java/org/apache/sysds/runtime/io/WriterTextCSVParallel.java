...
public class WriterTextCSVParallel extends WriterTextCSV
{
		...
		throws IOException 
	{
		...
		int numPartFiles = (int)(OptimizerUtils.estimateSizeTextOutput(src.getNumRows(), src.getNumColumns(),
				src.getNonZeros(), FileFormat.CSV)  / InfrastructureAnalyzer.getHDFSBlockSize());
	...
	private static class WriteCSVTask implements Callable<Object> 
	{
		...
		public WriteCSVTask(Path path, JobConf job, FileSystem fs, MatrixBlock src, int rl, int ru, FileFormatPropertiesCSV props) {
			_path = path;
			_job = job;
			_fs = fs;
			_src = src;
