...
public class FormatIdentifying {
    ...
    private void runIdentification() {
        ...
        mapHas = mappingValues.getMapHas();
        ...
        for (int r = 0; r < nrows; r++)
            missedCount += ncols - mostCommonScore(r);
        if ((float) missedCount / NaN < 0.07) isSingleRow = true;
        ...
        if (isSingleRow) {
                ...
                if (c == numberOfSelectedCols - 1) {
                    ...
                    for (int si : selectedRowIndex) {
                        for (int ci = 0; ci < ncols; ci++) {
                            int cri = (int) mapRow.getValue(si, ci);
                            if (mapHas.getValue(si, ci) == 1) {
                                String str = sampleRawIndexes.get(cri).getSubString(0, (int) mapCol.getValue(si, ci));
                    ...
                    do {
                        ...
                        for (ArrayList<String> keyPattern : keyPatterns) {
                            ...
                            check |= newCheck;
                            if (newCheck) selectedKeyPatterns.add(keyPattern);
                        }
                        if (check) keyPatterns = selectedKeyPatterns;
                        else {
                            flagReconstruct = rowTrie.reConstruct();
                            if (!flagReconstruct) break;
                        }
                    } while (!check);
            ...
            if (beginPos.size() == 1) {
                ...
                if (bpos > 0) properties.setRowIndexBegin("-" + bpos);
                else properties.setRowIndexBegin("");
            } else {
    ...
    private boolean checkPrefixRowIndex(int colIndex, int beginPos, ArrayList<RawIndex> prefixRawIndex) {
        for (int r = 0; r < nrows; r++) {
            int rowIndex = (int) this.mapRow.getValue(r, colIndex);
            if (rowIndex != -1) {
                boolean flag = false;
    ...
    private Integer mostCommonScore(int rowIndex) {
        Map<Integer, Integer> map = new HashMap<>();
        int nan = 0;
        for (int c = 0; c < ncols; c++) {
            if (mapHas.getValue(rowIndex, c) != 0) {
                int t = (int) mapRow.getValue(rowIndex, c);
                Integer val = map.get(t);
                map.put(t, val == null ? 1 : val + 1);
            } else nan++;
        }
        if (map.size() == 0) return nan;
    ...
    private KeyTrie[] buildColsKeyPatternSingleRow() {
        ...
        for (int c = 0; c < ncols; c++) {
            ...
            for (int w = 1; w < windowSize && flag; w++) {
                ...
                for (String s : list) {
                    if (s.length() < w) flag = false;
                    else {
                        String subStr = s.substring(s.length() - w);
                        if (!subStr.contains(Lop.OPERAND_DELIMITOR)) wts.add(subStr);
                        else flag = false;
                    }
                }

                if (flag) {
                    if (wts.size() == 1) token = wts.iterator().next();
                    else {
                        for (String t : wts) {
                            int count = 0;
                            for (String s : list) {
                                if (s.endsWith(t)) count++;
                            }
                            float percent = (float) count / list.size();
                            if (percent >= 1) token = t;
                        }
                    }
                } else if (wts.size() == 0) token = "";
            }
            if (token == null) {
                int[] listLength = new int[nrows];
                for (int r = 0; r < nrows; r++) {
                    if (mapHas.getValue(r, c) == 1) listLength[r] = (int) mapCol.getValue(r, c);
                    else listLength[r] = -1;
                }
                ...
                if (commonLength == 0) {
                    ...
                    for (String s : list) {
                        if (s.length() == 0) newList.add(s);
                    }
                    prefixStrings.getKey()[c] = newList;
                } else throw new RuntimeException("can't build a key pattern for the column: " + c);
            } else if (token.length() > 0) {
                ArrayList<String> newList = new ArrayList<>();
                for (String s : list) {
                    if (s.endsWith(token)) newList.add(s);
                }
                prefixStrings.getKey()[c] = newList;
            }
        }
        ...
        for (int c = 0; c < ncols; c++) {
            ...
            if (trie.getRoot().getChildren().size() == 1) {
                ...
                for (String ps : splitPattern)
                    if (ps.length() > 0) reverseSplitPattern.add(ps);
                if (reverseSplitPattern.size() == 0) reverseSplitPattern.add("");
    ...
    public Pair<ArrayList<String>, ArrayList<Integer>> extractAllPrefixStringsOfAColSingleLine(int colIndex, boolean reverse) {
        ...
        for (int r = 0; r < nrows; r++) {
            int rowIndex = (int) mapRow.getValue(r, colIndex);
            if (mapHas.getValue(r, colIndex) == 1) {
                rowIndexes.add(rowIndex);
                String str = sampleRawIndexes.get(rowIndex).getRemainedTexts((int) mapCol.getValue(r, colIndex));//sampleRawIndexes.get(rowIndex)
                // .getSubString(0,
                // mapCol[r][colIndex]);
                if (reverse) prefixStrings.add(new StringBuilder(str).reverse().toString());
                else prefixStrings.add(str);
            }
        }
        return new Pair<>(prefixStrings, rowIndexes);
    }
    ...
    private ArrayList<String>[] extractAllSuffixStringsOfColsSingleLine() {
        ...
        for (int c = 0; c < ncols; c++) {
            ...
            for (int r = 0; r < nrows; r++) {
                int rowIndex = (int) mapRow.getValue(r, c);
                if (mapHas.getValue(r,c) != 1) continue;
                String str = sampleRawIndexes.get(rowIndex).getRaw().substring((int) (mapCol.getValue(r, c) + mapLen.getValue(r, c)));
                result[c].add(str);
            }
        }
        return result;
    ...
    private ArrayList<ArrayList<String>> findRowDelimiters() {
        ...
        for (int r = 0; r < nrows; r++) {
            ...
            for (int c = 0; c < ncols; c++)
                if (mapHas.getValue(r, c) != 0) {
                    tmpUsedLines[r].set((int) mapRow.getValue(r, c));
                    if (mapRow.getValue(r, c) <= min) {
                        min = (int) mapRow.getValue(r, c);
                        if (minPos != 0) minPos = (int) Math.min(minPos, mapCol.getValue(r, c));
                        else minPos = (int) mapCol.getValue(r, c);

                    }
                    if (maxColPos.containsKey((int) mapRow.getValue(r, c)))
                        maxColPos.put((int) mapRow.getValue(r, c), (int) Math.max(maxColPos.get((int) mapRow.getValue(r, c)), (int) mapCol.getValue(r, c) + mapLen.getValue(r, c)));
                    else maxColPos.put((int) mapRow.getValue(r, c), (int) (mapCol.getValue(r, c) + mapLen.getValue(r, c)));
                }
            minList[r] = min;
            minColPos[r] = minPos;
        }
        ...
        for (int r = 0; r < nrows; r++) {
            ...
            for (int i = 0; i < nrows; i++) {
                if (i != r) usedLines[r].or(tmpUsedLines[i]);
            }
        }
        ...
        for (int r = 0; r < nrows; r++) {
            ...
            for (; beginLine >= 0; beginLine--)
                if (usedLines[r].get(beginLine)) break;

            StringBuilder sb = new StringBuilder();
            beginLine = Math.max(beginLine, 0);

            if (beginLine + 1 == nlines) continue;

            Integer subStrPos = 0;
            if (maxColPos.containsKey(beginLine)) subStrPos = maxColPos.get(beginLine);
    ...
    private Pair<ArrayList<String>[], Pair<Integer, Integer>[]> extractAllPrefixStringsOfColsMultiLine(boolean reverse) {
        ...
        for (int r = 0; r < nrows; r++)
            for (int c = 0; c < ncols; c++)
                if (mapHas.getValue(r, c) != 0) tmpUsedLines[r].set((int) mapRow.getValue(r, c));
        ...
        for (int c = 0; c < ncols; c++) {
            ...
            for (int r = 0; r < nrows; r++) {
                ...
                for (int i = lastLine; i < rowIndex; i++) {
                    if (sampleRawIndexes.get(i).getRawLength() > 0) sb.append(sampleRawIndexes.get(i).getRaw()).append("\n");
                }
                String str = sampleRawIndexes.get(rowIndex).getSubString(0, (int) mapCol.getValue(r, c));
                if (str.length() > 0 && !str.equals("\n")) sb.append(str);
                else if (lastLine < rowIndex) sb.deleteCharAt(sb.length() - 1);


                if (reverse) result[c].add(sb.reverse().toString());
                else result[c].add(sb.toString());
                max = Math.max(max, sb.length());
                if (sb.length() < min || min == 0) min = sb.length();
    ...
    private HashSet<String> findStartWithIntersectOfStrings(ArrayList<String> strList) {
        ...
        for (int w = windowSize; w > 2; w--) {
            ...
            if (totalIntersect.size() > 0) return totalIntersect;
    ...
    private Pair<Integer, Integer> getIndexOfKeyPatternOnString(String str, ArrayList<String> key, int beginPos) {
        ...
        for (String k : key) {
            ...
            if (index != -1) currPos = index + k.length();
            else {
                flag = false;
                break;
            }
            if (startPos == -1) startPos = currPos;
        }
        if (flag) return new Pair<>(startPos, currPos + key.get(key.size() - 1).length());
    ...
    private String getRemainedSubstring(String str, ArrayList<String> keys) {
        ...
        if (flag) return str.substring(currPos);
        else return null;
    }
