...
public class LibMatrixCuDNN extends LibMatrixCUDA {
	...
	public static void conv2dBiasAdd(GPUContext gCtx, String instName, MatrixObject image, MatrixObject bias,
			...
			int pad_w, int stride_h, int stride_w, int P, int Q, double intermediateMemoryBudget) {
	 ...
	 */
	private static Pointer denseIm2col(GPUContext gCtx, String instName, MatrixObject image, boolean isSparseImage,
			long N, long C, long H, long W, int R, int S, int pad_h, int pad_w, int stride_h, int stride_w, int P,
			int Q) {
		Pointer im2colPointer;
		if(isSparseImage) {
			...
			if(inPointer.nnz < 0) {
				...
				getCudaKernels(gCtx).launchKernel("sparse_dense_im2col",
						ExecutionConfig.getConfigForSimpleVectorOperations(toInt(inPointer.nnz)), inPointer.val,
						inPointer.rowPtr, inPointer.colInd, im2colPointer, inPointer.nnz, N, C * H * W, H * W, W, R, S,
						P, Q, P * Q, R * S, N * P * Q, stride_h, stride_w, pad_h, pad_w);
			}
			else
				return null;
		}
			...
			getCudaKernels(gCtx).launchKernel("dense_dense_im2col",
					ExecutionConfig.getConfigForSimpleVectorOperations(toInt(N * C * H * W)), imagePointer,
					im2colPointer, N * C * H * W, C * H * W, H * W, W, R, S, P, Q, P * Q, R * S, N * P * Q, stride_h,
	...
	public static void conv2d(GPUContext gCtx, String instName, MatrixObject image, MatrixObject filter,
			...
			int stride_h, int stride_w, int P, int Q, double intermediateMemoryBudget) {
		...
		if(NCHW < maxNumElementsOfCuDNNTensor && NKPQ < maxNumElementsOfCuDNNTensor &&
				KCRS < maxNumElementsOfCuDNNTensor) {
			if(isSparseFilter && (OptimizerUtils.estimateSizeExactSparsity(CRS, NPQ, 1.0) +
					OptimizerUtils.estimateSizeExactSparsity(K, NPQ, 1.0)) <
				...
				LibMatrixCuMatMult.sparseDenseMatMult(gCtx, instName, matmultOutputPointer, filterPointer,
						im2colPointer, K, CRS, CRS, NPQ, K, NPQ, false, false);
				...
				getCudaKernels(gCtx).launchKernel("reorg_knpq",
						ExecutionConfig.getConfigForSimpleVectorOperations(toInt(NKPQ)), matmultOutputPointer,
				...
				try(LibMatrixCuDNNConvolutionAlgorithm algo = LibMatrixCuDNNConvolutionAlgorithm.cudnnGetConvolutionForwardAlgorithm(
						gCtx, instName, localN, C, H, W, K, R, S, pad_h, pad_w, stride_h, stride_w, P, Q,
						workspaceLimit)) {
					if(localN == N) {
						...
						try(LibMatrixCuDNNInputRowFetcher imgFetcher = new LibMatrixCuDNNInputRowFetcher(gCtx, instName,
								image)) {
							for(int n = 0; n < N; n++) {
	...
	/**
	 * Performs a "softmax" operation on a matrix on the GPU
	 *
	 ...
	 */
	public static void softmax(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1,
			String outputName) {
		if(LOG.isTraceEnabled()) {
			LOG.trace("GPU : softmax" + ", GPUContext=" + gCtx);
		}
		cudnnTensorDescriptor tensorDesc = allocateTensorDescriptor(toInt(in1.getNumRows()), toInt(in1.getNumColumns()),
				1, 1);
	 ...
	 */
	@SuppressWarnings("SameParameterValue") private static cudnnTensorDescriptor allocateTensorDescriptor(int N, int C,
			int H, int W) {
	...
	/**
	 * Throw a user-friendly error that shows limitation of invoking a cuDNN kernel
	 *
	...
	private static void throwCuDNNDimensionError(long dim1, long dim2, long dim3, long dim4) {
		throw new DMLRuntimeException(
				"The dimensions of input/output matrices is too large to execute a CuDNN kernel. " +
						"Max CuDNN matrix size:" + maxNumElementsOfCuDNNTensor + ". " +
						"Given input matrix dimensions: [" + dim1 + "," + dim2 + "]. Output dimension:  [" + dim3 +
						"," + dim4 + "].");
	}
	...
	private static void throwCuDNNDimensionError(long dim1, long dim2, long dim3, long dim4, long dim5, long dim6) {
		throw new DMLRuntimeException(
				"The dimensions of input/output matrices is too large to execute a CuDNN kernel. " +
						"Max CuDNN matrix size:" + maxNumElementsOfCuDNNTensor + ". " +
						"Given input matrix dimensions: [" + dim1 + "," + dim2 + "], [" + dim3 + "," + dim4 +
						"]. Output dimension: [" + dim5 + "," + dim6 + "]");
	}

	/**
	 * Performs 2D convolution
	 * Takes up an insignificant amount of intermediate space when CONVOLUTION_PREFERENCE is set to
	 * CUDNN_CONVOLUTION_FWD_NO_WORKSPACE
	 * Intermediate space is required by the filter descriptor and convolution descriptor which are metadata structures
	 * and don't scale with the size of the input
	 *
	 * @param gCtx     a valid {@link GPUContext}
	 * @param instName the invoking instruction's name for record {@link Statistics}.
	...
	private static void cudnnConv2d(GPUContext gCtx, String instName, Pointer image, Pointer filter, Pointer output,
		...
		if(LOG.isTraceEnabled()) {
			LOG.trace("GPU : conv2d (instName=" + instName + "), GPUContext=" + gCtx);
		}
		try {
			int status = cudnnConvolutionForward(getCudnnHandle(gCtx), one(), algo.nchwTensorDesc, image,
					algo.filterDesc, filter, algo.convDesc, algo.algo, algo.workSpace, algo.sizeInBytes, zero(),
					algo.nkpqTensorDesc, output);
			if(status != cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException(
						"Could not executed cudnnConvolutionForward: " + cudnnStatus.stringFor(status));
			}
		}
		catch(CudaException e) {
			throw new DMLRuntimeException(
					"Error in conv2d in GPUContext " + gCtx.toString() + " from Thread " + Thread.currentThread(), e);
		}
	}
	...
	public static void conv2dBackwardFilter(GPUContext gCtx, String instName, MatrixObject image, MatrixObject dout,
		...
		if(NCHW < maxNumElementsOfCuDNNTensor && NKPQ < maxNumElementsOfCuDNNTensor &&
			...
			try(LibMatrixCuDNNConvolutionAlgorithm algo = LibMatrixCuDNNConvolutionAlgorithm.cudnnGetConvolutionBackwardFilterAlgorithm(
					gCtx, instName, localN, C, H, W, K, R, S, pad_h, pad_w, stride_h, stride_w, P, Q, workspaceLimit)) {
				if(localN == N) {
					...
					try(LibMatrixCuDNNInputRowFetcher imgFetcher = new LibMatrixCuDNNInputRowFetcher(gCtx, instName,
							image); LibMatrixCuDNNInputRowFetcher doutFetcher = new LibMatrixCuDNNInputRowFetcher(gCtx,
							instName, dout)) {
						...
						for(int n = 0; n < N; n++) {
							...
							// Perform one-input conv2dBackwardFilter
							cudnnConv2dBackwardFilter(gCtx, instName, imgFetcher.getNthRow(n), doutFetcher.getNthRow(n),
									tempdwPointer, algo);
							getCudaKernels(gCtx).launchKernel("inplace_add",
									ExecutionConfig.getConfigForSimpleMatrixOperations(K, toInt(CRS)), tempdwPointer,
	 ...
	 * @param algo         cudnn algorithm wrapper
	 */
	private static void cudnnConv2dBackwardFilter(GPUContext gCtx, String instName, Pointer imagePointer,
			Pointer doutPointer, Pointer dwPointer, LibMatrixCuDNNConvolutionAlgorithm algo) {
		if(LOG.isTraceEnabled()) {
			LOG.trace("GPU : conv2dBackwardFilter (instName=" + instName + "), GPUContext=" + gCtx);
		}
		try {
			int status = cudnnConvolutionBackwardFilter(getCudnnHandle(gCtx), one(), algo.nchwTensorDesc, imagePointer,
					algo.nkpqTensorDesc, doutPointer, algo.convDesc, algo.algo, algo.workSpace, algo.sizeInBytes,
					zero(), algo.filterDesc, dwPointer);
	...
	public static void conv2dBackwardData(GPUContext gCtx, String instName, MatrixObject filter, MatrixObject dout,
			...
			int stride_w, int P, int Q, double intermediateMemoryBudget) {
		long CHW = (long) C * H * W;
		long KPQ = (long) K * P * Q;
		long CRS = (long) C * R * S;
		long NCHW = N * CHW;
		...
		if(NCHW < maxNumElementsOfCuDNNTensor && NKPQ < maxNumElementsOfCuDNNTensor &&
			...
			try(LibMatrixCuDNNConvolutionAlgorithm algo = LibMatrixCuDNNConvolutionAlgorithm.cudnnGetConvolutionBackwardDataAlgorithm(
					gCtx, instName, localN, C, H, W, K, R, S, pad_h, pad_w, stride_h, stride_w, P, Q, workspaceLimit)) {
				if(localN == N) {
					...
					try(LibMatrixCuDNNInputRowFetcher doutFetcher = new LibMatrixCuDNNInputRowFetcher(gCtx, instName,
							dout)) {
						for(int n = 0; n < N; n++) {
							cudnnConv2dBackwardData(gCtx, instName, doutFetcher.getNthRow(n), filterPointer,
									dstPointer.withByteOffset(n * CHW * sizeOfDataType), algo);
						}
					}
				}
			}
	 ...
	 * @param algo     cudnn algorithm wrapper
	 */
	private static void cudnnConv2dBackwardData(GPUContext gCtx, String instName, Pointer w, Pointer dy, Pointer dx,
			LibMatrixCuDNNConvolutionAlgorithm algo) {
		if(LOG.isTraceEnabled()) {
			LOG.trace("GPU : conv2dBackwardData (instName=" + instName + "), GPUContext=" + gCtx);
		}
		try {
			int status = cudnnConvolutionBackwardData(getCudnnHandle(gCtx), one(), algo.filterDesc, w,
					algo.nkpqTensorDesc, dy, algo.convDesc, algo.algo, algo.workSpace, algo.sizeInBytes, zero(),
					algo.nchwTensorDesc, dx);
	 ...
	 * @param intermediateMemoryBudget intermediate memory budget
	 */
	public static void pooling(GPUContext gCtx, String instName, MatrixObject image, MatrixObject outputBlock, int N,
			int C, int H, int W, int R, int S, int pad_h, int pad_w, int stride_h, int stride_w, int P, int Q,
			PoolingType poolingType, double intermediateMemoryBudget) {
		long CHW = (long) C * H * W;
		long CPQ = (long) C * P * Q;
		long NCHW = N * CHW;
		long NCPQ = N * CPQ;
		...
		if(NCHW < maxNumElementsOfCuDNNTensor && NCPQ < maxNumElementsOfCuDNNTensor) {
			...
			if(overhead <= intermediateMemoryBudget) {
				...
				cudnnPoolingHelper(gCtx, instName, x, y, N, C, H, W, R, S, pad_h, pad_w, stride_h, stride_w, P, Q,
						poolingType);
			}
			else {
				try(LibMatrixCuDNNInputRowFetcher imgFetcher = new LibMatrixCuDNNInputRowFetcher(gCtx, instName,
						image)) {
					for(int n = 0; n < N; n++) {
						cudnnPoolingHelper(gCtx, instName, imgFetcher.getNthRow(n),
								y.withByteOffset(n * CPQ * sizeOfDataType), 1, C, H, W, R, S, pad_h, pad_w, stride_h,
								stride_w, P, Q, poolingType);
					}
				}
			}
		}
	...
	private static void cudnnPoolingHelper(GPUContext gCtx, String instName, Pointer x, Pointer y, int N, int C, int H,
			int W, int R, int S, int pad_h, int pad_w, int stride_h, int stride_w, int P, int Q,
			PoolingType poolingType) {
		if(LOG.isTraceEnabled()) {
			LOG.trace("GPU : perform pooling instName=(" + instName + "), GPUContext=" + gCtx);
		}

		try(LibMatrixCuDNNPoolingDescriptors desc = LibMatrixCuDNNPoolingDescriptors.cudnnPoolingDescriptors(N, C, H, W,
				R, S, pad_h, pad_w, stride_h, stride_w, P, Q, poolingType)) {
			int status = cudnnPoolingForward(getCudnnHandle(gCtx), desc.poolingDesc, one(), desc.xDesc, x, zero(),
					desc.yDesc, y);
			if(status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException(
						"Could not executed cudnnPoolingForward: " + jcuda.jcudnn.cudnnStatus.stringFor(status));
			}
		}
		catch(CudaException e) {
			throw new DMLRuntimeException(
	 ...
	 * This method computes the backpropagation errors for previous layer of maxpooling operation
	 *
	 * @param gCtx                     a valid {@link GPUContext}
	 * @param instName                 the invoking instruction's name for record {@link Statistics}.
	 * @param image                    image as matrix object
	 ...
	 */
	 public static void poolingBackward(GPUContext gCtx, String instName,
			MatrixObject image, MatrixObject dout, MatrixObject maxpoolOutput, MatrixObject outputBlock, int N, int C,
		...
		if(NCHW < maxNumElementsOfCuDNNTensor && NCPQ < maxNumElementsOfCuDNNTensor) {
			...
			if(overhead <= intermediateMemoryBudget) {
				...
				cudnnPoolingBackwardHelper(gCtx, instName, x, dy, y, dx, N, C, H, W, R, S, pad_h, pad_w, stride_h,
						stride_w, P, Q, poolingType);
			}
			else {
				...
				LibMatrixCuDNNInputRowFetcher maxPoolOutFetcher = isMaxPoolOutputProvided ? new LibMatrixCuDNNInputRowFetcher(
						gCtx, instName, maxpoolOutput) : null;
				for(int n = 0; n < N; n++) {
					...
					cudnnPoolingBackwardHelper(gCtx, instName, x, dy, y, dx.withByteOffset(n * CHW * sizeOfDataType), 1,
							C, H, W, R, S, pad_h, pad_w, stride_h, stride_w, P, Q, poolingType);
				}
				// Deallocate temporary array to hold one element of input
				imgFetcher.close();
				doutFetcher.close();
	...
	private static void cudnnPoolingBackwardHelper(GPUContext gCtx, String instName, Pointer x, Pointer dy, Pointer y,
			Pointer dx, int N, int C, int H, int W, int R, int S, int pad_h, int pad_w, int stride_h, int stride_w,
			int P, int Q, PoolingType poolingType) {
		if(LOG.isTraceEnabled()) {
		...
		try(LibMatrixCuDNNPoolingDescriptors desc = LibMatrixCuDNNPoolingDescriptors.cudnnPoolingBackwardDescriptors(N,
				C, H, W, R, S, pad_h, pad_w, stride_h, stride_w, P, Q, poolingType)) {
			int status;
			if(!isMaxPoolOutputProvided) {
				long numBytes = (long) N * C * P * Q * sizeOfDataType;
				y = gCtx.allocate(instName, numBytes);
				status = cudnnPoolingForward(getCudnnHandle(gCtx), desc.poolingDesc, one(), desc.xDesc, x, zero(),
						desc.yDesc, y);
				if(status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
					throw new DMLRuntimeException(
							"Could not executed cudnnPoolingForward before cudnnPoolingBackward: " +
									jcuda.jcudnn.cudnnStatus.stringFor(status));
				}
			}
			status = cudnnPoolingBackward(getCudnnHandle(gCtx), desc.poolingDesc, one(), desc.yDesc, y, desc.dyDesc, dy,
					desc.xDesc, x, zero(), desc.dxDesc, dx);

			if(status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException(
						"Could not executed cudnnPoolingBackward: " + jcuda.jcudnn.cudnnStatus.stringFor(status));
			}
		}
		catch(CudaException e) {
			throw new DMLRuntimeException(
	...
	private static void cudnnReLU(GPUContext gCtx, String instName, MatrixObject in, Pointer dstData,
	...
	public static void relu(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in, String outputName) {
		...
		if(N * CHW >= maxNumElementsOfCuDNNTensor) {
			...
			getCudaKernels(gCtx).launchKernel("relu",
					ExecutionConfig.getConfigForSimpleMatrixOperations(toInt(N), toInt(CHW)), srcData, dstData,
					toInt(N), toInt(CHW));
		}
		else {
			cudnnTensorDescriptor tensorDescriptor = new cudnnTensorDescriptor();
			cudnnCreateTensorDescriptor(tensorDescriptor);
			cudnnSetTensor4dDescriptor(tensorDescriptor, CUDNN_TENSOR_NCHW, CUDNN_DATA_TYPE, toInt(N), 1, 1,
					toInt(CHW));
	 ...
	 * @param out0             Outputs from previous timestamp
	 * @param c0               Initial cell state
	 * @param return_sequences Whether to return `out` at all timestamps, or just for the final timestamp.
	 * @param outputName       name of the out variable. If `return_sequences` is True, outputs for all timestamps.
	 * @param cyName           name of the output cell state. Cell state for final timestamp.
	...
	private static void singleLayerUnidirectionalRNNForward(ExecutionContext ec, GPUContext gCtx, String instName,
			...
			@SuppressWarnings("SameParameterValue") String rnnMode, boolean return_sequences, int N, int M, int D,
			int T) throws DMLRuntimeException {
		boolean hasCarry = rnnMode.equalsIgnoreCase("lstm");
		// Get output pointers
		Pointer cudnnYPointer = gCtx.allocate(instName, (long) N * T * M * sizeOfDataType);
		Pointer hyPointer = !return_sequences ? getDenseOutputPointer(ec, gCtx, instName, outputName, N,
				M) : gCtx.allocate(instName, (long) N * M * sizeOfDataType);
		...
		try(LibMatrixCuDNNRnnAlgorithm algo = new LibMatrixCuDNNRnnAlgorithm(gCtx, instName, rnnMode, N, T, M, D,
				true)) {
			JCudnn.cudnnRNNForwardTraining(gCtx.getCudnnHandle(), algo.rnnDesc, T, algo.xDesc, x, algo.hxDesc, hx,
					algo.cxDesc, cx, algo.wDesc, wPointer, algo.yDesc, cudnnYPointer, algo.hyDesc, hyPointer,
					algo.cyDesc, cyPointer, algo.workSpace, algo.sizeInBytes, algo.reserveSpace,
					algo.reserveSpaceSizeInBytes);
		}

		if(return_sequences) {
			gCtx.cudaFreeHelper(instName, hyPointer, DMLScript.EAGER_CUDA_FREE);
			Pointer sysdsYPointer = getDenseOutputPointer(ec, gCtx, instName, outputName, N, (long) T * M);
	...
	public static void lstmBackward(ExecutionContext ec, GPUContext gCtx, String instName, Pointer x, Pointer hx,
			Pointer cx, Pointer wPointer, String doutName, String dcyName,  // input
		...
		Pointer dy = gCtx.allocate(instName, (long) N * T * M * sizeOfDataType);
		...
		LibMatrixCUDA.getCudaKernels(gCtx).launchKernel("prepare_lstm_backward_gradients",
				...
				size, return_sequences ? 1 : 0);
		ec.releaseMatrixInputForGPUInstruction(doutName);

		// Allocate intermediate pointers computed by forward
		Pointer yPointer = gCtx.allocate(instName, (long) N * T * M * sizeOfDataType);
		try(LibMatrixCuDNNRnnAlgorithm algo = new LibMatrixCuDNNRnnAlgorithm(gCtx, instName, "lstm", N, T, M, D,
				true)) {
			JCudnn.cudnnRNNForwardTraining(gCtx.getCudnnHandle(), algo.rnnDesc, T, algo.xDesc, x, algo.hxDesc, hx,
					algo.cxDesc, cx, algo.wDesc, wPointer, algo.yDesc, yPointer, algo.hyDesc, new Pointer(),
					algo.cyDesc, new Pointer(), algo.workSpace, algo.sizeInBytes, algo.reserveSpace,
					algo.reserveSpaceSizeInBytes);

			Pointer cudnnDx = gCtx.allocate(instName, (long) N * T * D * LibMatrixCUDA.sizeOfDataType);
			JCudnn.cudnnRNNBackwardData(gCtx.getCudnnHandle(), algo.rnnDesc, T, algo.yDesc, yPointer,
					// ----------------------
					// Additional inputs:
					algo.dyDesc, dy, algo.dhyDesc, new Pointer(), algo.dcyDesc,
					getDenseInputPointer(ec, gCtx, instName, dcyName, N, M),
					// ----------------------
					algo.wDesc, wPointer, algo.hxDesc, hx, algo.cxDesc, cx,
					// ----------------------
					// Output:
					algo.dxDesc, cudnnDx, algo.dhxDesc, getDenseOutputPointer(ec, gCtx, instName, dhxName, N, M),
			...
			Pointer smlDx = getDenseOutputPointer(ec, gCtx, instName, dxName, N, (long) T * D);
			LibMatrixCUDA.getCudaKernels(gCtx)
			...
			// -------------------------------------------------------------------------------------------
			Pointer cudnnDwPointer = gCtx.allocate(instName, (D + M + 2) * (4L * M) * LibMatrixCUDA.sizeOfDataType);
			JCudnn.cudnnRNNBackwardWeights(gCtx.getCudnnHandle(), algo.rnnDesc, T, algo.xDesc, x, algo.hxDesc, hx,
					algo.yDesc, yPointer, algo.workSpace, algo.sizeInBytes, algo.dwDesc, cudnnDwPointer,
			...
			LibMatrixCUDA.getCudaKernels(gCtx).launchKernel("prepare_lstm_dweight",
					...
					getDenseOutputPointer(ec, gCtx, instName, dbName, 1, 4L * M), cudnnDwPointer, D, M);
	...
	public static void batchNormalizationForwardTraining(GPUContext gCtx, String instName, MatrixObject image,
		...
		cudaMemcpy(retRunningMeanPtr, runningMeanPtr, (long) C * sizeOfDataType, cudaMemcpyDeviceToDevice);
		cudaMemcpy(retRunningVarPtr, runningVarPtr, (long) C * sizeOfDataType, cudaMemcpyDeviceToDevice);
		...
		checkStatus(cudnnBatchNormalizationForwardTraining(getCudnnHandle(gCtx),
				jcuda.jcudnn.cudnnBatchNormMode.CUDNN_BATCHNORM_SPATIAL, one(), zero(), nCHWDescriptor, imagePtr,
				nCHWDescriptor, retPtr, scaleTensorDesc, scalePtr, biasPtr, exponentialAverageFactor, retRunningMeanPtr,
				retRunningVarPtr, epsilon, resultSaveMeanPtr, resultSaveInvVariancePtr));
	}
	...
	public static void batchNormalizationForwardInference(GPUContext gCtx, String instName, MatrixObject image,
		...
		checkStatus(cudnnBatchNormalizationForwardInference(getCudnnHandle(gCtx),
				jcuda.jcudnn.cudnnBatchNormMode.CUDNN_BATCHNORM_SPATIAL, one(), zero(), nCHWDescriptor, imagePtr,
				nCHWDescriptor, retPtr, scaleTensorDesc, scalePtr, biasPtr, runningMeanPtr, runningVarPtr, epsilon));
	}
	...
	public static void batchNormalizationBackward(GPUContext gCtx, String instName, MatrixObject image,
			MatrixObject dout, MatrixObject scale, MatrixObject dX, MatrixObject dScale, MatrixObject dBias,
			double epsilon, MatrixObject resultSaveMean, MatrixObject resultSaveInvVariance)
		...
		checkStatus(cudnnBatchNormalizationBackward(getCudnnHandle(gCtx),
				jcuda.jcudnn.cudnnBatchNormMode.CUDNN_BATCHNORM_SPATIAL, one(), zero(), one(), zero(), nCHWDescriptor,
				imagePtr, nCHWDescriptor, doutPtr, nCHWDescriptor, dXPtr, scaleTensorDesc, scalePtr, dScalePtr,
				dBiasPtr, epsilon, resultSaveMeanPtr, resultSaveInvVariancePtr));
	}
	...
	private static void validateBatchNormalizationDimensions(MatrixObject scale, MatrixObject bias,
		...
		if(scale.getNumRows() != C || scale.getNumColumns() != 1) {
			throw new DMLRuntimeException(
					"Incorrect dimensions for scale. Expected a column vector of size " + C + ", but found [" +
							scale.getNumRows() + ", " + scale.getNumColumns() + "]");
		}
		if(bias.getNumRows() != C || bias.getNumColumns() != 1) {
			throw new DMLRuntimeException(
					"Incorrect dimensions for bias. Expected a column vector of size " + C + ", but found [" +
							bias.getNumRows() + ", " + bias.getNumColumns() + "]");
		}
		if(runningMean.getNumRows() != C || runningMean.getNumColumns() != 1) {
			throw new DMLRuntimeException(
					"Incorrect dimensions for running mean. Expected a column vector of size " + C + ", but found [" +
							runningMean.getNumRows() + ", " + runningMean.getNumColumns() + "]");
		}
		if(runningVar.getNumRows() != C || runningVar.getNumColumns() != 1) {
			throw new DMLRuntimeException(
					"Incorrect dimensions for running variance. Expected a column vector of size " + C +
							", but found [" + runningVar.getNumRows() + ", " + runningVar.getNumColumns() + "]");
		}
	}
	 ...
	 * @throws DMLRuntimeException if error occurs
	 */
	private static cudnnTensorDescriptor allocateNCHWDescriptors(int N, int C, long CHW) throws DMLRuntimeException {
	...
	protected static Pointer getDensePointerForCuDNN(GPUContext gCtx, MatrixObject image, String instName) {
		...
		if(numElems > maxNumElementsOfCuDNNTensor) {
			throw new DMLRuntimeException(
					"CuDNN restriction: the size of input tensor cannot have greater than 2 giga-elements, but has " +
	 ...
	 */
	public static Pointer getDensePointerForCuDNN(GPUContext gCtx, MatrixObject image, String instName, int numRows,
			int numCols) throws DMLRuntimeException {
		long numElems = image.getNumRows() * image.getNumColumns();
		if(image.getNumRows() != numRows || image.getNumColumns() != numCols) {
			throw new DMLRuntimeException(
					"Expected input of size:[" + numRows + ", " + numCols + "], but found [" + image.getNumRows() +
							", " + image.getNumColumns() + "].");
		}
		...
		if(sizeOfPtr != numElems * sizeOfDataType) {
			throw new DMLRuntimeException(
					"Incorrect pointer: expected size:" + (numElems * sizeOfDataType) + ", but found " + sizeOfPtr);
		}
		return ptr;
	}
	...
	protected static void checkStatus(int status) {
		if(status != cudnnStatus.CUDNN_STATUS_SUCCESS)
			throw new DMLRuntimeException(
					"Error status returned by CuDNN:" + jcuda.jcudnn.cudnnStatus.stringFor(status));
	}
}
