 ...
 * The naming of these methods is consistent with that of CuDNN library.
 */
public class LibMatrixCuDNNConvolutionAlgorithm implements java.lang.AutoCloseable {
	...
	private LibMatrixCuDNNConvolutionAlgorithm(GPUContext gCtx, String instName, int N, int C, int H, int W, int K,
	...
	public static LibMatrixCuDNNConvolutionAlgorithm cudnnGetConvolutionForwardAlgorithm(GPUContext gCtx,
			String instName, int N, int C, int H, int W, int K, int R, int S, int pad_h, int pad_w, int stride_h,
			int stride_w, int P, int Q, long workspaceLimit) {
		...
		jcuda.jcudnn.JCudnn.cudnnGetConvolutionForwardAlgorithmMaxCount(LibMatrixCuDNN.getCudnnHandle(gCtx),
				requestedAlgoCount);
		long[] sizeInBytesArray = {Math.min(workspaceLimit, MAX_WORKSPACE_LIMIT_BYTES)};
		cudnnConvolutionFwdAlgoPerf[] perfResults = new cudnnConvolutionFwdAlgoPerf[requestedAlgoCount[0]];

		jcuda.jcudnn.JCudnn.cudnnFindConvolutionForwardAlgorithm(LibMatrixCuDNN.getCudnnHandle(gCtx),
				ret.nchwTensorDesc, ret.filterDesc, ret.convDesc, ret.nkpqTensorDesc, requestedAlgoCount[0],
				returnedAlgoCount, perfResults);

		jcuda.jcudnn.JCudnn.cudnnGetConvolutionForwardWorkspaceSize(LibMatrixCuDNN.getCudnnHandle(gCtx),
				ret.nchwTensorDesc, ret.filterDesc, ret.convDesc, ret.nkpqTensorDesc, perfResults[0].algo,
				sizeInBytesArray);

		if(sizeInBytesArray[0] != 0)
			ret.workSpace = gCtx.allocate(instName, sizeInBytesArray[0]);
		ret.sizeInBytes = sizeInBytesArray[0];
		ret.algo = perfResults[0].algo;
		return ret;
	}
	...
	public static LibMatrixCuDNNConvolutionAlgorithm cudnnGetConvolutionBackwardFilterAlgorithm(GPUContext gCtx,
		...
		int[] returnedAlgoCount = {-1};
		int[] requestedAlgoCount = {-1};
		jcuda.jcudnn.JCudnn.cudnnGetConvolutionBackwardFilterAlgorithmMaxCount(LibMatrixCuDNN.getCudnnHandle(gCtx),
				requestedAlgoCount);
		long[] sizeInBytesArray = {Math.min(workspaceLimit, MAX_WORKSPACE_LIMIT_BYTES)};

		cudnnConvolutionBwdFilterAlgoPerf[] perfResults = new cudnnConvolutionBwdFilterAlgoPerf[requestedAlgoCount[0]];
		jcuda.jcudnn.JCudnn.cudnnFindConvolutionBackwardFilterAlgorithm(LibMatrixCuDNN.getCudnnHandle(gCtx),
				ret.nchwTensorDesc, ret.nkpqTensorDesc, ret.convDesc, ret.filterDesc, requestedAlgoCount[0],
				returnedAlgoCount, perfResults);

		jcuda.jcudnn.JCudnn.cudnnGetConvolutionBackwardFilterWorkspaceSize(LibMatrixCuDNN.getCudnnHandle(gCtx),
				ret.nchwTensorDesc, ret.nkpqTensorDesc, ret.convDesc, ret.filterDesc, perfResults[0].algo,
				sizeInBytesArray);
	...
	public static LibMatrixCuDNNConvolutionAlgorithm cudnnGetConvolutionBackwardDataAlgorithm(GPUContext gCtx,
		...
		LibMatrixCuDNNConvolutionAlgorithm ret = new LibMatrixCuDNNConvolutionAlgorithm(gCtx, instName, N, C, H, W, K,
				R, S, pad_h, pad_w, stride_h, stride_w, P, Q);
		if(H == R || W == S) {
			// CuDNN's cudnnGetConvolutionBackwardDataAlgorithm returns CUDNN_CONVOLUTION_BWD_DATA_ALGO_1 for at least
			// one scenario for sentence CNN (N=1, C=1, H=2060, W=300, F=500, Hf=5, Wf=300, sparsity=0.1).
			// This causes more than 100x slowdown when compared with CUDNN_CONVOLUTION_BWD_DATA_ALGO_0.
			// To keep things simple for now, we will always prefer to use memory-less operator for conv1d:
			...
			int[] requestedAlgoCount = {-1};
			int[] returnedAlgoCount = {-1};
			jcuda.jcudnn.JCudnn.cudnnGetConvolutionBackwardDataAlgorithmMaxCount(LibMatrixCuDNN.getCudnnHandle(gCtx),
					requestedAlgoCount);
			long[] sizeInBytesArray = {Math.min(workspaceLimit, MAX_WORKSPACE_LIMIT_BYTES)};

			cudnnConvolutionBwdDataAlgoPerf[] perfResults = new cudnnConvolutionBwdDataAlgoPerf[requestedAlgoCount[0]];
			jcuda.jcudnn.JCudnn.cudnnFindConvolutionBackwardDataAlgorithm(LibMatrixCuDNN.getCudnnHandle(gCtx),
					ret.filterDesc, ret.nkpqTensorDesc, ret.convDesc, ret.nchwTensorDesc, requestedAlgoCount[0],
					returnedAlgoCount, perfResults);

			jcuda.jcudnn.JCudnn.cudnnGetConvolutionBackwardDataWorkspaceSize(LibMatrixCuDNN.getCudnnHandle(gCtx),
					ret.filterDesc, ret.nkpqTensorDesc, ret.convDesc, ret.nchwTensorDesc, perfResults[0].algo,
					sizeInBytesArray);
	...
	private static cudnnConvolutionDescriptor allocateConvolutionDescriptor(int[] padding, int[] strides) {
		...
		cudnnSetConvolution2dDescriptor(convDesc, padding[0], padding[1], strides[0], strides[1], 1, 1,
				CUDNN_CROSS_CORRELATION, LibMatrixCUDA.CUDNN_DATA_TYPE);
		return convDesc;
	}
}
