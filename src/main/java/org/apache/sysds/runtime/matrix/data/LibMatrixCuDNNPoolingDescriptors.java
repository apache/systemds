...
public class LibMatrixCuDNNPoolingDescriptors implements java.lang.AutoCloseable {
	 ...
	 */
	public static LibMatrixCuDNNPoolingDescriptors cudnnPoolingBackwardDescriptors(int N, int C, int H, int W, int R,
			int S, int pad_h, int pad_w, int stride_h, int stride_w, int P, int Q, PoolingType poolingType) {
	 ...
	 * @return descriptor wrapper
	 */
	public static LibMatrixCuDNNPoolingDescriptors cudnnPoolingDescriptors(int N, int C, int H, int W, int R, int S,
			int pad_h, int pad_w, int stride_h, int stride_w, int P, int Q, PoolingType poolingType) {
		LibMatrixCuDNNPoolingDescriptors ret = new LibMatrixCuDNNPoolingDescriptors();
		ret.xDesc = allocateTensorDescriptor(N, C, H, W);
		ret.yDesc = allocateTensorDescriptor(N, C, P, Q);
		ret.poolingDesc = allocatePoolingDescriptor(R, S, pad_h, pad_w, stride_h, stride_w, poolingType);
	...
	private static cudnnPoolingDescriptor allocatePoolingDescriptor(int R, int S, int pad_h, int pad_w, int stride_h,
			int stride_w, PoolingType poolingType) {
		cudnnPoolingDescriptor poolingDesc = new cudnnPoolingDescriptor();
		cudnnCreatePoolingDescriptor(poolingDesc);
		int CUDNN_POOLING = (poolingType ==
				PoolingType.MAX) ? CUDNN_POOLING_MAX : CUDNN_POOLING_AVERAGE_COUNT_INCLUDE_PADDING;
		cudnnSetPooling2dDescriptor(poolingDesc, CUDNN_POOLING, CUDNN_PROPAGATE_NAN, R, S, pad_h, pad_w, stride_h,
				stride_w);
		return poolingDesc;
	}
}
