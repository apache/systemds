...
public class LibMatrixCuDNNRnnAlgorithm implements java.lang.AutoCloseable {
	...
	public LibMatrixCuDNNRnnAlgorithm(GPUContext gCtx, String instName, String rnnMode, int N, int T, int M, int D,
			boolean isTraining) throws DMLRuntimeException {
		this.gCtx = gCtx;
		...
		JCudnn.cudnnSetRNNDescriptor_v6(gCtx.getCudnnHandle(), rnnDesc, M, 1, dropoutDesc, CUDNN_LINEAR_INPUT,
				CUDNN_UNIDIRECTIONAL, getCuDNNRnnMode(rnnMode), CUDNN_RNN_ALGO_STANDARD, LibMatrixCUDA.CUDNN_DATA_TYPE);
	...
	@SuppressWarnings("unused") private static int getNumLinearLayers(String rnnMode) throws DMLRuntimeException {
		int ret;
	...
	private static cudnnTensorDescriptor allocateTensorDescriptorWithStride(int firstDim, int secondDim, int thirdDim)
