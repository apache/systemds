...
public class LibMatrixFourierOld {

    /**
     * Function to perform Fast Fourier Transformation
     */

    public static MatrixBlock[] fft_old(MatrixBlock re, MatrixBlock im){
    ...
    public static MatrixBlock[] ifft_old(MatrixBlock re, MatrixBlock im){
        ...
        double[][][] res = fft_old(in, true);

        return convertToMatrixBlocks(res);
    }

    public static double[][][] fft_old(double[][][] in, boolean calcInv){
        ...
        for(int i = 0; i < rows; i++){
            ...
            double[][] res_row = calcInv? ifft_one_dim_old(get_complex_row(in, i)) : fft_one_dim_old(get_complex_row(in, i));
        ...
        for(int j = 0; j < cols; j++){
            // use fft on each col
            double[][] res_col = calcInv? ifft_one_dim_old(get_complex_col(res, j)) : fft_one_dim_old(get_complex_col(res, j));
    ...
    public static double[][] fft_one_dim_old(double[][] in){
        ...
        double[][] res_even = fft_one_dim_old(even);
        double[][] res_odd = fft_one_dim_old(odd);
    ...
    public static double[][] ifft_one_dim_old(double[][] in) {
    ...
    public static MatrixBlock[] fft_old(double[] in){
        double[][][] arr = new double[2][1][in.length];
        arr[0][0] = in;
        return fft_old(convertToMatrixBlocks(arr));
    }

    public static MatrixBlock[] fft_old(double[][][] in){
        return fft_old(convertToMatrixBlocks(in));
    }

    public static MatrixBlock[] fft_old(MatrixBlock[] in){
        return fft_old(in[0], in[1]);
    }

    public static MatrixBlock[] fft_old(MatrixBlock re){
        return fft_old(re, getZeroMatrixBlock(re.getNumRows(), re.getNumColumns()));
    }
}
