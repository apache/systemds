...
public class MatrixBlock extends MatrixValue implements CacheBlock<MatrixBlock>, Externalizable {
	...
	/**
	 * Recompute the number of non-zero values 
	...
	private static SparsityEstimate estimateSparsityOnBinary(MatrixBlock m1, MatrixBlock m2, BinaryOperator op)
	{
		...
		else if(op.fn instanceof Divide && m2.getSparsity() == 1.0){
		...
		if(nz1 <= 0)
			nz1 = m1.recomputeNonZeros();
		if(nz2 <= 0)
			nz2 = m2.recomputeNonZeros();
    ...
    * D = ctable(seq,A,w)
	 ...
	 * @param thatScalar scalar double that is the weight to multiply into the table output
