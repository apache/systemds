...
public class MetaDataAll extends DataIdentifier {
		...
		if(exists && isDir) {
			for(FileStatus stat : HDFSTool.getDirectoryListing(filename)) {
				...
				try(BufferedReader br = new BufferedReader(
					...
					raiseValidateError("for MTD file in directory, error parting part of MTD file with path "
	...
	private void parseMetaDataParam(Object key, Object val) {
		switch(key.toString()) {
			case DataExpression.READROWPARAM:
				_dim1 = val instanceof Long ? (Long) val : (Integer) val;
				break;
			case DataExpression.READCOLPARAM:
				_dim2 = val instanceof Long ? (Long) val : (Integer) val;
				break;
			case DataExpression.ROWBLOCKCOUNTPARAM:
				setBlocksize((Integer) val);
				break;
			case DataExpression.READNNZPARAM:
				setNnz(val instanceof Long ? (Long) val : (Integer) val);
				break;
			case DataExpression.FORMAT_TYPE:
				setFormatTypeString((String) val);
				break;
			case DataExpression.DATATYPEPARAM:
				setDataType(Types.DataType.valueOf(((String) val).toUpperCase()));
				break;
			case DataExpression.VALUETYPEPARAM:
				setValueType(Types.ValueType.fromExternalString((String) val));
				break;
			case DataExpression.DELIM_DELIMITER:
				setDelim(val.toString());
				break;
			case DataExpression.SCHEMAPARAM:
				setSchema(val.toString());
				break;
	...
	public void setFormatTypeString(String format) {
		_formatTypeString = _formatTypeString != null && format == null &&
			_metaObj != null ? (String) JSONHelper.get(_metaObj, DataExpression.FORMAT_TYPE) : format;
	...
	public void setPrivacyConstraints(String privacyConstraints) {
		if(privacyConstraints != null && !privacyConstraints.equals("private") &&
			!privacyConstraints.equals("private-aggregate") && !privacyConstraints.equals("public")) {
	...
	@SuppressWarnings("unchecked")
	public HashMap<String, Expression> parseMetaDataFileParameters(String mtdFileName, boolean conditional,
		...
		for(Object obj : _metaObj.entrySet()) {
			...
			if(varParams.get(key.toString()) != null && (varParams.get(key.toString()) instanceof ConstIdentifier) &&
				...
				raiseValidateError("Parameter '" + key.toString()
					+ "' has conflicting values in metadata and read statement. MTD file value: '" + val.toString()
					+ "'. Read statement value: '" + varParams.get(key.toString()) + "'.", conditional);
			}
			else {
				// if the read method does not specify parameter value, then add MTD metadata file value to parameter
				// list
					...
					if((!key.toString().equalsIgnoreCase(DataExpression.DESCRIPTIONPARAM)) &&
						...
						if(key.toString().equalsIgnoreCase(DataExpression.DELIM_HAS_HEADER_ROW) ||
							...
							if(strId.toString().equalsIgnoreCase("true")) {
								...
								raiseValidateError("Invalid value provided for '" + DataExpression.DELIM_HAS_HEADER_ROW
									+ "' in metadata file '" + mtdFileName + "'. " + "Must be either TRUE or FALSE.",
									conditional);
							}
