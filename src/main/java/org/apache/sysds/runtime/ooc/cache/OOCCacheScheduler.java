...
public interface OOCCacheScheduler {
	...
	/**
	 * Places a new source-backed block in the cache and registers the location with the IO handler.
	 * The entry is treated as backed by disk, so eviction does not schedule spill writes.
	 * @param key the associated key of the block
	 * @param data the block data
	 * @param size the size of the data
