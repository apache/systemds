...
/**
 * Class with static methods merging privacy constraints of operands in expressions to generate the privacy constraints
 * of the output.
 */
public class PrivacyPropagator {
	/**
	 * Parses the privacy constraint of the given metadata object and sets the field of the given Data if the privacy
	...
	/**
	 * Parses the privacy constraint of the given metadata object or returns null if no privacy constraint is set in the
	 * metadata.
	 * 
	 * @param mtd metadata
	...
	/**
	 * Returns the output privacy level based on the given input privacy levels and operator type. It represents the
	 * basic logic of privacy propagation:
	 *
	 * Unary input: Input | NonAggregate | Aggregate ----------------------------------- priv | priv | priv privAgg |
	 * privAgg | none none | none | none
	 *
	 * Binary input: Input | NonAggregate | Aggregate -------------------------------------------- priv-priv | priv |
	 * priv priv-privAgg | priv | priv priv-none | priv | priv privAgg-priv | priv | priv none-priv | priv | priv
	 * privAgg-privAgg | privAgg | none none-none | none | none privAgg-none | privAgg | none none-privAgg | privAgg |
	 * none
	 *
	 * @param inputLevels  privacy levels of the input
	 * @param operatorType type of the operator which is either an aggregation (Aggregate) or not an aggregation
	 *                     (NonAggregate)
	...
	private static PrivacyConstraint mergeNary(PrivacyConstraint[] privacyConstraints, OperatorType operatorType) {
		PrivacyLevel[] privacyLevels = Arrays.stream(privacyConstraints).map(constraint -> {
			if(constraint != null)
	...
	public static PrivacyConstraint mergeBinary(PrivacyConstraint privacyConstraint1,
		...
		if(privacyConstraint1 != null && privacyConstraint2 != null) {
			PrivacyLevel[] privacyLevels = new PrivacyLevel[] {privacyConstraint1.getPrivacyLevel(),
	...
	public static void hopPropagation(Hop hop, ArrayList<Hop> inputHops) {
		PrivacyConstraint[] inputConstraints = inputHops.stream().map(Hop::getPrivacy)
			.toArray(PrivacyConstraint[]::new);
		...
		if(opType == null && Arrays.stream(inputConstraints).anyMatch(Objects::nonNull))
			throw new DMLException("Input has constraint but hop type not recognized by PrivacyPropagator. " + "Hop is "
				+ hop + " " + hop.getClass());
	}

	/**
	 * Get operator type of given hop. Returns null if hop type is not known.
	 * 
	...
	private static OperatorType getOpType(Hop hop) {
		if(hop instanceof TernaryOp || hop instanceof BinaryOp || hop instanceof ReorgOp || hop instanceof DataOp ||
			hop instanceof LiteralOp || hop instanceof NaryOp || hop instanceof DataGenOp ||
			hop instanceof FunctionOp || hop instanceof IndexingOp || hop instanceof ParameterizedBuiltinOp ||
	...
	/**
	 * Propagate privacy constraints to output variables based on privacy constraint of CPOperand output in instruction
	...
	/**
	 * Propagate privacy constraints from input to output CPOperands in case the privacy constraints of the input are
	 * activated.
	 * 
	...
	private static Instruction preprocessVariableCPInstruction(VariableCPInstruction inst, ExecutionContext ec) {
		switch(inst.getVariableOpcode()) {
			...
			case RemoveVariable:
				return mergePrivacyConstraintsFromInput(inst, ec, OperatorType.NonAggregate);
			case Read:
				// Adds scalar object to variable map, hence input (data type and filename)
				// privacy should not be propagated
				return inst;
			default:
				return throwExceptionIfInputOrInstPrivacy(inst, ec);
		}
	...
	/**
	 * Propagates fine-grained constraints if input has fine-grained constraints, otherwise it propagates general
	 * constraints.
	 * 
	 ...
	 */
	private static Instruction preprocessAggregateBinaryCPInstruction(AggregateBinaryCPInstruction inst,
		ExecutionContext ec) {
		...
		if(PrivacyUtils.someConstraintSetBinary(privacyConstraints)) {
			...
			if((privacyConstraints[0] != null && privacyConstraints[0].hasFineGrainedConstraints()) ||
				...
				Propagator propagator = new MatrixMultiplicationPropagatorPrivateFirst(input1, privacyConstraints[0],
	...
	private static Instruction preprocessAppendCPInstruction(AppendCPInstruction inst, ExecutionContext ec) {
		...
		if(PrivacyUtils.someConstraintSetBinary(privacyConstraints)) {
			if(inst.getAppendType() == AppendCPInstruction.AppendType.STRING) {
				...
				PrivacyConstraint outputConstraint = new PrivacyConstraint(
					corePropagation(privacyLevels, OperatorType.NonAggregate));
				...
				if(inst.getOpcode().equals("remove")) {
					...
					PropagatorMultiReturn propagator = new ListRemovePropagator(input1, privacyConstraints[0],
						removePosition, removePosition.getPrivacyConstraint());
					...
					Propagator propagator = new ListAppendPropagator(input1, privacyConstraints[0], input2,
				...
				if(inst.getAppendType() == AppendCPInstruction.AppendType.RBIND)
					...
					throw new DMLPrivacyException("Instruction " + inst.getCPInstructionType() + " with append type "
	...
	/**
	 * Propagates privacy constraints from input to instruction and output CPOperand based on given operator type. The
	 * propagation is done through the core propagation.
	 * 
	...
	private static Instruction mergePrivacyConstraintsFromInput(Instruction inst, ExecutionContext ec,
		...
		return mergePrivacyConstraintsFromInput(inst, ec, getInputOperands(inst), getOutputOperands(inst),
			operatorType);
	}
		...
		if(inputOperands != null) {
			for(CPOperand input : inputOperands) {
				...
				if(privacyConstraint != null && privacyConstraint.hasConstraints()) {
					throw new DMLPrivacyException("Input of instruction " + inst
						+ " has privacy constraints activated, but the constraints are not propagated during preprocessing of instruction.");
				}
			}
		}
		return inst;
	...
	private static void throwExceptionIfPrivacyActivated(Instruction inst) {
		if(inst.getPrivacyConstraint() != null && inst.getPrivacyConstraint().hasConstraints()) {
			throw new DMLPrivacyException("Instruction " + inst
				+ " has privacy constraints activated, but the constraints are not propagated during preprocessing of instruction.");
		}
	}

	/**
	 * Propagate privacy constraint to instruction and output of instruction if data of first input is CacheableData and
	...
	/**
	 * Propagate privacy constraint to instruction and output of instruction if data of second input is CacheableData
	...
	/**
	 * Propagate privacy constraint to instruction and output of instruction if data of the specified variable is
	 * CacheableData and privacy constraint is activated
	 * 
	 ...
	 * @return instruction with or without privacy constraints
	 */
	private static Instruction propagateInputPrivacy(Instruction inst, ExecutionContext ec, CPOperand inputOperand,
		CPOperand outputOperand) {
	...
	/**
	 * Set privacy constraint of data variable with outputName if the variable exists and the privacy constraint is not
	 * null.
	 * 
	 * @param ec                execution context from which the data variable is retrieved
	 ...
	 */
	private static void setOutputPrivacyConstraint(ExecutionContext ec, PrivacyConstraint privacyConstraint,
		String outputName) {
		if(privacyConstraint != null) {
	...
	/**
	 * Returns a list of output CPOperands of instruction or an empty list if the instruction has no outputs. Note that
	 * this method needs to be extended as new instruction types are added, otherwise it will return an empty list for
	 * instructions that may have outputs.
	 * 
