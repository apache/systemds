...
public class ColumnEncoderBagOfWords extends ColumnEncoder {
	...
	public void computeMapSizeEstimate(CacheBlock<?> in, int[] sampleIndices) {
		...
		for (int i = 0; i < max_index; i++) {
			...
			HashSet<String> tokenSetRow = new HashSet<>();
			if(current != null)
	...
	private static void insertionSort(Pair @NotNull [] arr) {
	...
	private static HashMap<String, Integer> countTokenAppearances(CacheBlock<?> in, int r, int c,
		...
		if(current != null)
			for (String token : tokenize(current, caseSensitive, separator))
				if (!token.isEmpty()) {
					Integer old = counter.getOrDefault(token, 0);
					counter.put(token, old + 1);
				}
		return counter;
	}
