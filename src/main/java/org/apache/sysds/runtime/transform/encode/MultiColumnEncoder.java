...
//	}
	...
	private static ArrayList<int[]> getNnzPerRowFromBOWEncoders(CacheBlock<?> input, EncoderMeta encm, int k) {
		...
		try {
			encm.bowEncoders.forEach(e -> e.nnzPerRow = new int[input.getNumRows()]);
			ArrayList<Future<?>> list = new ArrayList<>();
			for (int i = 0; i < num_blks1; i++) {
				int start = i * blk_len1;
				int end = Math.min((i + 1) * blk_len1, input.getNumRows());
				list.add(pool.submit(() -> encm.bowEncoders.stream().parallel().forEach(e -> e.computeNnzPerRow(input, start, end))));
			}
			for(Future<?> f : list)
				f.get();
			list.clear();
			...
			for(ColumnEncoderComposite enc : encm.encs){
				if(enc.hasEncoder(ColumnEncoderBagOfWords.class)){
					previous = previous == null? enc.getEncoder(ColumnEncoderBagOfWords.class).nnzPerRow
			...
			for (int i = 0; i < num_blks2; i++) {
				int start = i * blk_len1;
				list.add(pool.submit(() -> aggregateNnzPerRow(start, blk_len2,  input.getNumRows(),
						encm.encs, bowNnzRowOffsetsFinal)));
			}
			for(Future<?> f : list)
				f.get();
			...
			if (enc.hasEncoder(ColumnEncoderBagOfWords.class)) {
				...
				if(aggRowOffsets == null){
					...
					for (int i = start; i < end; i++) {
						currentOffsets[i] = aggRowOffsets[i] + bow.nnzPerRow[i] - 1;
					}
				}
			}
			pos++;
