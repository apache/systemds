...
public class PorterStemmer
{
   ...
   */
	...
	private final static int calcM(String word)
	{
		int l = word.length() ;
		int count = 0;
	...
	private final static boolean doublec(String word)
   ...
   /* cvc(i) is true <=> i-2,i-1,i has the form consonant - vowel - consonant
      ...
      restore an e at the end of a short word. e.g.
         ...
         snow, box, tray.
*	*/
	private final static boolean cvc(String word)
	{
		int len = word.length();
		int l = len - 1;
		if (len < 3)
	...
	private final static boolean cons(String stem, int i)
	{
		String vowels = "aeiou";
	...
	private final  static String processMatched(String word, HashMap suffixAndfix, int mCount)
	{
		String stemmed = null;
	...
	private final static String replacer(String word, String orig, String replace, int mCount)
	{
		int l = word.length();
   ...
   */
	...
	private final static String step1(String word)
	{
		...
		System.out.println("before y: "+(word.endsWith("y") && vowelinStem(word, "y")));
	...
	private final static String step2(String word) {
		int len = word .length();
	...
	private final static String step3(String word) {
		int len = word .length();
		if (len == 0) return word;
		HashMap<String, String> suffixAndfix = new HashMap<String, String>()
		{{
	...
	private final static String step4(String word)
	{
		// first part.
		...
		if(stemmed == null)
		{
			if(word.length() > 4)
			{
				if(word.charAt(word.length() - 4) == 's' || word.charAt(word.length() - 4) == 't')
				{
	...
	private static final String step5(String word)
	{
	...
	public static String stem (String word)
	{
		if(word.length() >= 3) {
			...
			if(word.length() > 0)
				word = step5(word);
			System.out.println("step 5: "+word);
		}
		return word;
	}
}