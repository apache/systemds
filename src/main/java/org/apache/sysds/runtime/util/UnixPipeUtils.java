...
public class UnixPipeUtils {
    ...
    public static Array<?> readFrameColumnFromPipe(
            BufferedInputStream in, int id, int rows, int totalBytes,
    ...
    private static VarFillTiming readVariableTypeColumn(
        ...
        while (offset < elems) {
            ...
            if (chunk == -1) {
                throw new EOFException("Unexpected end of stream: read "
                        + offset + " of " + elems + " elements");
            }
    ...
    private static VarFillResult fillVariableArrayFromBytes(
        ...
        while (bb.remaining() >= 4 && offsetOut < maxOffset) {
            ...
            switch (type) {
                case STRING -> {
                    // Use the backing array directly if available to avoid CharBuffer allocation
                    int stringStart = bb.position();
                    if (bb.hasArray()) {
                        // Direct access to backing array - most efficient, no intermediate objects
                        ...
                        // Fallback: create a slice and decode (creates CharBuffer, but necessary)
                        ByteBuffer stringSlice = bb.slice();
                        stringSlice.limit(len);
                        String s = StandardCharsets.UTF_8.decode(stringSlice).toString();
                        array.set(offsetOut++, s);
                    }
                    // Advance the position in the original buffer past the string data
    ...
    private static VarWriteTiming writeVariableTypeColumn(
        ...
        byte[] buffer = new byte[batchSize * 2]; // Use 2x batch size like Python side
        int pos = 0;
        ...
        for (int i = 0; i < rows; i++) {
            ...
            if (length > 0) {
                System.arraycopy(encoded, 0, buffer, pos, length);
                pos += length;
            }
        }
