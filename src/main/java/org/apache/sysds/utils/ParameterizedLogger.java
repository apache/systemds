...
public class ParameterizedLogger {
	...
	/**
	 * Substitutes {@code {}} and {@code {%fmt}} placeholders in {@code pattern} with {@code args}.
	 * Non-empty braces that aren't a {@code String.format} spec (e.g. {@code {n}}) are left intact, and any
	 * unconsumed args are appended as {@code  [arg]} — keep call sites strict if accidental drift matters.
	 * Invalid {@code {%fmt}} specs fall back to {@code String.valueOf(arg)} so a log call never throws.
	 */
