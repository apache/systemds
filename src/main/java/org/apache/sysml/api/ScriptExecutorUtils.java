...
public class ScriptExecutorUtils {
	...
	public static void executeRuntimeProgram(Program rtprog, ExecutionContext ec, DMLConfig dmlconf, int statisticsMaxHeavyHitters)
		...
		if(DMLScript.USE_ACCELERATOR) {
			String prevDataType = DMLScript.GPU_DATA_TYPE; 
			DMLScript.GPU_DATA_TYPE = dmlconf.getTextValue(DMLConfig.GPU_DATA_TYPE);
			if(!prevDataType.equalsIgnoreCase(DMLScript.GPU_DATA_TYPE)) {
				if(DMLScript.GPU_DATA_TYPE.equalsIgnoreCase("double")) {
				...
				else if(DMLScript.GPU_DATA_TYPE.equalsIgnoreCase("float")) {
					...
					throw new DMLRuntimeException("Unsupported gpu data type: " + DMLScript.GPU_DATA_TYPE);
				}
			}
		}
