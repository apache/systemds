	...
	private double estimIntern(MatrixHistogram h1, MatrixHistogram h2, OpCode op) {
		...
		case MULT:
			...
			for (int i = 0; i < h1.getRows(); i++) {
				out_r[i] = (h1.rNnz[i] * h2.rNnz[i]
						* IntStream.range(0, h1.getCols()).mapToDouble(k -> h1.cNnz[k] * h2.cNnz[k]).sum())
						/ (Arrays.stream(h1.rNnz).sum() * Arrays.stream(h1.rNnz).sum());
			}
			return Arrays.stream(out_r).sum() / msize;
		case PLUS:
			return Math.min(
