...
public class BuiltinFunctionExpression extends DataIdentifier {
    ...
    public void validateExpression(HashMap<String, DataIdentifier> ids, HashMap<String, ConstIdentifier> constVars, boolean conditional)
        ...
        switch (this.getOpCode()) {
            case EVAL:
                checkNumParameters(Integer.MAX_VALUE);
                output.setDataType(DataType.MATRIX);
                output.setValueType(ValueType.DOUBLE);
                output.setDimensions(1, 1);
                output.setBlockDimensions(1000, 1000);
                break;
            case COLSUM:
            case COLMAX:
            case COLMIN:
    ...
    public static BuiltinFunctionExpression getBuiltinFunctionExpression(ParserRuleContext ctx,
                                                                         ...
                                                                         String filename) throws LanguageException {
        ...
        if (functionName.equals("avg"))
            ...
            //Need to modify the first param "name of the referenced func" into a string identifier
            if (!paramExprsPassed.isEmpty()) {
                DataIdentifier func = (DataIdentifier) paramExprsPassed.get(0).getExpr();
                paramExprsPassed.get(0).setExpr(new StringIdentifier(func.getName(), func));
            } else {
                throw new LanguageException("needs to provide at least 1 argument for eval fucntion");
            }
        } else
            return null;
