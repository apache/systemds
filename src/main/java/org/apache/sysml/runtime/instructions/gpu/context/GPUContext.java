...
public class GPUContext {
  ...
  /** Eviction policies for {@link GPUContext#evict(long)} */
    ...
    public static HashMap<Thread, GPUContext> getAssignedGPUContextsMap() {
        return GPUContextFactory.assignedGPUContextsMap;
    }
  ...
  private JCudaKernels kernels;
	...
	public void ensureComputeCapability() throws DMLRuntimeException {
		...
		for (int i=0; i<devices[0]; i++) {
			cudaDeviceProp properties = GPUContextFactory.getGPUProperties(i);
	...
	public cudaDeviceProp getGPUProperties() {
		return GPUContextFactory.getGPUProperties(deviceNum);
	}
  ...
  public static int getDeviceCount() {
	  if (!GPUContextFactory.initialized) GPUContextFactory.initializeGPU();
	  return GPUContextFactory.deviceCount;
  }
   ...
   */
    public void destroy() throws DMLRuntimeException {
        ...
        synchronized (GPUContext.class) {
            GPUContextFactory.assignedGPUContextsMap.entrySet().removeIf(e -> e.getValue().equals(this));
            GPUContextFactory.freeDevices.add(deviceNum);
            LOG.trace("GPU : removed this from contexts map (size : " + GPUContextFactory.assignedGPUContextsMap.size() + ") and added freed up device back into freeDevices (size : " + GPUContextFactory.freeDevices.size()+ ")");
        }
	}
