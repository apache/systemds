...
public class MapmmChainSPInstruction extends SPInstruction 
{
	...
	{	
		...
		PartitionedBroadcast inV = sec.getBroadcastForVariable( _input2.getName() );
		...
		if( _chainType == ChainType.XtXv ) {
			...
			PartitionedBroadcast inW = sec.getBroadcastForVariable( _input3.getName() );
	...
	private static class RDDMapMMChainFunction implements Function<MatrixBlock, MatrixBlock> 
	{
		...
		public RDDMapMMChainFunction( PartitionedBroadcast bV) 
			throws DMLRuntimeException
		{			
		...
		public MatrixBlock call( MatrixBlock arg0 ) 
			throws Exception 
		{
			MatrixBlock pmV = (MatrixBlock)_pmV.getBlock(1, 1);
	...
	private static class RDDMapMMChainFunction2 implements PairFunction<Tuple2<MatrixIndexes, MatrixBlock>, MatrixIndexes, MatrixBlock> 
	{
		...
		public RDDMapMMChainFunction2( PartitionedBroadcast bV, PartitionedBroadcast bW, ChainType chain) 
			throws DMLRuntimeException
		{			
			...
			throws Exception 
		{
			...
			//execute mapmmchain operation
			blkIn.chainMatrixMultOperations(pmV, (MatrixBlock)_pmW.getBlock(rowIx,1), blkOut, _chainType);
