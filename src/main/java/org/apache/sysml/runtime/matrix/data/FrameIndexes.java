...
/**
 * This represent the indexes to the blocks of the frame.
 * Please note that these indexes are 1-based, whereas the data in the block are zero-based (as they are double arrays).
 */
public class FrameIndexes implements WritableComparable<FrameIndexes>, RawComparator<FrameIndexes>, Externalizable
{	
	...
	public FrameIndexes(long r, long c) {
		setIndexes(r,c);
	}
	
	public FrameIndexes(FrameIndexes indexes) {
	...
	public void setIndexes(FrameIndexes that) {
		_row = that._row;
		_col = that._col;
	}
	
	@Override
	public int compareTo(FrameIndexes other) {
		if( _row != other._row )
	...
	public boolean equals(Object other) {
		if( !(other instanceof FrameIndexes))
			return false;
		
		FrameIndexes tother = (FrameIndexes)other;
	...
	public int compareWithOrder(FrameIndexes other, boolean leftcached) {
		if( !leftcached )
	...
	@Override
	public int compare(FrameIndexes m1, FrameIndexes m2) {
