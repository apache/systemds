...
import org.apache.sysml.runtime.instructions.gpu.context.GPUUtils;
...
public class LibMatrixCUDA {
	...
	/*
	 * static GPUContext gCtx throws DMLRuntimeException { return
	 * GPUContext.gCtx; }
	 */

	/**
	 * Utility function to get maximum number of threads supported by the active
	 * CUDA device.
	 * 
	 * @param gCtx
	...
	/**
	 * Utility function to get maximum number of blocks supported by the active
	 * CUDA device.
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	...
	/**
	 * Convenience method to get a pointer to value '1.0' on device. Instead of
	 * allocating and deallocating it for every kernel invocation.
	 * 
	...
	/**
	 * Convenience method to get a pointer to value '0.0f' on device. Instead of
	 * allocating and deallocating it for every kernel invocation.
	 * 
	 * @return jcuda pointer
	 */
	 ...
	 * @throws DMLRuntimeException
	 *             if the input descriptor and matrix dimensions don't match
	 */
	private static cudnnTensorDescriptor allocateTensorDescriptor(GPUContext gCtx, MatrixObject mat, int N, int C,
			int H, int W) throws DMLRuntimeException {
	...
	/**
	 * Convenience method to get jcudaDenseMatrixPtr. This method explicitly
	 * converts sparse to dense format, so use it judiciously.
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param image
	 *            input matrix object
	 * @param isForCuDNN
	 *            true if the dense pointer is to be used by a CuDNN kernel
	 * @return jcuda pointer
	 * @throws DMLRuntimeException
	 *             if error occurs while sparse to dense conversion
	 */
	private static Pointer getDensePointer(GPUContext gCtx, MatrixObject image, boolean isForCuDNN, String instName)
			throws DMLRuntimeException {
		if (isForCuDNN && image.getNumRows() * image.getNumColumns() > numDoublesIn2GB) {
			throw new DMLRuntimeException(
					"CuDNN restriction: the size of input tensor cannot be greater than 2GB. Hint: try reducing the mini-batch size.");
		}
	 ...
	 *            input matrix object
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @return jcuda pointer
	 * @throws DMLRuntimeException
	 *             if error occurs while sparse to dense conversion
	 */
	public static Pointer getDensePointer(GPUContext gCtx, MatrixObject input, String instName)
			throws DMLRuntimeException {
	 ...
	 * {@link MatrixObject}. Converts dense to sparse if necessary.
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param input
	...
	private static CSRPointer getSparsePointer(GPUContext gCtx, MatrixObject input, String instName)
			throws DMLRuntimeException {
		if (!isInSparseFormat(gCtx, input)) {
	 ...
	 *
	 * @param status
	 *            status returned by CuDNN
	 * @throws DMLRuntimeException
	 *             if status is not CUDNN_STATUS_SUCCESS
	 */
	private static void checkStatus(int status) throws DMLRuntimeException {
		if (status != cudnnStatus.CUDNN_STATUS_SUCCESS)
			throw new DMLRuntimeException("Error status returned by CuDNN:"
	 ...
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param image
	 *            input image matrix object
	 * @param bias
	 *            bias matrix object
	 * @param filter
	 *            filter matrix object
	 * @param output
	 ...
	 *             if error
	 */
	public static void conv2dBiasAdd(GPUContext gCtx, String instName, MatrixObject image, MatrixObject bias,
			MatrixObject filter, MatrixObject output, int N, int C, int H, int W, int K, int R, int S, int pad_h,
			int pad_w, int stride_h, int stride_w, int P, int Q) throws DMLRuntimeException {
		/*
		 * int rows = (int) output.getNumRows(); int cols = (int)
		 * output.getNumColumns(); long size = rows * cols * Sizeof.DOUBLE;
		 * 
		 * Pointer imagePointer = getDensePointer(image, instName); Pointer
		 * biasPointer = getDensePointer(bias, instName); Pointer outputPointer
		 * = getDensePointer(output, instName); Pointer filterPointer =
		 * getDensePointer(filter, instName);
		 * 
		 * Pointer tmp = allocate(size);
		 * 
		 * conv2d(instName, imagePointer, filterPointer, tmp, N, C, H, W, K, R,
		 * S, pad_h, pad_w, stride_h, stride_w, P, Q); cudaDeviceSynchronize();
		 * 
		 * long k1 = bias.getNumColumns(); if(k1 != bias.getNumColumns() ||
		 * bias.getNumColumns() != 1 || cols % k1 != 0) { throw new
		 * DMLRuntimeException("Incorrect inputs for bias_add: input[" + rows +
		 * " X " + cols + "] and bias[" + K + " X " + bias.getNumColumns() +
		 * "]"); } // biasAdd(instName, output, bias, output); biasAdd(instName,
		 * tmp, biasPointer, outputPointer, rows, cols, (int)k1);
		 * 
		 * cudaFreeHelper(tmp);
		 */
	...
	public static void conv2d(GPUContext gCtx, String instName, MatrixObject image, MatrixObject filter,
			MatrixObject outputBlock, int N, int C, int H, int W, int K, int R, int S, int pad_h, int pad_w,
			int stride_h, int stride_w, int P, int Q) throws DMLRuntimeException {
		...
		conv2d(gCtx, instName, imagePointer, filterPointer, dstPointer, N, C, H, W, K, R, S, pad_h, pad_w, stride_h,
				stride_w, P, Q);
	}

	/**
	 * Performs 2D convolution Takes up an insignificant amount of intermediate
	 * space when CONVOLUTION_PREFERENCE is set to
	 * CUDNN_CONVOLUTION_FWD_NO_WORKSPACE Intermediate space is required by the
	 * filter descriptor and convolution descriptor which are metadata
	 * structures and don't scale with the size of the input
	 *
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param image
	 *            the input matrix (or image) allocated on the GPU
	 * @param filter
	 *            the filter allocated on the GPU
	 * @param output
	 *            the output matrix allocated on the GPU
	 * @param N
	 *            number of input images
	 * @param C
	 *            number of channels
	...
	public static void conv2d(GPUContext gCtx, String instName, Pointer image, Pointer filter, Pointer output, int N,
		...
		try {
			...
			// Select the best algorithm depending on the data and supported
			// CUDA
			...
			if (CONVOLUTION_PREFERENCE == cudnnConvolutionFwdPreference.CUDNN_CONVOLUTION_FWD_NO_WORKSPACE) {
				...
				jcuda.jcudnn.JCudnn.cudnnGetConvolutionForwardAlgorithm(getCudnnHandle(gCtx), srcTensorDesc,
						filterDesc, convDesc, dstTensorDesc, CONVOLUTION_PREFERENCE, sizeInBytesArray[0], algos);
				cudnnGetConvolutionForwardWorkspaceSize(getCudnnHandle(gCtx), srcTensorDesc, filterDesc, convDesc,
						dstTensorDesc, algos[0], sizeInBytesArray);
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CUDNN_INIT, System.nanoTime()
						- t1);
			if (GPUStatistics.DISPLAY_STATISTICS)
				t2 = System.nanoTime();
			int status = cudnnConvolutionForward(getCudnnHandle(gCtx), one(), srcTensorDesc, image, filterDesc, filter,
					convDesc, algo, workSpace, sizeInBytes, zero(), dstTensorDesc, output);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CONVOLUTION_FORWARD_LIB,
						System.nanoTime() - t2);
			if (status != cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException("Could not executed cudnnConvolutionForward: "
						+ cudnnStatus.stringFor(status));
			}
		} catch (CudaException e) {
			throw new DMLRuntimeException("Error in conv2d in GPUContext " + gCtx.toString() + " from Thread "
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CUDNN_CLEANUP, System.nanoTime()
						- t3);
		}
	}
	...
	private static cudnnConvolutionDescriptor allocateConvolutionDescriptor(int padding[], int strides[]) {
		...
		cudnnSetConvolution2dDescriptor(convDesc, padding[0], padding[1], strides[0], strides[1], 1, 1,
	 ...
	 */
	private static cudnnPoolingDescriptor allocatePoolingDescriptor(int R, int S, int pad_h, int pad_w, int stride_h,
			int stride_w) {
		cudnnPoolingDescriptor poolingDesc = new cudnnPoolingDescriptor();
		cudnnCreatePoolingDescriptor(poolingDesc);
		cudnnSetPooling2dDescriptor(poolingDesc, CUDNN_POOLING_MAX, CUDNN_PROPAGATE_NAN, R, S, pad_h, pad_w, stride_h,
				stride_w);
		return poolingDesc;
	}

	/**
	 * This method computes the backpropagation errors for previous layer of
	 * relu operation
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 ...
	 *            input image
	 * @param dout
	 *            next layer error propogation
	 * @param outputBlock
	 *            output
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	public static void reluBackward(GPUContext gCtx, String instName, MatrixObject input, MatrixObject dout,
		...
		getCudaKernels(gCtx).launchKernel("relu_backward",
				ExecutionConfig.getConfigForSimpleMatrixOperations((int) rows, (int) cols), imagePointer, doutPointer,
	...
	/**
	 * Performs the operation corresponding to the DML script: ones = matrix(1,
	 * rows=1, cols=Hout*Wout) output = input * matrix(bias %*% ones, rows=1,
	 * cols=F*Hout*Wout) This operation is often followed by conv2d and hence we
	 * have introduced bias_add(input, bias) built-in function
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 ...
	 *             if DMLRuntimeException occurs
	 */
	public static void biasMultiply(GPUContext gCtx, String instName, MatrixObject input, MatrixObject bias,
			MatrixObject outputBlock) throws DMLRuntimeException {
		...
		if (bias.getNumColumns() != 1 || cols % K != 0) {
			throw new DMLRuntimeException("Incorrect inputs for bias_multiply: input[" + rows + " X " + cols
					+ "] and bias[" + K + " X " + bias.getNumColumns() + "]");
		}
		...
		getCudaKernels(gCtx).launchKernel("bias_multiply",
				ExecutionConfig.getConfigForSimpleMatrixOperations((int) rows, (int) cols), imagePointer, biasPointer,
				outputPointer, (int) rows, (int) cols, (int) PQ);
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_RELU_BACKWARD_KERNEL,
					System.nanoTime() - t1);
	 ...
	 *             if DMLRuntimeException occurs
	 */
	public static void biasAdd(GPUContext gCtx, String instName, MatrixObject input, MatrixObject bias,
			MatrixObject outputBlock) throws DMLRuntimeException {
		Pointer imagePointer = getDensePointer(gCtx, input, instName);
		...
		if (bias.getNumColumns() != 1 || cols % K != 0) {
			throw new DMLRuntimeException("Incorrect inputs for bias_add: input[" + rows + " X " + cols + "] and bias["
					+ K + " X " + bias.getNumColumns() + "]");
		}
	 ...
	 * @throws DMLRuntimeException
	 */
	private static void biasAdd(GPUContext gCtx, String instName, Pointer image, Pointer bias, Pointer output,
			int rows, int cols, int k) throws DMLRuntimeException {
		LOG.trace("GPU : biasAdd" + ", GPUContext=" + gCtx);
		int PQ = cols / k;
		long t1 = 0;
		if (GPUStatistics.DISPLAY_STATISTICS)
			t1 = System.nanoTime();
		getCudaKernels(gCtx).launchKernel("bias_add", ExecutionConfig.getConfigForSimpleMatrixOperations(rows, cols),
	...
	private static void validateBatchNormalizationDimensions(MatrixObject scale, MatrixObject bias,
			MatrixObject runningMean, MatrixObject runningVar, int C) throws DMLRuntimeException {
	 ...
	 *            input image
	 * @param scale
	 *            scale (as per CuDNN) and gamma as per original paper: shape
	 *            [1, C, 1, 1]
	 * @param bias
	 *            bias (as per CuDNN) and beta as per original paper: shape [1,
	 *            C, 1, 1]
	 * @param runningMean
	 *            running mean accumulated during training phase: shape [1, C,
	 *            1, 1]
	 * @param runningVar
	 *            running variance accumulated during training phase: shape [1,
	 *            C, 1, 1]
	 * @param ret
	 *            normalized input
	 * @param epsilon
	...
	public static void batchNormalizationForwardInference(GPUContext gCtx, String instName, MatrixObject image,
			MatrixObject scale, MatrixObject bias, MatrixObject runningMean, MatrixObject runningVar, MatrixObject ret,
		...
		// Allocate descriptors
		cudnnTensorDescriptor nCHWDescriptor = allocateNCHWDescriptors(gCtx, N, C, CHW, new MatrixObject[] { image },
				new MatrixObject[] { ret });
		...
		checkStatus(cudnnBatchNormalizationForwardInference(getCudnnHandle(gCtx), mode, one(), zero(), nCHWDescriptor,
				imagePtr, nCHWDescriptor, retPtr, scaleTensorDesc, scalePtr, biasPtr, runningMeanPtr, runningVarPtr,
				epsilon));
	}
	 ...
	 * @param retRunningMean
	 *            (output) running mean accumulated during training phase: shape
	 *            [1, C, 1, 1]
	 * @param retRunningVar
	 *            (output) running variance accumulated during training phase:
	 *            shape [1, C, 1, 1]
	 * @param epsilon
	 *            epsilon value used in the batch normalization formula
	 * @param exponentialAverageFactor
	...
	public static void batchNormalizationForwardTraining(GPUContext gCtx, String instName, MatrixObject image,
			MatrixObject scale, MatrixObject bias, MatrixObject runningMean, MatrixObject runningVar, MatrixObject ret,
			MatrixObject retRunningMean, MatrixObject retRunningVar, double epsilon, double exponentialAverageFactor)
			throws DMLRuntimeException {
		...
		// ignoring resultSaveMean and resultSaveVariance as it requires state
		// management
		checkStatus(cudnnBatchNormalizationForwardTraining(getCudnnHandle(gCtx), mode, one(), zero(), nCHWDescriptor,
				imagePtr, nCHWDescriptor, retPtr, scaleTensorDesc, scalePtr, biasPtr, exponentialAverageFactor,
	 ...
	 * @param output
	 *            output matrix objects
	 * @return one of the NCHW descriptor
	 * @throws DMLRuntimeException
	 *             if error occurs
	 */
	private static cudnnTensorDescriptor allocateNCHWDescriptors(GPUContext gCtx, int N, int C, long CHW,
			MatrixObject[] input, MatrixObject[] output) throws DMLRuntimeException {
		...
		for (int i = 0; i < input.length; i++) {
			...
			if (knownNCHWdescriptor != null) {
				...
				if (shape[0] != N || shape[1] != C) {
					throw new DMLRuntimeException("Incorrect N and C:" + shape[0] + " != " + N + " || " + shape[1]
							+ " != " + C);
				}
				H = shape[2];
				W = shape[3];
	...
	/**
	 * This method computes the backpropagation errors for image, scale and bias
	 ...
	 *            input image
	 * @param dout
	 *            input errors of shape C, H, W
	 * @param scale
	 *            scale (as per CuDNN) and gamma as per original paper: shape
	 *            [1, C, 1, 1]
	 * @param ret
	 *            (output) backpropagation errors for previous layer
	 * @param retScale
	 *            backpropagation error for scale
	 * @param retBias
	 *            backpropagation error for bias
	 * @param epsilon
	 *            epsilon value used in the batch normalization formula
	 * @throws DMLRuntimeException
	 *             if error occurs
	 */
	public static void batchNormalizationBackward(GPUContext gCtx, String instName, MatrixObject image,
			MatrixObject dout, MatrixObject scale, MatrixObject ret, MatrixObject retScale, MatrixObject retBias,
			double epsilon) throws DMLRuntimeException {
		...
		checkStatus(cudnnBatchNormalizationBackward(getCudnnHandle(gCtx), mode, one(), zero(), one(), zero(),
				nCHWDescriptor, imagePtr, nCHWDescriptor, doutPtr, nCHWDescriptor, retPtr, scaleTensorDesc, scalePtr,
				retScalePtr, retBiasPtr, epsilon, new Pointer(), new Pointer()));
	}

	/**
	 * This method computes the backpropogation errors for filter of convolution
	 * operation
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	...
	public static void conv2dBackwardFilter(GPUContext gCtx, String instName, MatrixObject image, MatrixObject dout,
		...
		try {
			...
			// TODO: Select the best algorithm depending on the data and
			// supported CUDA
			int algo = jcuda.jcudnn.cudnnConvolutionBwdFilterAlgo.CUDNN_CONVOLUTION_BWD_FILTER_ALGO_0;

			workSpace = new Pointer();
			cudnnGetConvolutionBackwardFilterWorkspaceSize(getCudnnHandle(gCtx), xTensorDesc, doutTensorDesc, convDesc,
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CONVOLUTION_BACKWARD_FILTER_LIB,
						System.nanoTime() - t2);

			if (status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException("Could not executed cudnnConvolutionBackwardFilter: "
						+ jcuda.jcudnn.cudnnStatus.stringFor(status));
			}
		} catch (CudaException e) {
			throw new DMLRuntimeException("Error in conv2d in GPUContext " + gCtx.toString() + " from Thread "
	...
	/**
	 * This method computes the backpropogation errors for previous layer of
	...
	public static void conv2dBackwardData(GPUContext gCtx, String instName, MatrixObject filter, MatrixObject dout,
			MatrixObject output, int N, int C, int H, int W, int K, int R, int S, int pad_h, int pad_w, int stride_h,
			int stride_w, int P, int Q) throws DMLRuntimeException {
		...
		try {
			...
			workSpace = new Pointer();
			cudnnGetConvolutionBackwardDataWorkspaceSize(getCudnnHandle(gCtx), wDesc, dyDesc, convDesc, dxDesc, algo,
					sizeInBytesArray);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CUDNN_INIT, System.nanoTime()
						- t1);
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t2 = System.nanoTime();
			int status = cudnnConvolutionBackwardData(getCudnnHandle(gCtx), one(), wDesc, w, dyDesc, dy, convDesc,
					algo, workSpace, sizeInBytes, zero(), dxDesc, dx);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CONVOLUTION_BACKWARD_DATA_LIB,
						System.nanoTime() - t2);

			if (status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException("Could not executed cudnnConvolutionBackwardData: "
						+ jcuda.jcudnn.cudnnStatus.stringFor(status));
			}
		} catch (CudaException e) {
			throw new DMLRuntimeException("Error in conv2d in GPUContext " + gCtx.toString() + " from Thread "
	 ...
	 *            vertical stride
	 * @param P
	 *            (H - R + 1 + 2*pad_h)/stride_h
	 * @param Q
	 *            (W - S + 1 + 2*pad_w)/stride_w
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	public static void maxpooling(GPUContext gCtx, String instName, MatrixObject image, MatrixObject outputBlock,
			int N, int C, int H, int W, int K, int R, int S, int pad_h, int pad_w, int stride_h, int stride_w, int P,
		...
		performMaxpooling(gCtx, instName, x, xDesc, outputBlock, N, C, H, W, K, R, S, pad_h, pad_w, stride_h, stride_w,
				P, Q);
	}
	...
	public static void performMaxpooling(GPUContext gCtx, String instName, Pointer x, cudnnTensorDescriptor xDesc,
		...
		try {
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_MAXPOOLING_FORWARD_LIB,
						System.nanoTime() - t2);

			if (status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException("Could not executed cudnnPoolingForward: "
						+ jcuda.jcudnn.cudnnStatus.stringFor(status));
			}
		} catch (CudaException e) {
			throw new DMLRuntimeException("Error in conv2d in GPUContext " + gCtx.toString() + " from Thread "
	 ...
	 * cudnnPoolingBackward(...) This method computes the backpropogation errors
	 * for previous layer of maxpooling operation
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 ...
	 *            image as matrix object
	 * @param dout
	 *            delta matrix, output of previous layer
	 * @param outputBlock
	 *            output matrix
	 * @param N
	 *            batch size
	...
	public static void maxpoolingBackward(GPUContext gCtx, String instName, MatrixObject image, MatrixObject dout,
		...
		try {
			...
			// Calling PoolForward first, y is one of the inputs for
			// poolBackward
			// TODO: Remove calling poolForward after necessary changes at
			// language level for poolBackward
			...
			if (status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException("Could not executed cudnnPoolingForward before cudnnPoolingBackward: "
						+ jcuda.jcudnn.cudnnStatus.stringFor(status));
			}

			if (GPUStatistics.DISPLAY_STATISTICS)
				t3 = System.nanoTime();
			status = cudnnPoolingBackward(getCudnnHandle(gCtx), poolingDesc, one(), yDesc, y, dyDesc, dy, xDesc, x,
					zero(), dxDesc, dx);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_MAXPOOLING_BACKWARD_LIB,
						System.nanoTime() - t3);

			if (status != jcuda.jcudnn.cudnnStatus.CUDNN_STATUS_SUCCESS) {
				throw new DMLRuntimeException("Could not executed cudnnPoolingBackward: "
						+ jcuda.jcudnn.cudnnStatus.stringFor(status));
			}
		} catch (CudaException e) {
			throw new DMLRuntimeException("Error in conv2d in GPUContext " + gCtx.toString() + " from Thread "
	...
	private static void performCuDNNReLU(GPUContext gCtx, String instName, MatrixObject in, Pointer dstData,
			cudnnTensorDescriptor srcTensorDesc) throws DMLRuntimeException {
		long t0 = 0;
		try {
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t0 = System.nanoTime();
			cudnnActivationForward(getCudnnHandle(gCtx), activationDescriptor, one(), srcTensorDesc, srcData, zero(),
					dstTensorDesc, dstData);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_ACTIVATION_FORWARD_LIB,
						System.nanoTime() - t0);
		} catch (CudaException e) {
			throw new DMLRuntimeException("Error in conv2d in GPUContext " + gCtx.toString() + " from Thread "
					+ Thread.currentThread().toString(), e);
	 ...
	 * 
	 * @param ec
	 *            currently active {@link ExecutionContext}
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	...
	public static void relu(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in, String outputName)
		...
		getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, in.getNumRows(), in.getNumColumns()); // Allocated
																												// the
																												// dense
																												// output
																												// matrix
		...
		if (N * CHW >= numDoublesIn2GB || srcTensorDesc == null) {
			...
			Pointer srcData = getDensePointer(gCtx, in, instName); // TODO:
																	// FIXME:
																	// Add
																	// sparse
																	// kernel
			...
			getCudaKernels(gCtx).launchKernel("relu",
					ExecutionConfig.getConfigForSimpleMatrixOperations((int) N, (int) CHW), srcData, dstData, (int) N,
					(int) CHW);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_RELU_KERNEL, System.nanoTime()
						- t0);
		} else {
	 ...
	 * cublasDsyrk(...)
	 * <p>
	 * Memory Usage - If dense, input space - rows * cols, no intermediate
	 * memory, output - Max(rows*rows, cols*cols) If sparse, calls matmult
	 *
	 * @param ec
	 *            execution context
	 * @param gCtx
	 ...
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param left
	 *            input matrix, as in a tsmm expression like A %*% A' or A' %*%
	 *            A, we just need to check whether the left one is transposed or
	 *            not, I named it 'left'
	 * @param outputName
	 *            output matrix name
	 ...
	 *             if DMLRuntimeException occurs
	 */
	public static void matmultTSMM(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject left,
			String outputName, boolean isLeftTransposed) throws DMLRuntimeException {
		...
		// reverse the order of matrix-multiplication and take care of dimension
		// mismatch.
		...
		// For dense TSMM, exploit cublasDsyrk(...) and call custom kernel to
		// flip the matrix
		MatrixObject output = getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, m, m); // Allocated
																										// the
																										// dense
																										// output
																										// matrix
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		JCublas2.cublasDsyrk(getCublasHandle(gCtx), cublasFillMode.CUBLAS_FILL_MODE_LOWER, transa, m, k, one(), A, lda,
				zero(), C, ldc);
		if (GPUStatistics.DISPLAY_STATISTICS)
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_UPPER_TO_LOWER_TRIANGLE_KERNEL,
					System.nanoTime() - t1);
	}
	...
	private static void copyUpperToLowerTriangle(GPUContext gCtx, String instName, MatrixObject ret)
		...
		getCudaKernels(gCtx).launchKernel("copy_u2l_dense",
				ExecutionConfig.getConfigForSimpleMatrixOperations(dim, dim), getDensePointer(gCtx, ret, instName),
				dim, dim * dim);
	}
	 ...
	 * <p>
	 * Memory Requirements - Both dense - inputs, output, no intermediate Both
	 * sparse - inputs, output, no intermediate One sparse, one dense - inputs,
	 * output, intermediates - (input_dim1 * input_dim2) OR (input_dim1 *
	 * input_dim2 + input in sparse format)
	 *
	 * @param ec
	...
	public static MatrixObject matmult(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject left,
		...
		if (!left.getGPUObject(gCtx).isAllocated() || !right.getGPUObject(gCtx).isAllocated())
			throw new DMLRuntimeException("One of input is not allocated:" + left.getGPUObject(gCtx).isAllocated()
		...
		if (bothDense) { // Dense C = Dense A * Dense B
			...
			getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, outRLen, outCLen); // Allocated
																								// the
																								// dense
																								// output
																								// matrix
		...
		} else { // Either of A or B is sparse, Sparse C = Sparse/Dense A *
					// Dense/Sparse B
	 ...
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param output
	 *            allocated output object for C on host to which GPU output will
	 *            be attached
	 * @param left
	 *            Matrix A on host
	 * @param right
	 *            Matrix B on host
	 * @param isLeftTransposed
	 *            op for A, tranposed or not
	 * @param isRightTransposed
	 *            op for B, transposed or not
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	private static void eitherSparseMatmult(GPUContext gCtx, String instName, MatrixObject output, MatrixObject left,
			MatrixObject right, boolean isLeftTransposed, boolean isRightTransposed) throws DMLRuntimeException {
	...
	/**
	 * C = op(A) * op(B) where A is dense and B is sparse If B is ultrasparse, A
	 * is converted to a sparse matrix and
	 * {@code sparseSparseMatmult(MatrixObject, int, int, int, int, int, CSRPointer, CSRPointer)}
	 * is invoked otherwise B is converted to a dense matrix and
	 * {@code denseDenseMatmult(Pointer, int, int, int, int, boolean, boolean, Pointer, Pointer)}
	 * is invoked.
	 *
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 ...
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param left
	 *            {@link MatrixObject} of A
	 * @param right
	 *            {@link MatrixObject} of B
	 * @param output
	 *            {@link MatrixObject} of the output matrix C
	 * @param isLeftTransposed
	 *            whether matrix A needs to be transposed
	 * @param isRightTransposed
	 ...
	 *             if DMLRuntimeException occurs
	 */
	private static void denseSparseMatmult(GPUContext gCtx, String instName, MatrixObject left, MatrixObject right,
			MatrixObject output, boolean isLeftTransposed, boolean isRightTransposed, int m, int n, int k)
			throws DMLRuntimeException {
		...
		if (B.isUltraSparse(k, n)) {
			...
			if (DMLScript.STATISTICS)
				t0 = System.nanoTime();
			Pointer AT = GPUUtils.transpose(gCtx, ADense, rowsA, colsA, colsA, rowsA);
			if (GPUStatistics.DISPLAY_STATISTICS)
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CUDA_FREE,
						System.nanoTime() - t2, 2);
			...
			if (DMLScript.STATISTICS)
				t0 = System.nanoTime();
			Pointer BDenseTransposed = B.toColumnMajorDenseMatrix(getCusparseHandle(gCtx), getCublasHandle(gCtx),
					(int) right.getNumRows(), (int) right.getNumColumns());
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_SPARSE_TO_DENSE,
						System.nanoTime() - t0);
			if (DMLScript.STATISTICS)
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t1 = System.nanoTime();
			boolean allocated = output.getGPUObject(gCtx).acquireDeviceModifyDense(); // To
																						// allocate
																						// the
																						// dense
																						// matrix
			if (GPUStatistics.DISPLAY_STATISTICS && allocated)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_ALLOCATE_DENSE_OUTPUT,
						System.nanoTime() - t1);
			Pointer C = getDensePointer(gCtx, output, instName);
			denseDenseMatmult(gCtx, instName, C, (int) left.getNumRows(), (int) left.getNumColumns(),
					(int) right.getNumColumns(), (int) right.getNumRows(), isLeftTransposed, !isRightTransposed,
	...
	/**
	 * * C = op(A) * op(B) where A is sparse and B is dense If A is ultrasparse,
	 * B is converted to a sparse matrix and
	 * {@code sparseSparseMatmult(MatrixObject, int, int, int, int, int, CSRPointer, CSRPointer)}
	 * is invoked otherwise A is converted to a dense matrix and
	 * {@code denseDenseMatmult(Pointer, int, int, int, int, boolean, boolean, Pointer, Pointer)}
	 * is invoked.
	 *
	 * @param gCtx
	 ...
	 *             if DMLRuntimeException occurs
	 */
	private static void sparseDenseMatmult(GPUContext gCtx, String instName, MatrixObject output, MatrixObject left,
			MatrixObject right, boolean isLeftTransposed, boolean isRightTransposed, int m, int n, int k)
			throws DMLRuntimeException {
		...
		if (n == 1) {
			...
			sparseMatrixDenseVectorMult(gCtx, instName, output, A, BDense, isLeftTransposed, (int) left.getNumRows(),
			...
			if (A.isUltraSparse(m, k)) {
				...
				if (DMLScript.STATISTICS)
					t0 = System.nanoTime();
				Pointer BT = GPUUtils.transpose(gCtx, BDense, rowsB, colsB, colsB, rowsB);
				if (GPUStatistics.DISPLAY_STATISTICS)
					GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_TRANSPOSE_LIB,
							System.nanoTime() - t0);

				if (GPUStatistics.DISPLAY_STATISTICS)
					t1 = System.nanoTime();
				CSRPointer B = GPUObject.columnMajorDenseToRowMajorSparse(gCtx, getCusparseHandle(gCtx), BT, rowsB,
						colsB);
				if (GPUStatistics.DISPLAY_STATISTICS)
					GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DENSE_TO_SPARSE,
							System.nanoTime() - t1);
				...
				if (GPUStatistics.DISPLAY_STATISTICS)
					GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CUDA_FREE, System.nanoTime()
							- t2, 2);
				...
				// Note the arguments to denseDenseMatmult to accommodate for
				// this.
				if (DMLScript.STATISTICS)
					t0 = System.nanoTime();
				Pointer ADenseTransposed = A.toColumnMajorDenseMatrix(getCusparseHandle(gCtx), getCublasHandle(gCtx),
				...
				Pointer C = getDensePointer(gCtx, output, instName);
				denseDenseMatmult(gCtx, instName, C, (int) left.getNumColumns(), (int) left.getNumRows(),
	 ...
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param output
	 *            allocated output on the host, to which the GPU output C will
	 *            be attached
	 * @param A
	 *            sparse matrix A on the GPU
	 * @param B_dense
	 *            dense matrix/vector B on the GPU
	 * @param isATranposed
	 *            op for A, tranposed or not
	 * @param m
	 *            number of rows in A (not op(A))
	 * @param k
	 *            number of cols in A or number of rows in B (not op(A) or
	 *            op(B))
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	private static void sparseMatrixDenseVectorMult(GPUContext gCtx, String instName, MatrixObject output,
			CSRPointer A, Pointer B_dense, boolean isATranposed, int m, int k) throws DMLRuntimeException {
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			t1 = System.nanoTime();
		cusparseDcsrmv(getCusparseHandle(gCtx), transA, m, k, (int) A.nnz, one(), A.descr, A.val, A.rowPtr, A.colInd,
				B_dense, zero(), C_dense);
		// cudaDeviceSynchronize; // Since cusparseDcsrmv is asynchronously
		// executed
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_SPARSE_MATRIX_DENSE_VECTOR_LIB,
					System.nanoTime() - t1);
	...
	/**
	 * Sparse C = Sparse op(A) * Sparse op(B) Reroutes call to sparse
	 * matrix-vector mult if needed
	 *
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 ...
	 *             if DMLRuntimeException occurs
	 */
	private static void bothSparseMatmult(GPUContext gCtx, String instName, MatrixObject output, MatrixObject left,
			MatrixObject right, boolean isLeftTransposed, boolean isRightTransposed) throws DMLRuntimeException {
		int m = (int) (isLeftTransposed ? left.getNumColumns() : left.getNumRows());
		int n = (int) (isRightTransposed ? right.getNumRows() : right.getNumColumns());
		int k = (int) (isLeftTransposed ? left.getNumRows() : left.getNumColumns());
		...
		if (!isRightTransposed && right.getNumColumns() == 1) { // Matrix-Vector
																// multiplication
			sparseMatrixVectorMult(gCtx, instName, output, isLeftTransposed, (int) left.getNumRows(),
					(int) left.getNumColumns(), (int) right.getNumRows(), A, B);
	...
	/**
	 * Does a sparse matrix-vector multiply. C = op(A) x B, A is a sparse
	 * matrix, B is a sparse vector with numCols = 1.
	 *
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param output
	 *            allocated output object C to which the GPU output matrix will
	 *            be attached
	 * @param isATranposed
	 *            if A is to be transposed or not (the op in op(A))
	 * @param m
	 *            number of rows in A (not op(A))
	 * @param n
	 *            number of cols in A (not op(A))
	 * @param k
	 *            number of rows in B, (cols in B is assumed to be 1)
	 * @param A
	 *            left sparse matrix on GPU
	 * @param B
	 *            right sparse vector on GPU
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	private static void sparseMatrixVectorMult(GPUContext gCtx, String instName, MatrixObject output,
			boolean isATranposed, int m, int n, int k, CSRPointer A, CSRPointer B) throws DMLRuntimeException {
		long t0 = 0;
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		Pointer BDenseVector = B.toColumnMajorDenseMatrix(getCusparseHandle(gCtx), getCublasHandle(gCtx), k, 1);
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_SPARSE_TO_DENSE, System.nanoTime()
					- t0);
		sparseMatrixDenseVectorMult(gCtx, instName, output, A, BDenseVector, isATranposed, m, k);
	}

	/**
	 * Does a sparse-sparse Matrix multiply C = op(A) x op(B), A, B are sparse
	 * matrices
	 *
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 ...
	 *            left sparse matrix on GPU
	 * @param B
	 *            right sparse matrix on GPU
	 * @param output
	 *            allocated output object on host to which the GPU output matrix
	 *            will be attached
	 * @param isLeftTransposed
	 *            op for A - to be transposed or not
	 * @param isRightTransposed
	 *            op for B
	 * @param m
	 *            number of rows in op(A)
	 * @param n
	 *            number of cols in op(B)
	 * @param k
	 *            number of cols in op(A) or rows in op(B)
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	private static void sparseSparseMatmult(GPUContext gCtx, String instName, CSRPointer A, CSRPointer B,
			MatrixObject output, boolean isLeftTransposed, boolean isRightTransposed, int m, int n, int k)
			throws DMLRuntimeException {
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		CSRPointer C = CSRPointer.allocateForMatrixMultiply(gCtx, getCusparseHandle(gCtx), A, transA, B, transB, m, n,
				k);
		if (GPUStatistics.DISPLAY_STATISTICS)
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			t1 = System.nanoTime();
		cusparseDcsrgemm(getCusparseHandle(gCtx), transA, transB, m, n, k, A.descr, (int) A.nnz, A.val, A.rowPtr,
				A.colInd, B.descr, (int) B.nnz, B.val, B.rowPtr, B.colInd, C.descr, C.val, C.rowPtr, C.colInd);
		// cudaDeviceSynchronize;
		if (GPUStatistics.DISPLAY_STATISTICS)
	 ...
	 *            name of the invoking instruction to record{@link Statistics}.
	 * @param output
	 *            output object C on host with GPU data allocated
	 * @param left
	 *            left matrix A (in row-major order)
	 * @param right
	 *            right matrix B (in row-major order)
	 * @param isLeftTransposed
	 *            op for A, transposed or not
	 * @param isRightTransposed
	 *            op for B, transposed or not
	 ...
	 *             if DMLRuntimeException occurs
	 */
	private static void denseDenseMatmult(GPUContext gCtx, String instName, MatrixObject output, MatrixObject left,
			MatrixObject right, boolean isLeftTransposed, boolean isRightTransposed) throws DMLRuntimeException {
		...
		Pointer C = getDensePointer(gCtx, output, instName);
		denseDenseMatmult(gCtx, instName, C, leftRows, leftCols, rightRows, rightCols, isLeftTransposed,
				isRightTransposed, leftPtr, rightPtr);
	}

	/**
	 * Dense-dense matrix multiply C = op(A) * op(B), A and B are dense matrices
	 * On the host, the matrices are in row-major format; cuBLAS expects them in
	 * column-major format. What we have as input is t(A) and t(B), t(X) =
	 * transpose of X. We do t(B) %*% t(A) to get t(C); If we were to calculate
	 * t(t(C), we would get the resultant matrix C, but this would be in
	 * column-major format. What we really want is t(C). This we already have as
	 * the result of t(B) %*% t(A).
	 *
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 *            name of the invoking instruction to record{@link Statistics}.
	 * @param output
	 *            output allocated on GPU in column major format
	 * @param leftRows1
	 *            number of rows in A
	 * @param leftCols1
	 *            number of cols in A
	 ...
	 * @param rightCols1
	 *            number of cols in B
	 * @param isLeftTransposed1
	 *            op for A, transposed or not
	 * @param isRightTransposed1
	 *            op for B, transposed or not
	 * @param leftPtr
	 *            A allocated on the GPU in row-major format
	 * @param rightPtr
	 *            B allocated on the GPU in row-major format
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	public static void denseDenseMatmult(GPUContext gCtx, String instName, Pointer output, int leftRows1,
			int leftCols1, int rightRows1, int rightCols1, boolean isLeftTransposed1, boolean isRightTransposed1,
			Pointer leftPtr, Pointer rightPtr) throws DMLRuntimeException {
		...
		if (m == 1 && n == 1) {
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DENSE_DOT_LIB, System.nanoTime()
						- t0);
		} else if (m == 1) {
			...
			JCublas2.cublasDgemv(getCublasHandle(gCtx), transb, rightRows, rightCols, Pointer.to(one), B, ldb, A, 1,
					Pointer.to(zero), C, 1);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DENSE_VECTOR_DENSE_MATRIX_LIB,
						System.nanoTime() - t0);
		} else if (n == 1) {
			// Matrix-vector multiply
			LOG.debug(" GPU Dense Matrix-Vector Multiply");
			JCublas2.cublasDgemv(getCublasHandle(gCtx), transa, leftRows, leftCols, Pointer.to(one), A, lda, B, 1,
					Pointer.to(zero), C, 1);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DENSE_MATRIX_DENSE_VECTOR_LIB,
						System.nanoTime() - t0);
		} else {
			LOG.debug(" GPU Dense-Dense Matrix Multiply ");
			JCublas2.cublasDgemm(getCublasHandle(gCtx), transa, transb, m, n, k, Pointer.to(one), A, lda, B, ldb,
					Pointer.to(zero), C, ldc);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DENSE_MATRIX_DENSE_MATRIX_LIB,
						System.nanoTime() - t0);
		}
	}
	...
	/**
	 * Entry point to perform Unary aggregate operations on the GPU. The
	 * execution context object is used to allocate memory for the GPU.
	 *
	 * @param ec
	 *            Instance of {@link ExecutionContext}, from which the output
	 ...
	 *            input matrix
	 * @param output
	 *            output matrix/scalar name
	 * @param op
	 *            Instance of {@link AggregateUnaryOperator} which encapsulates
	 *            the direction of reduction/aggregation and the reduction
	 *            operation.
	 * @throws DMLRuntimeException
	 *             if {@link DMLRuntimeException} occurs
	 */
	public static void unaryAggregate(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1,
			String output, AggregateUnaryOperator op) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		// A kahan sum implemention is not provided. is a "uak+" or other kahan
		...
		if (!in1.getGPUObject(gCtx).isAllocated())
			throw new DMLRuntimeException("Internal Error - The input is not allocated for a GPU Aggregate Unary:"
					+ in1.getGPUObject(gCtx).isAllocated());
		...
		if (indexFn instanceof ReduceAll) {
			...
			throw new DMLRuntimeException(
					"Internal Error - Invalid index function type, only reducing along rows, columns, diagonals or all elements is supported in Aggregate Unary operations");
		}
		...
		if (aggOp.increOp.fn instanceof KahanPlus) {
			...
			case MININDEX:
				...
				break;
			default:
				new DMLRuntimeException(
						"Internal Error - Unsupported Builtin Function for Aggregate unary being done on GPU");
			}
		} else {
		...
		if (isSparse) {
			...
			// long nnz = in1.getNnz();
			// assert nnz > 0 : "Internal Error - number of non zeroes set to "
			// + nnz + " in Aggregate Binary for GPU";
			// MatrixObject out =
			// ec.getSparseMatrixOutputForGPUInstruction(output, nnz);
			// throw new
			// DMLRuntimeException("Internal Error - Not implemented");
		...
		case OP_PLUS: {
			...
			case REDUCTION_COL: { // The names are a bit misleading,
									// REDUCTION_COL refers to the direction
									// (reduce all elements in a column)
		...
		case OP_VARIANCE: {
			...
			case REDUCTION_COL: {
				...
				matrixMatrixOp(gCtx, instName, in, out, rlen, clen, VectorShape.NONE.code(), VectorShape.COLUMN.code(),
						tmp, minusOp);
			...
			case REDUCTION_ROW: {
				...
				// matrix
				BinaryOperator minusOp = new BinaryOperator(Minus.getMinusFnObject());
				matrixMatrixOp(gCtx, instName, in, out, rlen, clen, VectorShape.NONE.code(), VectorShape.ROW.code(),
						tmp, minusOp);
	 ...
	 * @param in
	 *            {@link Pointer} to matrix in device memory
	 * @param n
	 *            size of array
	...
	private static double reduceAll(GPUContext gCtx, String instName, String kernelFunction, Pointer in, int n)
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			t1 = System.nanoTime();
		getCudaKernels(gCtx).launchKernel(kernelFunction, new ExecutionConfig(blocks, threads, sharedMem), in, tempOut,
				n);
		// cudaDeviceSynchronize;
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_REDUCE_ALL_KERNEL, System.nanoTime()
					- t1);
		...
		while (s > 1) {
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t2 = System.nanoTime();
			getCudaKernels(gCtx).launchKernel(kernelFunction, new ExecutionConfig(blocks, threads, sharedMem), tempOut,
					tempOut, s);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_REDUCE_ALL_KERNEL,
						System.nanoTime() - t2);
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DEVICE_TO_HOST, System.nanoTime()
					- t3);
	...
	/**
	 * Do a reduction by row. Data is reduced per row and the resulting vector
	 * is calculated.
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 ...
	 *            name of the kernel function to invoke
	 * @param in
	 *            {@link Pointer} to input matrix in device memory (size - rows
	 *            * columns)
	 * @param out
	 *            {@link Pointer} to output matrix in device memory (size - rows
	 *            * 1)
	 * @param rows
	 ...
	 *             if DMLRuntimeException occurs
	 */
	private static void reduceRow(GPUContext gCtx, String instName, String kernelFunction, Pointer in, Pointer out,
			int rows, int cols) throws DMLRuntimeException {
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		getCudaKernels(gCtx).launchKernel(kernelFunction, new ExecutionConfig(blocks, threads, sharedMem), in, out,
				rows, cols);
		// cudaDeviceSynchronize;
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_REDUCE_ROW_KERNEL, System.nanoTime()
					- t0);

	}

	/**
	 * Do a reduction by column. Data is reduced per column and the resulting
	 ...
	 *            * columns)
	 * @param out
	 *            {@link Pointer} to output matrix in device memory (size - 1 *
	 *            cols)
	 * @param rows
	 *            number of rows in input matrix
	 * @param cols
	 ...
	 *             if DMLRuntimeException occurs
	 */
	private static void reduceCol(GPUContext gCtx, String instName, String kernelFunction, Pointer in, Pointer out,
			int rows, int cols) throws DMLRuntimeException {
		...
		// cudaDeviceSynchronize;
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_REDUCE_COL_KERNEL, System.nanoTime()
					- t0);
	}
	...
	/**
	 * Entry point to perform elementwise matrix-scalar relational operation
	 * specified by op
	 *
	 * @param ec
	 *            execution context
	 ...
	 *             if DMLRuntimeException occurs
	 */
	public static void matrixScalarRelational(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in,
			String outputName, ScalarOperator op) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		if (isSparseAndEmpty(gCtx, in)) {
			...
			MatrixObject out = getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, in.getNumRows(),
					in.getNumColumns()); // Allocated the dense output matrix
	...
	/**
	 * Entry point to perform elementwise matrix-scalar arithmetic operation
	 * specified by op
	 *
	 * @param ec
	 *            execution context
	 ...
	 *            true if input transposed
	 * @param op
	 *            scalar operator
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	public static void matrixScalarArithmetic(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in,
			String outputName, boolean isInputTransposed, ScalarOperator op) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		double constant = op.getConstant();
		...
		// || (op.fn instanceof Divide && op instanceof RightScalarOperator &&
		// constant != 0)) && !isSparseAndEmpty(gCtx, in);
		...
		if (constant == 0) {
			...
			// to be re-enabled and the Java computation logic for divide by
			// zero
			...
			// //For division, IEEE 754 defines x/0.0 as INFINITY and 0.0/0.0 as
			// NaN.
			// compareAndSet(ec, gCtx, instName, in, outputName, 0.0, 1e-6,
		...
		// TODO: Performance optimization: Call cublasDaxpy if(in.getNumRows()
		// == 1 || in.getNumColumns() == 1)
		// C = alpha* op( A ) + beta* op ( B )
		// dgeam(ec, gCtx, instName, in, in, outputName, isInputTransposed,
		// isInputTransposed, alpha, 0.0);
		// }
	}

	/**
	 * Performs elementwise operation relational specified by op of two input
	 * matrices in1 and in2
	 *
	 * @param ec
	 *            execution context
	 * @param gCtx
	 ...
	 *             if DMLRuntimeException occurs
	 */
	public static void matrixMatrixRelational(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1,
			MatrixObject in2, String outputName, BinaryOperator op) throws DMLRuntimeException {
	...
	/**
	 * Performs elementwise arithmetic operation specified by op of two input
	 * matrices in1 and in2
	 *
	 * @param ec
	 *            execution context
	 ...
	 *            output matrix name
	 * @param isLeftTransposed
	 *            true if left-transposed
	 * @param isRightTransposed
	 *            true if right-transposed
	 * @param op
	 *            binary operator
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	public static void matrixMatrixArithmetic(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1,
			MatrixObject in2, String outputName, boolean isLeftTransposed, boolean isRightTransposed, BinaryOperator op)
			throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		boolean isCUDALibAvailable = (op.fn instanceof Plus || op.fn instanceof Minus) && !isSparseAndEmpty(gCtx, in1)
				&& !isSparseAndEmpty(gCtx, in2) && !isVector(in1) && !isVector(in2);
		if (!isCUDALibAvailable) {
	 ...
	 * @param isInputTransposed
	 *            true if input is transposed
	 * @param op
	 *            operator
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	private static void matrixScalarOp(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in,
			String outputName, boolean isInputTransposed, ScalarOperator op) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		int rlenA = (int) in.getNumRows();
		int clenA = (int) in.getNumColumns();
		Pointer A = getDensePointer(gCtx, in, instName); // TODO: FIXME:
															// Implement sparse
															// binCellSparseScalarOp
															// kernel
		double scalar = op.getConstant();
		// MatrixObject out = ec.getMatrixObject(outputName);
		MatrixObject out = getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, rlenA, clenA); // Allocated
																											// the
																											// dense
																											// output
																											// matrix
	 ...
	 * operations CUDA kernel. This method is isolated to be taken advantage of
	 * from other operations as it accepts JCuda {@link Pointer} instances
	 * instead of {@link MatrixObject} instances.
	 *
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param a
	 *            the dense input matrix (allocated on GPU)
	 * @param scalar
	 *            the scalar value to do the op
	 * @param rlenA
	 *            row length of matrix a
	 * @param clenA
	 *            column lenght of matrix a
	 * @param c
	 *            the dense output matrix
	 * @param op
	 *            operation to perform
	 * @throws DMLRuntimeException
	 *             throws runtime exception
	 */
	private static void matrixScalarOp(GPUContext gCtx, String instName, Pointer a, double scalar, int rlenA,
			int clenA, Pointer c, ScalarOperator op) throws DMLRuntimeException {
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		getCudaKernels(gCtx).launchKernel("matrix_scalar_op", ExecutionConfig.getConfigForSimpleVectorOperations(size),
				a, scalar, c, size, getBinaryOp(op.fn), isLeftScalar);
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_MATRIX_SCALAR_OP_KERNEL,
					System.nanoTime() - t0);
	}
	 ...
	 *            output variable name
	 * @param isLeftTransposed
	 *            true if left matrix is transposed
	 * @param isRightTransposed
	 *            true if right matrix is transposed
	 * @param op
	 *            operator
	 * @throws DMLRuntimeException
	 *             if DMLRuntimeException occurs
	 */
	private static void matrixMatrixOp(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1,
			MatrixObject in2, String outputName, boolean isLeftTransposed, boolean isRightTransposed, BinaryOperator op)
			throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
		...
		if (isEmpty1 && isEmpty2) {
			...
			// When both inputs are empty, the output is empty too (except in
		...
		// Check for M1 * M2 when M1 is empty; if M2 is a vector then fallback
		// to general case
		else if (isEmpty1 && clenB != 1 && rlenB != 1) {
			// C = empty_in1 op in2 ==> becomes ==> C = 0.0 op in2
			matrixScalarArithmetic(ec, gCtx, instName, in2, outputName, isRightTransposed, new LeftScalarOperator(
					op.fn, 0.0));
		}
		// Check for M1 * M2 when M2 is empty; if M1 is a vector then fallback
		// to general case
		else if (isEmpty2 && clenA != 1 && rlenA != 1) {
			// C = in1 op empty_in2 ==> becomes ==> C = in1 op 0.0
			matrixScalarArithmetic(ec, gCtx, instName, in1, outputName, isLeftTransposed, new RightScalarOperator(
					op.fn, 0.0));
		} else {
	...
	/**
	 * Do an elementwise matrix-matrix arithmetic operation on the GPU c = a op
	 * b Either rows and cols in A are the same as in B or one of them is a
	 * vector or both are.
	 * 
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 ...
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param a
	 *            The input matrix a allocated on the GPU
	 * @param b
	 *            The input matrix b allocated on the GPU
	 * @param maxRlen
	 *            the maximum of the row lengths between a & b
	 * @param maxClen
	 *            the maximum of the column lengths between a & b
	 * @param vecStatusA
	 *            if matrix A is a vector
	 * @param vecStatusB
	 *            if matrix B is a vector
	 * @param c
	 *            output matrix of size (maxRlen, maxClen) allocated on GPU
	 * @param op
	 *            the operation to perform
	 * @throws DMLRuntimeException
	 */
	private static void matrixMatrixOp(GPUContext gCtx, String instName, Pointer a, Pointer b, int maxRlen,
			int maxClen, int vecStatusA, int vecStatusB, Pointer c, BinaryOperator op) throws DMLRuntimeException {
		LOG.trace("GPU : matrix_matrix_cellwise_op" + ", GPUContext=" + gCtx);
		long t0 = 0;
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		getCudaKernels(gCtx).launchKernel("matrix_matrix_cellwise_op",
				ExecutionConfig.getConfigForSimpleMatrixOperations(maxRlen, maxClen), a, b, c, maxRlen, maxClen,
				vecStatusA, vecStatusB, getBinaryOp(op.fn));
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_MATRIX_MATRIX_CELLWISE_OP_KERNEL,
					System.nanoTime() - t0);
	}

	/**
	 * This enum declares the different vector shapes as they recognized in the
	 * invoked CUDA kernel(s).
	 */
	enum VectorShape {
		COLUMN(1), ROW(2), NONE(0);
	...
	/**
	 * Given the number of rows and columns, returns whether this is a row
	 * vector, column vector or neither.
	 * 
	 * @param rows
	 * @param cols
	...
	private static void deviceCopy(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject src,
	...
	private static void deviceCopy(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject src,
		...
		Pointer srcPtr = getDensePointer(gCtx, src, instName); // TODO: FIXME:
																// Implement
																// sparse kernel
		MatrixObject out = ec.getMatrixObject(outputName);
		getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, (int) src.getNumRows(),
				(int) src.getNumColumns()); // Allocated the dense output matrix
	...
	@SuppressWarnings("unused")
	private static void compareAndSet(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in,
			String outputName, double compareVal, double tolerance, double ifEqualsVal, double ifLessThanVal,
			double ifGreaterThanVal) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		Pointer A = getDensePointer(gCtx, in, instName); // TODO: FIXME:
		...
		getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, rlen, clen); // Allocated
																						// the
																						// dense
																						// output
																						// matrix
		...
		// compareAndSet(double* A, double* ret, int rlen, int clen, double
		// compareVal, double ifEqualsVal, double ifNotEqualsVal)
		long t0 = 0;
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		getCudaKernels(gCtx).launchKernel("compare_and_set",
				ExecutionConfig.getConfigForSimpleMatrixOperations(rlen, clen), A, ret, rlen, clen, compareVal,
				tolerance, ifEqualsVal, ifLessThanVal, ifGreaterThanVal);
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_COMPARE_AND_SET_KERNEL,
					System.nanoTime() - t0);
	}
	 ...
	 * 
	 * @param ec
	 *            currently active instance of the {@link ExecutionContext}
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 *            name of the invoking instruction to record{@link Statistics}.
	 * @param constant
	 *            scalar value with which to fill the matrix
	 * @param outputName
	 *            (internal) name of the matrix that is to be filled
	 * @param numRows
	 *            number of rows of output matrix object
	 * @param numCols
	 *            number of columns of output matrix object
	 * @throws DMLRuntimeException
	 *             if error
	 */
	private static void setOutputToConstant(ExecutionContext ec, GPUContext gCtx, String instName, double constant,
			String outputName, long numRows, long numCols) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		if (constant == 0) {
			...
			// MatrixObject out = ec.getMatrixObject(outputName);
			MatrixObject out = getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, numRows, numCols); // Allocated
																													// the
																													// dense
																													// output
																													// matrix
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t0 = System.nanoTime();
			int size = rlen * clen;
			getCudaKernels(gCtx).launchKernel("fill", ExecutionConfig.getConfigForSimpleVectorOperations(size), A,
					constant, size);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_FILL_KERNEL, System.nanoTime()
						- t0);
		}
	}
	...
	private static void deviceCopy(String instName, Pointer src, Pointer dest, int rlen, int clen)
		...
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DEVICE_TO_DEVICE, System.nanoTime()
					- t0);
	}

	/**
	 * Helper function to get numeric value for binary op. This number is passed
	 * down to the CUDA kernel and the appropriate binary operation is performed
	 * on the GPU. op = {0=plus, 1=minus, 2=multiply, 3=divide, 4=power, 5=less,
	 * 6=lessequal, 7=greater, 8=greaterequal, 9=equal, 10=notequal, 11=min,
	 * 12=max, 13=and, 14=or, 15=minus1multiply, 16=minusnz, 17=modulus,
	 * 18=integer division}
	 */
	private static int getBinaryOp(ValueFunction fn) throws DMLRuntimeException {
		if (fn instanceof Plus)
			return 0;
		else if (fn instanceof Minus)
			return 1;
		else if (fn instanceof Multiply)
			return 2;
		else if (fn instanceof Divide)
			return 3;
		else if (fn instanceof Power)
			return 4;
		else if (fn instanceof LessThan)
			return 5;
		else if (fn instanceof LessThanEquals)
			return 6;
		else if (fn instanceof GreaterThan)
			return 7;
		else if (fn instanceof GreaterThanEquals)
			return 8;
		else if (fn instanceof Equals)
			return 9;
		else if (fn instanceof NotEquals)
			return 10;
		else if (fn instanceof And)
			return 13;
		else if (fn instanceof Or)
			return 14;
		else if (fn instanceof Multiply2)
			return 2;
		else if (fn instanceof Power2)
			return 4;
		else if (fn instanceof Minus1Multiply)
			return 15;
		else if (fn instanceof MinusNz)
			return 16;
		else if (fn instanceof Modulus)
			return 17;
		else if (fn instanceof IntegerDivide)
			return 18;
	...
	/**
	 * Performs sparse and dense dgeam given two input matrices C = alpha* op( A
	 * ) + beta* op ( B ) where op = transpose or not (specified by
	 * isLeftTransposed and isRightTransposed). To indicate a transpose
	 * operation, make sure in1 == in2 and isLeftTransposed == isRightTransposed
	 * == true
	 * 
	 * @param ec
	 *            execution context
	 ...
	 *             if DMLRuntimeException occurs
	 */
	private static void dgeam(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1,
			MatrixObject in2, String outputName, boolean isLeftTransposed, boolean isRightTransposed, double alpha,
			double beta) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		if (isSparse1 || isSparse2) {
			...
			if (in1 == in2 && isLeftTransposed == true && isLeftTransposed == isRightTransposed) {
				...
				cusparseDcsr2csc(getCusparseHandle(gCtx), m, n, nnz, A.val, A.rowPtr, A.colInd, C.val, C.colInd,
						C.rowPtr, cusparseAction.CUSPARSE_ACTION_NUMERIC, cusparseIndexBase.CUSPARSE_INDEX_BASE_ZERO);
			} else {
				// General case (cusparse does not support accept the transpose
				// operator for dgeam)
				// TODO: to implement the transposed + dgeam for sparse
				// matrices, they need to be converted to csc, which is
				// effectively a tranpose
				if (isLeftTransposed || isRightTransposed) {
					throw new DMLRuntimeException(
				...
				if (GPUStatistics.DISPLAY_STATISTICS)
					t0 = System.nanoTime();
				JCusparse.cusparseDcsrgeam(getCusparseHandle(gCtx), m, n, alphaPtr, A.descr, (int) A.nnz, A.val,
						A.rowPtr, A.colInd, betaPtr, B.descr, (int) B.nnz, B.val, B.rowPtr, B.colInd, C.descr, C.val,
						C.rowPtr, C.colInd);
				// cudaDeviceSynchronize;
				if (GPUStatistics.DISPLAY_STATISTICS)
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DENSE_DGEAM_LIB,
						System.nanoTime() - t0);
		}
	}
	 ...
	 *             if DMLRuntimeException occurs
	 */
	public static void transpose(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in,
			String outputName) throws DMLRuntimeException {
		// C = alpha* op( A ) + beta* op ( B )
		// = 1.0 * A^T + 0.0 * A^T
		if (ec.getGPUContext(0) != gCtx)
	...
	public static void sliceOperations(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1,
		...
		// TODO: Handle case where in1.getGPUObject(gCtx).isSparse()
		MatrixObject out = getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, ru - rl + 1, cu - cl + 1);
		int len1 = (int) in1.getNumColumns();
		int len2 = (int) out.getNumColumns();
	...
	// lhsInput is always update in place. The caller has to perform necessary
	// copy.
	public static void denseLeftIndexingOperations(ExecutionContext ec, GPUContext gCtx, String instName,
			MatrixObject lhsOutput, MatrixObject rhsInput, IndexRange ixrange) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		LOG.trace("GPU : leftIndexingOperations" + ", GPUContext=" + gCtx);
		...
		int cl = (int) ixrange.colStart;
		int cu = (int) ixrange.colEnd;
		if (rl < 0 || rl >= lhsOutput.getNumRows() || ru < rl || ru >= lhsOutput.getNumRows() || cl < 0
				|| cu >= lhsOutput.getNumColumns() || cu < cl || cu >= lhsOutput.getNumColumns()) {
			throw new DMLRuntimeException("Invalid values for matrix indexing: [" + (rl + 1) + ":" + (ru + 1) + ","
					+ (cl + 1) + ":" + (cu + 1) + "] " + "must be within matrix dimensions [" + lhsOutput.getNumRows()
					+ "," + lhsOutput.getNumColumns() + "].");
		}
		if ((ru - rl + 1) < rhsInput.getNumRows() || (cu - cl + 1) < rhsInput.getNumColumns()) {
			throw new DMLRuntimeException("Invalid values for matrix indexing: " + "dimensions of the source matrix ["
					+ rhsInput.getNumRows() + "x" + rhsInput.getNumColumns() + "] "
					+ "do not match the shape of the matrix specified by indices [" + (rl + 1) + ":" + (ru + 1) + ", "
					+ (cl + 1) + ":" + (cu + 1) + "].");
		}

		Pointer lhsPointer = getDensePointer(gCtx, lhsOutput, instName);
		Pointer rhsPointer = getDensePointer(gCtx, rhsInput, instName);
		int rlen2 = (int) rhsInput.getNumRows();
		int clen1 = (int) lhsOutput.getNumColumns();
		int clen2 = (int) rhsInput.getNumColumns();
		if (clen1 == clen2) {
			cudaMemcpy(lhsPointer.withByteOffset(rl * clen1 * Sizeof.DOUBLE), rhsPointer,
					rlen2 * clen2 * Sizeof.DOUBLE, cudaMemcpyDeviceToDevice);
		} else {
			int rowLen = cu - cl + 1;
			for (int i = 0, ix1 = 0, ix2 = rl * clen1 + cl; i < rlen2; i++, ix1 += clen2, ix2 += clen1) {
				cudaMemcpy(lhsPointer.withByteOffset(ix2 * Sizeof.DOUBLE),
						rhsPointer.withByteOffset(ix1 * Sizeof.DOUBLE), rowLen * Sizeof.DOUBLE,
						cudaMemcpyDeviceToDevice);
			}
		}
	}

	public static void cbind(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1, MatrixObject in2,
			String outputName) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		getCudaKernels(gCtx).launchKernel("cbind",
				ExecutionConfig.getConfigForSimpleMatrixOperations(maxRows, maxCols), A, B, C, rowsA, colsA, rowsB,
				colsB);
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_CBIND_KERNEL, System.nanoTime() - t1);

	}

	public static void rbind(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1, MatrixObject in2,
			String outputName) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		if (colsA != colsB) {
			throw new DMLRuntimeException(
					"GPU : Invalid internal state - the columns must match up for a rbind operation");
		}
		...
		getCudaKernels(gCtx).launchKernel("rbind",
				...
				colsB);
		if (GPUStatistics.DISPLAY_STATISTICS)
			GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_RBIND_KERNEL, System.nanoTime() - t1);
	...
	public static void log(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1, String outputName)
		...
		// log(0) = -Inf
		unaryOp(ec, gCtx, in1, "matrix_log", Double.NEGATIVE_INFINITY, outputName, instName,
				GPUInstruction.MISC_TIMER_LOG_KERNEL);
	}
	...
	public static void acos(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1, String outputName)
		...
		// acos(0) = PI/2
		unaryOp(ec, gCtx, in1, "matrix_acos", Math.PI / 2.0, outputName, instName,
	 ...
	 *            input matrix
	 * @param kernel
	 *            name of CUDA kernel for the unary op to execute
	 * @param sparseAndEmptyFillValue
	 *            the result of the unary op on a completely empty input matrix
	 *            block
	 * @param outputName
	 *            output matrix name
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param kernelTimer
	 *            the name of the timer to measure the kernel invocation
	 * @throws DMLRuntimeException
	 */
	private static void unaryOp(ExecutionContext ec, GPUContext gCtx, MatrixObject in1, String kernel,
			double sparseAndEmptyFillValue, String outputName, String instName, String kernelTimer)
			throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		if (isSparseAndEmpty) {
			...
			// Dense
			MatrixObject out = getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, in1.getNumRows(),
					in1.getNumColumns());
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t1 = System.nanoTime();
			getCudaKernels(gCtx).launchKernel(kernel, ExecutionConfig.getConfigForSimpleVectorOperations(size), input,
					output, size);
			if (GPUStatistics.DISPLAY_STATISTICS)
	...
	public static void axpy(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1, MatrixObject in2,
		...
		Pointer B = getDensePointer(gCtx, in2, instName);
		MatrixObject out = ec.getMatrixObject(outputName);
		getDenseMatrixOutputForGPUInstruction(ec, instName, outputName, in1.getNumRows(), in1.getNumColumns()); // Allocated
																												// the
																												// dense
																												// output
																												// matrix
		...
		if (in1.getNumRows() == in2.getNumRows() && in1.getNumColumns() == in2.getNumColumns()) {
			...
			// Matrix-Matrix daxpy
			long n = in1.getNumRows() * in2.getNumColumns(); // Since A is
																// always a
																// matrix
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DEVICE_TO_DEVICE,
						System.nanoTime() - t1);
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics
						.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DAXPY_LIB, System.nanoTime() - t2);
		} else {
			LOG.trace("GPU : daxpy_matrix_vector" + ", GPUContext=" + gCtx);

			// Matrix-Vector daxpy
			// Note: Vector-Matrix operation is not supported
			// daxpy_matrix_vector(double* A, double* B, double alpha, double*
			// ret, int rlenA, int clenA, int rlenB, int clenB)
			if (GPUStatistics.DISPLAY_STATISTICS)
				t1 = System.nanoTime();
			int rlenA = (int) in1.getNumRows();
			int clenA = (int) in1.getNumColumns();
			...
			getCudaKernels(gCtx).launchKernel("daxpy_matrix_vector",
					ExecutionConfig.getConfigForSimpleMatrixOperations(rlenA, clenA), A, B, constant, C, rlenA, clenA,
					rlenB, clenB);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_DAXPY_MV_KERNEL,
						System.nanoTime() - t1);
		}
	}

	/**
	 * Implements the "solve" function for systemml Ax = B (A is of size m*n, B
	 * is of size m*1, x is of size n*1)
	 *
	 * @param ec
	 *            a valid {@link ExecutionContext}
	 * @param gCtx
	 *            a valid {@link GPUContext}
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 ...
	 *             if an error occurs
	 */
	public static void solve(ExecutionContext ec, GPUContext gCtx, String instName, MatrixObject in1, MatrixObject in2,
			String outputName) throws DMLRuntimeException {
		if (ec.getGPUContext(0) != gCtx)
			throw new DMLRuntimeException(
					"GPU : Invalid internal state, the GPUContext set with the ExecutionContext is not the same used to run this LibMatrixCUDA function");
		...
		if (!isInSparseFormat(gCtx, in1) && !isInSparseFormat(gCtx, in2)) { // Both
																			// dense
			...
			if ((int) in2.getNumRows() != m)
				throw new DMLRuntimeException(
						"GPU : Incorrect input for solve(), rows in A should be the same as rows in B");
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_OBJECT_CLONE, System.nanoTime()
						- t0);
			if (GPUStatistics.DISPLAY_STATISTICS)
				t0 = System.nanoTime();
			ATobj.denseRowMajorToColumnMajor();
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_ROW_TO_COLUMN_MAJOR,
						System.nanoTime() - t0);
			...
			// The following set of operations is done following the example in
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics
						.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_QR_BUFFER, System.nanoTime() - t0);
			...
			if (qrError[0] != 0) {
				throw new DMLRuntimeException(
						"GPU : Error in call to geqrf (QR factorization) as part of solve, argument " + qrError[0]
								+ " was wrong");
			}
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t0 = System.nanoTime();
			JCusolverDn.cusolverDnDormqr(gCtx.getCusolverDnHandle(), cublasSideMode.CUBLAS_SIDE_LEFT,
					cublasOperation.CUBLAS_OP_T, m, 1, n, A, m, tau, b, m, work, lwork[0], devInfo);
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_ORMQR, System.nanoTime() - t0);
			cudaMemcpy(Pointer.to(qrError), devInfo, Sizeof.INT, cudaMemcpyDeviceToHost);
			if (qrError[0] != 0) {
				throw new DMLRuntimeException(
						"GPU : Error in call to ormqr (to compuete Q^T*B after QR factorization) as part of solve, argument "
								+ qrError[0] + " was wrong");
			}
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				t0 = System.nanoTime();
			JCublas2.cublasDtrsm(gCtx.getCublasHandle(), cublasSideMode.CUBLAS_SIDE_LEFT,
					cublasFillMode.CUBLAS_FILL_MODE_UPPER, cublasOperation.CUBLAS_OP_N,
					cublasDiagType.CUBLAS_DIAG_NON_UNIT, n, 1, pointerTo(1.0), A, m, b, m);
			if (GPUStatistics.DISPLAY_STATISTICS)
			...
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_COLUMN_TO_ROW_MAJOR,
						System.nanoTime() - t0);

			// TODO : Find a way to assign bTobj directly to the output and set
			// the correct flags so as to not crash
			...
			cudaMemcpy(out.getGPUObject(gCtx).getJcudaDenseMatrixPtr(), bTobj.getJcudaDenseMatrixPtr(), n * 1
					* Sizeof.DOUBLE, cudaMemcpyDeviceToDevice);
		...
		} else if (isInSparseFormat(gCtx, in1) && isInSparseFormat(gCtx, in2)) { // Both
																					// sparse
			throw new DMLRuntimeException("GPU : solve on sparse inputs not supported");
		} else if (!isInSparseFormat(gCtx, in1) && isInSparseFormat(gCtx, in2)) { // A
																					// is
																					// dense,
																					// b
																					// is
	 ...
	 * 
	 * @param in
	 *            Pointer to a double array (matrix) on the GPU
	 * @param rlen
	 *            row length
	 * @param clen
	 *            column length
	...
	/**
	 * Helper method to get the output block (allocated on the GPU) Also records
	 * performance information into {@link Statistics}
	 * 
	 * @param ec
	 *            active {@link ExecutionContext}
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param name
	 *            name of input matrix (that the {@link ExecutionContext} is
	 *            aware of)
	 * @param numRows
	 *            number of rows of output matrix object
	 * @param numCols
	 ...
	 *            active {@link ExecutionContext}
	 * @param numRows
	 *            number of rows of matrix object
	 * @param numCols
	 *            number of columns of matrix object
	 * @param nnz
	 *            number of non zeroes in output matrix
	 * @param instName
	 *            the invoking instruction's name for record {@link Statistics}.
	 * @param name
	 *            name of input matrix (that the {@link ExecutionContext} is
	...
	private static MatrixObject getSparseMatrixOutputForGPUInstruction(ExecutionContext ec, long numRows, long numCols,
			long nnz, String instName, String name) throws DMLRuntimeException {
		long t0 = 0;
		if (GPUStatistics.DISPLAY_STATISTICS)
			t0 = System.nanoTime();
		...
		if (mb.getValue())
			if (GPUStatistics.DISPLAY_STATISTICS)
				GPUStatistics.maintainCPMiscTimes(instName, GPUInstruction.MISC_TIMER_ALLOCATE_SPARSE_OUTPUT,
						System.nanoTime() - t0);
		return mb.getKey();
	}
