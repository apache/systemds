...
def multiLogReg(X: OperationNode, Y: OperationNode, **kwargs: Dict[str, VALID_INPUT_TYPES]) -> OperationNode:
    ...
    :param icpt: Intercept presence, shifting and rescaling X columns: 0 = no intercept, no shifting, no rescaling; 1 = add intercept, but neither shift nor rescale X; 2 = add intercept, shift & rescale X columns to mean = 0, variance = 1
    ...
    return OperationNode(X.sds_context, 'multiLogReg', named_input_nodes=params_dict, output_type=OutputType.MATRIX)
