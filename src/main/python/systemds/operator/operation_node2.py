...
class OperationNode2(DAGNode):
    ...
    # Left hand side
    def __radd__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '+', [other, self], shape=self.shape)

    def __sub__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '-', [self, other], shape=self.shape)

    # Left hand side
    def __rsub__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '-', [other, self], shape=self.shape)

    def __mul__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '*', [self, other], shape=self.shape)

    def __rmul__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '*', [other, self], shape=self.shape)

    def __truediv__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '/', [self, other], shape=self.shape)

    def __rtruediv__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '/', [other, self], shape=self.shape)

    def __floordiv__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '//', [self, other], shape=self.shape)

    def __rfloordiv__(self, other: VALID_ARITHMETIC_TYPES) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '//', [other, self], shape=self.shape)

    def __lt__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '<', [self, other], shape=self.shape)

    def __rlt__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '<', [other, self], shape=self.shape)

    def __le__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '<=', [self, other], shape=self.shape)

    def __rle__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '<=', [other, self], shape=self.shape)

    def __gt__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '>', [self, other], shape=self.shape)

    def __rgt__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '>', [other, self], shape=self.shape)

    def __ge__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '>=', [self, other], shape=self.shape)

    def __rge__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '>=', [other, self], shape=self.shape)

    def __eq__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '==', [self, other], shape=self.shape)

    def __req__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '==', [other, self], shape=self.shape)

    def __ne__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '!=', [self, other], shape=self.shape)

    def __rne__(self, other) -> 'OperationNode2':
        return OperationNode2(self.sds_context, '!=', [other, self], shape=self.shape)

    def __matmul__(self, other: 'OperationNode2') -> 'OperationNode2':
        return OperationNode2(self.sds_context, '%*%', [self, other], shape=(self.shape[0], other.shape[1]))

    def sum(self, axis: int = None) -> 'OperationNode2':
        """Calculate sum of matrix.

        :param axis: can be 0 or 1 to do either row or column sums
        :return: `OperationNode2` representing operation
        """
        self._check_matrix_op()
        if axis == 0:
            return OperationNode2(self.sds_context, 'colSums', [self], shape=(self.shape[1],))
        elif axis == 1:
            return OperationNode2(self.sds_context, 'rowSums', [self], shape=(self.shape[0],))
        elif axis is None:
            return OperationNode2(self.sds_context, 'sum', [self], output_type=OutputType.DOUBLE)
        raise ValueError(
            f"Axis has to be either 0, 1 or None, for column, row or complete {self.operation}")

    def mean(self, axis: int = None) -> 'OperationNode2':
        ...
        self._check_matrix_op()
        if axis == 0:
            return OperationNode2(self.sds_context, 'colMeans', [self], shape=(self.shape[1],))
        elif axis == 1:
            return OperationNode2(self.sds_context, 'rowMeans', [self], shape=(self.shape[0],))
        elif axis is None:
            return OperationNode2(self.sds_context, 'mean', [self], output_type=OutputType.DOUBLE)
        raise ValueError(
            f"Axis has to be either 0, 1 or None, for column, row or complete {self.operation}")

    def var(self, axis: int = None) -> 'OperationNode2':
        ...
        self._check_matrix_op()
        if axis == 0:
            return OperationNode2(self.sds_context, 'colVars', [self], shape=(self.shape[1],))
        elif axis == 1:
            return OperationNode2(self.sds_context, 'rowVars', [self], shape=(self.shape[0],))
        elif axis is None:
            return OperationNode2(self.sds_context, 'var', [self], output_type=OutputType.DOUBLE)
        raise ValueError(
            f"Axis has to be either 0, 1 or None, for column, row or complete {self.operation}")

    def abs(self) -> 'OperationNode2':
        """Calculate absolute.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'abs', [self], shape=self.shape)

    def sin(self) -> 'OperationNode2':
        """Calculate sin.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'sin', [self], shape=self.shape)

    def cos(self) -> 'OperationNode2':
        """Calculate cos.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'cos', [self], shape=self.shape)

    def tan(self) -> 'OperationNode2':
        """Calculate tan.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'tan', [self], shape=self.shape)

    def asin(self) -> 'OperationNode2':
        """Calculate arcsin.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'asin', [self], shape=self.shape)

    def acos(self) -> 'OperationNode2':
        """Calculate arccos.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'acos', [self], shape=self.shape)

    def atan(self) -> 'OperationNode2':
        """Calculate arctan.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'atan', [self], shape=self.shape)

    def sinh(self) -> 'OperationNode2':
        """Calculate sin.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'sinh', [self], shape=self.shape)

    def cosh(self) -> 'OperationNode2':
        """Calculate cos.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'cosh', [self], shape=self.shape)

    def tanh(self) -> 'OperationNode2':
        """Calculate tan.

        :return: `OperationNode2` representing operation
        """
        return OperationNode2(self.sds_context, 'tanh', [self], shape=self.shape)

    def moment(self, moment: int, weights: DAGNode = None) -> 'OperationNode2':
        # TODO write tests
        self._check_matrix_op()
        unnamed_inputs = [self]
        ...
        return OperationNode2(self.sds_context, 'moment', unnamed_inputs, output_type=OutputType.DOUBLE)

    def write(self, destination: str, format: str = "binary",
              **kwargs: Dict[str, VALID_INPUT_TYPES]) -> 'OperationNode2':
        """ Write input to disk. 
        ...
        return OperationNode2(self.sds_context, 'write', unnamed_inputs, named_parameters, output_type=OutputType.NONE)

    def to_string(self, **kwargs: Dict[str, VALID_INPUT_TYPES]) -> 'OperationNode2':
        """ Converts the input to a string representation.
        :return: `OperationNode2` containing the string.
        """
        return OperationNode2(self.sds_context, 'toString', [self], kwargs, output_type=OutputType.SCALAR)

    def print(self, **kwargs: Dict[str, VALID_INPUT_TYPES]) -> 'OperationNode2':
        ...
        """
        return OperationNode2(self.sds_context, 'print', [self], kwargs, output_type=OutputType.NONE)

    def rev(self) -> 'OperationNode2':
        """ Reverses the rows in a matrix

        :return: the OperationNode2 representing this operation
        """

        self._check_matrix_op()
        return OperationNode2(self.sds_context, 'rev', [self], shape=self.shape)

    def order(self, by: int = 1, decreasing: bool = False,
              index_return: bool = False) -> 'OperationNode2':
        ...
        return OperationNode2(self.sds_context, 'order', [], named_input_nodes=named_input_nodes, shape=self.shape)

    def t(self) -> 'OperationNode2':
        ...
        return OperationNode2(self.sds_context, 't', [self], shape=shape)

    def cholesky(self, safe: bool = False) -> 'OperationNode2':
        ...
        return OperationNode2(self.sds_context, 'cholesky', [self], shape=self.shape)

    def to_one_hot(self, num_classes: int) -> 'OperationNode2':
        ...
        :return: The OperationNode2 containing the oneHotEncoded values
        """
        ...
        return OperationNode2(self.sds_context, 'toOneHot', named_input_nodes=named_input_nodes,
                             shape=(self.shape[0], num_classes))

    def rbind(self, other) -> 'OperationNode2':
        """
        ...
        :return: The OperationNode2 containing the concatenated matrices.
        """
        ...
        return OperationNode2(self.sds_context, 'rbind', [self, other],
                             shape=(self.shape[0] + other.shape[0], self.shape[1]))

    def cbind(self, other) -> 'OperationNode2':
        """
        ...
        return OperationNode2(self.sds_context, 'cbind', [self, other],
