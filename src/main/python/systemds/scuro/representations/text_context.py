...
class OverlappingSplit(Context):
    ...
    Parameters:
        max_words (int): Maximum number of words per chunk (default: 55)
        overlap_words (int): Number of overlapping words between chunks (default: 10)
        stride (int, optional): Step size in words. If None, stride = max_words - overlap_words
    """

    def __init__(self, max_words=55, overlap_words=10, stride=None):
        if stride is None:
            stride = max_words - overlap_words

        parameters = {
            "max_words": [40, 55, 70, 250, 300, 350, 400, 450],
            "overlap_words": [10, 20, 30],
            "stride": [10, 15, 20, 30],
        }
