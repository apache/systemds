...
def pipe_receive_header(pipe, pipe_id):
...
def pipe_receive_bytes(pipe, byte_array, offset, end, batch_size_bytes):
...
def numpy_to_matrix_block(sds, np_arr: np.array):
    ...
    try:
        if sds.data_transfer_mode == 1:
            ...
            use_single_pipe = (
                not sds.multi_pipe_enabled or total_bytes < 2 * min_bytes_per_pipe
            )
            if use_single_pipe:
                print("single pipe")
                pipe_id = 0
                pipe = sds.FIFO_PY2JAVA_PIPES[pipe_id]
                fut = sds.executor_pool.submit(
                ...
                num_pipes = min(
                    len(sds.FIFO_PY2JAVA_PIPES), total_bytes // min_bytes_per_pipe
                )
                ...
                fut_java = sds.executor_pool.submit(
...
def matrix_block_to_numpy(sds, mb: JavaObject):
    ...
    if sds.data_transfer_mode == 1:
        ...
        try:
            pipe_id = 0
            pipe = sds.FIFO_JAVA2PY_PIPES[pipe_id]

            # Java starts writing to pipe in background
            fut = sds.executor_pool.submit(ep.startWritingMbToPipe, pipe_id, mb)

            pipe_receive_header(pipe, pipe_id)
            pipe_receive_bytes(pipe, mv, 0, total_bytes, batch_size_bytes)
            pipe_receive_header(pipe, pipe_id)

            fut.result()  # wait for Java to finish writing
