...
class TestDataLoaders(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.video = VideoModality('', ResNet(f'{cls.test_file_path}/embeddings/resnet_embeddings.hdf5'))
        cls.audio = AudioModality('',
                                  MelSpectrogram(output_file=f'{cls.test_file_path}/embeddings/mel_sp_embeddings.npy'))
        cls.text = TextModality('', Bert(avg_layers=4,
    ...
    def test_load_audio_data_from_file(self):
        load_audio = AudioModality(f'{self.test_file_path}/embeddings/mel_sp_embeddings.npy', NPY())
        load_audio.read_all(self.indizes)
        
        for i in range(0, self.num_instances):
            assert (round(sum(self.audio.data[i]), 4) == round(sum(load_audio.data[i]), 4))
    
    def test_load_video_data_from_file(self):
        load_video = VideoModality(f'{self.test_file_path}/embeddings/resnet_embeddings.hdf5', HDF5())
        load_video.read_all(self.indizes)
        
        for i in range(0, self.num_instances):
            assert (round(sum(self.video.data[i]), 4) == round(sum(load_video.data[i]), 4))
    
    def test_load_text_data_from_file(self):
        load_text = TextModality(f'{self.test_file_path}/embeddings/bert_embeddings.pkl', Pickle())
