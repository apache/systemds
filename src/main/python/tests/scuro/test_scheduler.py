...
from systemds.scuro.drsearch.representation_dag import RepresentationDag, RepresentationNode, CSEAwareDAGBuilder
...
def _make_modality(data, modality_type):
    dummy_data_loader = TestDataLoader(indices=np.arange(len(data)), chunk_size=None, modality_type=modality_type, data=data, data_type=np.float32, metadata={})
    mod = UnimodalModality(data_loader=dummy_data_loader)
    return mod


def get_node_from_dags(dags: List[RepresentationDag], node_id: str) -> RepresentationNode:
    for dag in dags:
...
class TestNodeResourceScheduler(unittest.TestCase):
    def setUp(self):
        ...
        leaf_id = self.dag_builder.create_leaf_node(modality_id=self.modalities[0].modality_id)
        rep_id_0 = self.dag_builder.create_operation_node(DummyOperation, [leaf_id], {"dimensionality": 20})
        for i in range(10):
            rep_id = self.dag_builder.create_operation_node(DummyOperation, [rep_id_0], {"dimensionality": 1000 + i})
            rep_id_2 = self.dag_builder.create_operation_node(DummyOperation, [rep_id], {"dimensionality": 200 + i})
            self.dags.append(self.dag_builder.build(rep_id_2))

    def test_get_ready_nodes_first_level(self):
        scheduler = MemoryAwareNodeScheduler(self.dags, self.modalities, 1024 * 1024 * 12, 1024 * 1024 * 4)
        ready_nodes = scheduler.get_runnable()
        self.assertEqual(len(ready_nodes), 1)
        for node in ready_nodes:
            self.assertEqual(get_node_from_dags(self.dags, node).parameters["dimensionality"], 20)
    ...
    def test_complete_nodes(self):
        ...
        self.assertEqual(scheduler.memory_stats["cpu_in_use"], 0.0)
    ...
    def test_get_ready_nodes_second_level(self):
        ...
        for node in ready_nodes_2:
            self.assertGreaterEqual(get_node_from_dags(self.dags, node).parameters["dimensionality"], 1000)
    ...
    def test_schedule_nodes_when_memory_is_released(self):
        ...
        self.assertEqual(len(ready_nodes_3), 2)
    ...
    def test_deadlock_when_no_nodes_are_runnable(self):
        scheduler = MemoryAwareNodeScheduler(self.dags, self.modalities, 1024 * 1024 * 3, 0)
        while not scheduler.is_finished():
            ready_nodes = scheduler.get_runnable()
            for node in ready_nodes.copy():
                scheduler.move_to_running(node)
                scheduler.complete_node(node)
        self.assertTrue(scheduler.is_finished())
        self.assertTrue(scheduler.deadlock)
        self.assertGreater(len(scheduler.blocked_memory_nodes_perm), 0)