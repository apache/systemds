 ...
 *
 * This benchmark is part of the performance test package and is intended to be
 * run from the performance jar, not through JUnit/Surefire.
 *
 ...
 * - sparse_like_fp64
 *
 * Additionally manual multipart experiment:
 * - enable with -Dsysds.performance.parquet.manual.parts=2,4,8
 *
 * Output:
 * - target/performance/parquet/parquet-benchmark.csv
 * - target/performance/parquet/parquet-benchmark.json
 */
public class ParquetIOBenchmark {
    ...
    public void run() throws Exception {
        System.out.println("Starting ParquetIOBenchmark Run!");
    ...
    private void benchmarkProfile(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile,
        ...
        benchmarkFooterRead(csv, json, jsonState, dataProfile, "seq", seqInput, frameBlock, warmup, reps,
                "parquet_footer_only");
        benchmarkRead(csv, json, jsonState, dataProfile, "seq", new FrameReaderParquet(), seqInput, frameBlock,
                warmup, reps, "");
    ...
    private String[] parseProfileList(String value) {
        ...
        for(String token : tokens) {
            ...
            if(!isSupportedProfile(profile))
                throw new RuntimeException("Unknown Parquet benchmark data profile: " + token);
