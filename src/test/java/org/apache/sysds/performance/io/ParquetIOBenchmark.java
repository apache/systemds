...
public class ParquetIOBenchmark {
	...
	private static final String PROFILE_SPARSE_LIKE_FP64 = "sparse_like_fp64";
	...
	public ParquetIOBenchmark() {
		resultDir = new File("target/performance/parquet");
		workDir = new File(resultDir, "work");
	}

	public static void main(String[] args) throws Exception {
		new ParquetIOBenchmark().run();
	}

	public void run() throws Exception {
		final int rows = getIntProperty("sysds.performance.parquet.rows", "sysds.test.parquet.rows", 100_000);
		final int cols = getIntProperty("sysds.performance.parquet.cols", "sysds.test.parquet.cols", 50);
		...
		if(!resultDir.exists() && !resultDir.mkdirs())
			throw new IOException("Failed to create result directory: " + resultDir.getAbsolutePath());
		if(!workDir.exists() && !workDir.mkdirs())
			throw new IOException("Failed to create work directory: " + workDir.getAbsolutePath());

		File csvFile = new File(resultDir, "parquet-benchmark.csv");
		File jsonFile = new File(resultDir, "parquet-benchmark.json");

		System.out.println("Running ParquetIOBenchmark");
		System.out.println("rows=" + rows + ", cols=" + cols + ", warmup=" + warmup + ", reps=" + reps);
		System.out.println("results=" + resultDir.getAbsolutePath());

		try(PrintWriter csv = new PrintWriter(new FileWriter(csvFile));
			PrintWriter json = new PrintWriter(new FileWriter(jsonFile))) {
			writeCsvHeader(csv);
			...
			for(String dataProfile : profiles) {
				System.out.println("Benchmarking profile: " + dataProfile);
	...
	private void benchmarkProfile(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile,
		FrameBlock frameBlock, int warmup, int reps) throws Exception {
		...
		benchmarkRawIORead(csv, json, jsonState, dataProfile, "parallel", parallelInput, frameBlock, warmup, reps,
			"raw_bytes_only");
		benchmarkFooterRead(csv, json, jsonState, dataProfile, "parallel", parallelInput, frameBlock, warmup, reps,
			"parquet_footer_only");
		benchmarkRead(csv, json, jsonState, dataProfile, "parallel", new FrameReaderParquetParallel(), parallelInput,
			frameBlock, warmup, reps, "current_parallel_read_path_not_value_validated");
	}

	private void benchmarkManualMultipart(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile,
		FrameBlock frameBlock, int[] manualParts, int warmup, int reps) throws Exception {
		for(int numParts : manualParts) {
			...
			benchmarkRawIORead(csv, json, jsonState, dataProfile, impl, manualInput, frameBlock, warmup, reps,
				"raw_bytes_only");
			benchmarkFooterRead(csv, json, jsonState, dataProfile, impl, manualInput, frameBlock, warmup, reps,
				"parquet_footer_only");
			benchmarkRead(csv, json, jsonState, dataProfile, impl, new FrameReaderParquetParallel(), manualInput,
				frameBlock, warmup, reps, "manual_multipart_input_not_value_validated");
		}
	}
	...
	private void writeCsvHeader(PrintWriter csv) {
		csv.println("data_profile,impl,operation,rows,cols,cells,rep,is_warmup,wall_ms,"
			...
			+ "hdfs_block_size_bytes,legacy_cell_based_estimated_num_part_files,"
			+ "legacy_cell_based_estimated_num_threads,"
	...
	private void benchmarkWrite(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile, String impl,
		FrameWriter writer, FrameBlock frameBlock, int warmup, int reps, String notes) throws Exception {
		int rows = frameBlock.getNumRows();
		int cols = frameBlock.getNumColumns();
		...
		for(int i = 0; i < warmup + reps; i++) {
			...
			writeResult(csv, json, jsonState, dataProfile, impl, "write", rows, cols, i, isWarmup, t0, t1,
				pathStats.fileSizeBytes, pathStats.numPartFiles, heapBefore, heapAfter, gcAfter.count - gcBefore.count,
				gcAfter.timeMs - gcBefore.timeMs, notes);
		}
	}

	private void benchmarkRead(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile, String impl,
		FrameReader reader, String path, FrameBlock reference, int warmup, int reps, String notes) throws Exception {
		...
		for(int i = 0; i < warmup + reps; i++) {
			...
			writeResult(csv, json, jsonState, dataProfile, impl, "read", rows, cols, i, isWarmup, t0, t1,
				pathStats.fileSizeBytes, pathStats.numPartFiles, heapBefore, heapAfter, gcAfter.count - gcBefore.count,
				gcAfter.timeMs - gcBefore.timeMs, notes);
		}
	}

	private void benchmarkRawIORead(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile,
		String impl, String path, FrameBlock reference, int warmup, int reps, String notes) throws Exception {
		int rows = reference.getNumRows();
		int cols = reference.getNumColumns();
		...
		for(int i = 0; i < warmup + reps; i++) {
			...
			writeResult(csv, json, jsonState, dataProfile, impl, "raw_io_read", rows, cols, i, isWarmup, t0, t1,
				pathStats.fileSizeBytes, pathStats.numPartFiles, heapBefore, heapAfter, gcAfter.count - gcBefore.count,
				gcAfter.timeMs - gcBefore.timeMs, notes);
		}
	}

	private void benchmarkFooterRead(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile,
		String impl, String path, FrameBlock reference, int warmup, int reps, String notes) throws Exception {
		int rows = reference.getNumRows();
		int cols = reference.getNumColumns();
		...
		for(int i = 0; i < warmup + reps; i++) {
			...
			writeResult(csv, json, jsonState, dataProfile, impl, "footer_read", rows, cols, i, isWarmup, t0, t1,
				pathStats.fileSizeBytes, pathStats.numPartFiles, heapBefore, heapAfter, gcAfter.count - gcBefore.count,
				gcAfter.timeMs - gcBefore.timeMs, notes);
		}
	}
	...
	private String output(String name) {
		return new File(workDir, name).toURI().toString();
	}

	private void writeResult(PrintWriter csv, PrintWriter json, JsonState jsonState, String dataProfile, String impl,
		String operation, int rows, int cols, int rep, boolean isWarmup, long t0, long t1, long fileSizeBytes,
		int numPartFiles, long heapBefore, long heapAfter, long gcCountDelta, long gcTimeDeltaMs, String notes) {
		double wallMs = (t1 - t0) / 1e6;
		BenchmarkDiagnostics diag = getBenchmarkDiagnostics(dataProfile, impl, operation, rows, cols, fileSizeBytes,
			numPartFiles);

		csv.printf(Locale.US,
			"%s,%s,%s,%d,%d,%d,%d,%s,%.3f,%d,%d,%.3f,%d,%d,%d,%d,%d,%d,%d,%.6f,%.6f,%s,%s,%d,%d,%d,%d,%d,%s%n",
			escapeCsv(dataProfile), escapeCsv(impl), escapeCsv(operation), rows, cols, diag.cells, rep, isWarmup,
	...
	private BenchmarkDiagnostics getBenchmarkDiagnostics(String dataProfile, String impl, String operation, int rows,
		int cols, long fileSizeBytes, int numPartFiles) {
		...
		int currentWriterEstimatedNumThreads = (int) Math.min(configuredParallelWriteParallelism,
			currentWriterEstimatedNumPartFiles);
		int estimatedParallelReadTasks = estimateParallelReadTasks(impl, operation, numPartFiles,
			configuredParallelReadParallelism);
	...
	private double getExpectedNonZeroFraction(String dataProfile) {
		String profile = normalizeProfileName(dataProfile);
		if(PROFILE_DENSE_FP64.equals(profile))
			return 1.0;
		else if(PROFILE_SPARSE_LIKE_FP64.equals(profile))
			return 0.05;
		else
			return -1.0;
	}
	...
	private FrameBlock generateFrame(String dataProfile, int rows, int cols) {
		String profile = normalizeProfileName(dataProfile);
		if(PROFILE_DENSE_FP64.equals(profile))
			return generateDenseFp64Frame(rows, cols);
		else if(PROFILE_MIXED_SCHEMA.equals(profile))
			return generateMixedSchemaFrame(rows, cols);
		else if(PROFILE_SPARSE_LIKE_FP64.equals(profile))
			return generateSparseLikeFp64Frame(rows, cols);
		else
			throw new IllegalArgumentException("Unknown Parquet benchmark data profile: " + dataProfile);
	}

	private FrameBlock generateDenseFp64Frame(int rows, int cols) {
		...
		for(int i = 0; i < rows; i++) {
			...
			for(int j = 0; j < cols; j++)
				row[j] = (double) ((long) i * cols + j + 1);
			frameBlock.appendRow(row);
		}
		return frameBlock;
	}

	private FrameBlock generateSparseLikeFp64Frame(int rows, int cols) {
		ValueType[] schema = new ValueType[cols];
		for(int j = 0; j < cols; j++)
			schema[j] = ValueType.FP64;
		...
		for(int i = 0; i < rows; i++) {
			Object[] row = new Object[cols];
			for(int j = 0; j < cols; j++) {
				long linearIndex = (long) i * cols + j;
				row[j] = linearIndex % 20 == 0 ? (double) (linearIndex + 1) : 0.0;
			}
			frameBlock.appendRow(row);
		}
		return frameBlock;
			...
			for(int j = 0; j < cols; j++) {
				switch(schema[j]) {
					...
					case STRING:
						...
						break;
					case INT32:
						row[j] = (int) ((long) i * cols + j + 1);
						break;
					default:
						throw new IllegalArgumentException("Unsupported generated type: " + schema[j]);
				}
			}
			frameBlock.appendRow(row);
		}
	...
	private PathStats getPathStats(String fname) {
		try {
			...
			System.err.println("Failed to collect path stats for " + fname + ": " + ex.getMessage());
	...
	private String[] parseProfileList(String value) {
		if(value == null || value.trim().isEmpty())
			return new String[] {PROFILE_DENSE_FP64, PROFILE_MIXED_SCHEMA, PROFILE_SPARSE_LIKE_FP64};

		String[] tokens = value.split(",");
		List<String> profiles = new ArrayList<>();

		for(String token : tokens) {
			String profile = normalizeProfileName(token);
			if(profile.isEmpty())
				continue;
			if(!isSupportedProfile(profile))
				throw new IllegalArgumentException("Unknown Parquet benchmark data profile: " + token);
			profiles.add(profile);
		}

		if(profiles.isEmpty())
			throw new IllegalArgumentException("No valid Parquet benchmark data profiles specified.");

		return profiles.toArray(new String[0]);
	}

	private boolean isSupportedProfile(String profile) {
		return PROFILE_DENSE_FP64.equals(profile) || PROFILE_MIXED_SCHEMA.equals(profile) ||
			PROFILE_SPARSE_LIKE_FP64.equals(profile);
	}
	...
	private String normalizeProfileName(String value) {
		...
		String profile = value.trim().toLowerCase(Locale.US).replace('-', '_').replace(" ", "_");
		if("dense".equals(profile) || "dense_double".equals(profile) || "dense_double_only".equals(profile) ||
			"dense_fp64".equals(profile) || PROFILE_DENSE_FP64.equals(profile))
			return PROFILE_DENSE_FP64;
		if("mixed".equals(profile) || "mixed_types".equals(profile) || PROFILE_MIXED_SCHEMA.equals(profile))
			return PROFILE_MIXED_SCHEMA;
		if("sparse".equals(profile) || "sparse_like".equals(profile) || "sparse_double".equals(profile) ||
			"sparse_like_double".equals(profile) || "sparse_fp64".equals(profile) ||
			PROFILE_SPARSE_LIKE_FP64.equals(profile))
			return PROFILE_SPARSE_LIKE_FP64;
		return profile;
	}
	...
	private int getIntProperty(String primaryKey, String legacyKey, int defaultValue) {
		String value = getStringProperty(primaryKey, legacyKey, Integer.toString(defaultValue));
		return Integer.parseInt(value);
	}

	private String getStringProperty(String primaryKey, String legacyKey, String defaultValue) {
		String value = System.getProperty(primaryKey);
		if(value != null)
			return value;
		value = System.getProperty(legacyKey);
	...
	private void blackhole(FrameBlock frameBlock) {
		if(frameBlock == null)
			throw new IllegalArgumentException("Unexpected null FrameBlock.");
	...
	private static class BenchmarkDiagnostics {
		...
		private final long denseFp64ReferenceSizeBytes;
		private final double avgPartFileSizeBytes;
		private final double actualFileToDenseFp64ReferenceRatio;
		...
		private BenchmarkDiagnostics(long cells, long hdfsBlockSizeBytes, long currentWriterEstimatedNumPartFiles,
			int currentWriterEstimatedNumThreads, int configuredParallelReadParallelism,
			int configuredParallelWriteParallelism, int estimatedParallelReadTasks, long denseFp64ReferenceSizeBytes,
			double avgPartFileSizeBytes, double actualFileToDenseFp64ReferenceRatio, double expectedNonZeroFraction,
			String readerValueExtractionMode, String frameMaterialization) {
			this.cells = cells;
			...
			this.denseFp64ReferenceSizeBytes = denseFp64ReferenceSizeBytes;
			this.avgPartFileSizeBytes = avgPartFileSizeBytes;
			this.actualFileToDenseFp64ReferenceRatio = actualFileToDenseFp64ReferenceRatio;
