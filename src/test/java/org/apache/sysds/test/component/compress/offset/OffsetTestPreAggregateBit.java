...
public class OffsetTestPreAggregateBit extends OffsetTestPreAggregate {

	public OffsetTestPreAggregateBit(int[] data, OFF_TYPE type) {
		super(data, type);
	}
	...
	protected void preAggMapOutOfRangeBefore(int row) {
		...
		BitSet m = null; // new byte[data.length];
	...
	protected void preAggMapOutOfRangeAfter(int row) {
		double[] preAV = null; // should not need access this therefore we make a null argument here.
		BitSet m = null;// new char[data.length];
