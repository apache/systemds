...
public class OffsetTestPreAggregateChar extends OffsetTestPreAggregate {

	public OffsetTestPreAggregateChar(int[] data, OFF_TYPE type) {
		super(data, type);
	}
	...
	protected void preAggMapOutOfRangeBefore(int row) {
		...
		char[] m = null; // new byte[data.length];
	...
	protected void preAggMapOutOfRangeAfter(int row) {
		double[] preAV = null; // should not need access this therefore we make a null argument here.
		char[] m = null;// new char[data.length];
