...
public class QuantizationFusedForcedCompressionTypesTest {
     ...
     * multiplied by 1.0 and floored. Essentially m0 = floor(m1 * scaleFactor). The best compression types for both
     * matrices are DDC, but we explicitly create UNCOMPRESSED columns.
     * 
     */
    @Test
        ...
        try {
            ...
            for(int i = 0; i < compressedGroups0.getInfo().size(); i++) {
                AColGroup colGroup0 = ColGroupUncompressed.create(compressedGroups0.getInfo().get(i).getColumns(), m0,
                    cs0.transposed);
                AColGroup colGroup1 = ColGroupUncompressed.createQuantized(
                    compressedGroups1.getInfo().get(i).getColumns(), m1, cs1.transposed, cs1.scaleFactors);
     ...
     * internally multiplied by 1.0 and floored. Essentially m0 = floor(m1 * scaleFactor).
     * Reaches extractBitmapSingleColumn().
     */
    @Test
     ...
     * 
     * m0 is generated as a floored matrix. m1 is generated as a full-precision matrix, but will be
     * internally multiplied by 1.0 and floored. Essentially m0 = floor(m1 * scaleFactor).
     * Reaches extractBitmapMultiColumns().
     * 
     */
    @Test
     ...
     * m0 is generated as a floored matrix. m1 is generated as a full-precision matrix, but will be
     * internally multiplied by 1.0 and floored. Essentially m0 = floor(m1 * scaleFactor).
     * Reaches extractBitmapMultiColumn().
     */
    @Test
     ...
     * internally multiplied by 1.0 and floored. Essentially m0 = floor(m1 * scaleFactor).
     * Reaches extractBitmapSingleColumn().
     * This should also cover CONST, EMPTY, SDCFOR.
     */
    @Test
     ...
     * m0 is generated as a floored column matrix. m1 is generated as a full-precision column matrix, but will be
     * internally multiplied by 1.0 and floored. Essentially m0 = floor(m1 * scaleFactor).
     * Reaches directCompressDDCSingleCol().
     * This should also cover DDCFOR.
     */
    @Test
     ...
     * m0 is generated as a floored matrix. m1 is generated as a full-precision matrix, but will be
     * internally multiplied by 1.0 and floored. Essentially m0 = floor(m1 * scaleFactor).
     * Reaches directCompressDDCMultiCol().
     * This should also cover DDCFOR.
     */
    @Test
     ...
     * Reaches extractBitmapSingleColumn().
     */
    private void testForcedCompressionTypeSingleColumn(CompressionType compressionType, Class<? extends AColGroup> expectedGroupClass) {
        try {
            int nRow = 100;
            int nCol = 1;
            int max = 50;
            ...
            for (int i = 0; i < results0.size(); i++) {
                ...
                assertEquals("Mismatch in sum of values in column group", 
                    results0.get(i).getSum(nRow), results1.get(i).getSum(nRow), 0.0);
            }
        } catch (Exception e) {
            e.printStackTrace();
            fail("Compression extraction failed: " + e.getMessage());
    ...
    private void testForcedCompressionTypeMultiColumn(CompressionType compressionType, Class<? extends AColGroup> expectedGroupClass) {
        try {
            double[][] values = {
                {1.5, 2.5, 3.5, 4.5, 5.5}, 
                {1.5, 2.5, 3.5, 4.5, 5.5}, 
     ...
     */
    private static Pair<CompressedSizeInfo, AComEst> generateCompressedGroups(MatrixBlock matrix, CompressionSettings cs) {
    ...
    private static List<AColGroup> compressWithForcedTypeCoCode(MatrixBlock matrix, CompressionSettings cs,
        ...
        ACostEstimate ice = CostEstimatorFactory.create(cs, null, matrix.getNumRows(), matrix.getNumColumns(), matrix.getSparsity());
