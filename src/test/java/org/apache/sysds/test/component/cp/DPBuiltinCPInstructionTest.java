 ...
 *
 * The tests are grouped into three levels: - Unit tests on DPBudgetAccountant — verify composition, conversion, and
 * budget enforcement in isolation, with no dependency on the full SystemDS runtime. - Noise distribution tests — verify
 * that the noise blocks generated by the Laplace and Gaussian mechanisms have statistically correct means and variances
 * (Kolmogorov-Smirnov style sanity checks). - DML integration tests</b> — run complete DML scripts and verify
...
public class DPBuiltinCPInstructionTest {
	...
	public void testDMLProgramHasDPBudgetTracksSetState() {
		...
		Assert.assertFalse("No dp_set_budget call yet", dmlProg.hasDPBudget());
		dmlProg.setDPBudget(2.0, 1e-6);
		Assert.assertTrue("dp_set_budget was called", dmlProg.hasDPBudget());
		Assert.assertEquals(2.0, dmlProg.getDPBudgetEpsilon(), EPS);
		Assert.assertEquals(1e-6, dmlProg.getDPBudgetDelta(), EPS);
	}
	...
	public void testGetDPBudgetAccountantUsesCompileTimeResolvedBudget() {
		...
		Assert.assertTrue("Compile-time-resolved budget should be used instead of the hardcoded default",
	...
	public void testGetDPBudgetAccountantIsLazyAndCachedPerContext() {
		...
		Assert.assertEquals("Same accountant instance must be reused across calls", 1,
			ec.getDPBudgetAccountant().releaseCount());
	}
	...
	public void testConvenienceConstructorDefaultsDeltaTo1e5() {
		...
		Assert.assertEquals("Convenience constructor must default to delta=1e-5", twoArg.totalEpsilonSpent(),
			oneArg.totalEpsilonSpent(), EPS);
	}
	...
	public void testReleaseCountTracksAllReleases() {
		...
		Assert.assertEquals(2, acc.releaseCount());
		acc.compose(0.1, 0.0, 1.0); // Laplace
		acc.compose(0.1, 0.0, 1.0); // Laplace
		acc.compose(0.1, 1e-5, 1.0); // Gaussian
		Assert.assertEquals(5, acc.releaseCount());
	}
	...
	public void testGaussianSensitivityCancelsInRDP() {
		...
		Assert.assertEquals("Gaussian RDP cost must be independent of sensitivity when (ε,δ) are fixed",
