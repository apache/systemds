...
/**
 * Tests for DPBuiltinCPInstruction and DPBudgetAccountant.
 *
 * The tests are grouped into four levels:
 * - Unit tests on DPBudgetAccountant - verify composition, conversion, and
 * budget enforcement in isolation, with no dependency on the full SystemDS runtime.
 * - Noise distribution tests - verify that the noise blocks generated by the Laplace and
 * Gaussian mechanisms have statistically correct means and variances
 * (Kolmogorov-Smirnov style sanity checks).
 * - DPBuiltinCPInstruction structural tests - exercise parseInstruction()'s required-parameter
 * validation.
...
public class DPBuiltinCPInstructionTest {
	...
	// =======================================================================
	// 3. DPBuiltinCPInstruction structural tests
	// =======================================================================

	@Test
	public void testParseInstructionValidLaplace() {
		DPBuiltinCPInstruction inst = DPBuiltinCPInstruction
			.parseInstruction("CP°dp_laplace°target=mVar1°query=colMeans°sensitivity=1.0°epsilon=0.5°_mVar2·MATRIX·FP64");
		Assert.assertEquals(DPBuiltinCPInstruction.OPCODE_LAPLACE, inst.getOpcode());
		Assert.assertEquals("mVar1", inst.input1.getName());
		Assert.assertEquals("_mVar2", inst.getOutput().getName());
	}

	@Test
	public void testParseInstructionValidGaussian() {
		DPBuiltinCPInstruction inst = DPBuiltinCPInstruction.parseInstruction(
			"CP°dp_gaussian°target=mVar1°query=colMeans°sensitivity=1.0°epsilon=0.5°delta=1e-5°_mVar2·MATRIX·FP64");
		Assert.assertEquals(DPBuiltinCPInstruction.OPCODE_GAUSSIAN, inst.getOpcode());
		Assert.assertEquals("mVar1", inst.input1.getName());
		Assert.assertEquals("_mVar2", inst.getOutput().getName());
	}
	...
	private static void assertParseInstructionRejects(String instStr) {
		try {
			DPBuiltinCPInstruction.parseInstruction(instStr);
