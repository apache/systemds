...
/**
 * Manual micro-benchmark comparing the serial {@link FrameReaderDelta} against
 * the parallel {@link FrameReaderDeltaParallel} on multi-file Delta frame
 * tables. Not a correctness test (those live in {@link DeltaFrameReadWriteTest});
 * it just prints timing/throughput numbers and is {@link Ignore}d so it does not
 * run in the normal build.
 *
 * <p>The parallel reader decodes one task per parquet data file, so the speedup
 * scales with the number of files (controlled here via the writer target file
 * size). Run it on a JVM with a realistically sized heap; under a tiny young
 * generation (e.g. the Surefire fork's {@code -Xmn300m}) the concurrent decode's
 * higher allocation rate is dominated by young-GC pauses and the numbers are not
 * representative of a normal SystemDS process.</p>
 *
 * <p>Run explicitly (remove {@link Ignore} or run the compiled class directly),
 * e.g. {@code mvn -q test -Dtest=DeltaFrameReadPerf -DfailIfNoTests=false}.</p>
 */
public class DeltaFrameReadPerf {
	...
	/**
	 * Isolates the serial-reader change: compares the new direct (pre-sized,
	 * metadata-driven, single-pass) read against the old buffered (per-batch
	 * extract + concatenate) read on the SAME single-file table, so the only
	 * difference is the extra allocation + concatenation copy. Single file =>
	 * no file-level parallelism involved, pure serial decode cost.
	 */
	@Test
	...
	public void serialDirectVsBuffered() throws Exception {
		...
		for( String kind : new String[] {"numeric", "mixed", "string"} ) {
			...
			try {
				...
				System.out.printf("%-9s %11.2f %11.2f %8.2fx   alloc: %6.0f / %6.0f MB (%.2fx)%n",
					kind, md, mb, mb / md, ad / (double) MB, ab / (double) MB, ab / (double) ad);
			}
			finally {
	...
	/**
	 * End-to-end check of adaptive writer file sizing with NO explicit target size configured
	 * (the real default): the table should now be split into ~one file per reader and read fast,
	 * versus the single/few-file layout the fixed 64MB default produced.
	 */
	@Test
	@Ignore("manual benchmark; remove @Ignore (or run the compiled class directly) to run")
	public void adaptiveCheck() throws Exception {
		...
		System.out.printf("%-9s %-8s %-6s %11s %11s %9s%n",
			"rows", "adaptive", "files", "serial(ms)", "par(ms)", "speedup");
		for( int rows : new int[] {1_000_000, 4_000_000} ) {
			...
			try {
				...
				System.out.printf("%-9d %-8s %-6d %11.2f %11.2f %8.2fx%n",
					rows, (target / MB) + "MB", files, r[0], r[1], r[0] / r[1]);
			}
			finally {
				ConfigurationManager.clearLocalConfigs();
				FileUtils.deleteQuietly(dir.toFile());
	...
	/**
	 * Sweep the writer target file size ({@link DMLConfig#DELTA_WRITER_TARGET_FILE_SIZE}, the
	 * one public Delta knob that affects read parallelism) to find where the per-file parallel
	 * read stops improving, i.e. a good default for read-heavy use.
	 */
	@Test
	@Ignore("manual benchmark; remove @Ignore (or run the compiled class directly) to run")
	...
	/**
	 * Sweep the parquet reader batch size ({@link DMLConfig#DELTA_READER_BATCH_SIZE}, a
	 * public Delta Kernel knob) on a fixed multi-file table, with and without quieting the
	 * parquet/delta loggers. Pure "how we call the public API" tuning.
	 */
	@Test
	@Ignore("manual benchmark; remove @Ignore (or run the compiled class directly) to run")
	...
	public void benchmark() throws Exception {
		...
		System.out.printf("%-9s %-7s %-6s %11s %11s %9s%n",
			"rows", "fileMB", "files", "serial(ms)", "par(ms)", "speedup");
	...
	private void runCase(int rows, long targetFileSize) throws Exception {
		...
		try {
			...
			double ms = median(ts), mp = median(tp);
			System.out.printf("%-9d %-7d %-6d %11.2f %11.2f %8.2fx%n",
				rows, targetFileSize / MB, files, ms, mp, ms / mp);
		}
		finally {
			ConfigurationManager.clearLocalConfigs();
			FileUtils.deleteQuietly(dir.toFile());
	...
	private static FrameBlock genFrame(String kind, int nrow, int seed) {
		...
		switch( kind ) {
			case "numeric":
				schema = new ValueType[] {ValueType.INT64, ValueType.FP64, ValueType.INT32,
					ValueType.FP32, ValueType.BOOLEAN, ValueType.INT64};
				break;
			case "string":
				schema = new ValueType[] {ValueType.STRING, ValueType.STRING, ValueType.STRING,
					ValueType.STRING, ValueType.STRING, ValueType.STRING};
				break;
			default: // mixed
				schema = new ValueType[] {ValueType.STRING, ValueType.INT64, ValueType.FP64,
	...
	private static Object randVal(ValueType vt, Random rnd, int r) {
		switch( vt ) {
			case STRING:  return "row" + rnd.nextInt(1_000_000);
			case INT64:   return (long) rnd.nextInt();
			case FP64:    return rnd.nextDouble() * 200 - 100;
			case INT32:   return rnd.nextInt();
			case FP32:    return rnd.nextFloat();
			case BOOLEAN: return rnd.nextBoolean();
			default:      return null;
		}
	}

	private interface IORun { FrameBlock run() throws Exception; }
