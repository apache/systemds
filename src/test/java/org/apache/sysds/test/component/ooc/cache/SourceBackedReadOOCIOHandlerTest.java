...
public class SourceBackedReadOOCIOHandlerTest extends AutomatedTestBase {
	...
	public void testSourceBackedScheduleRead() throws Exception {
		...
		BlockEntry entry = BlockEntryTestAccess.newBlockEntry(key, desc.serializedSize, null);
		BlockEntryTestAccess.setState(entry, BlockState.COLD);
		handler.scheduleRead(entry).get();

		IndexedMatrixValue imv = (IndexedMatrixValue) BlockEntryTestAccess.getDataUnsafe(entry);
