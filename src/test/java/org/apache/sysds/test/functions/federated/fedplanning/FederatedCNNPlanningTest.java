...
public class FederatedCNNPlanningTest extends AutomatedTestBase {
	private static final Log LOG = LogFactory.getLog(FederatedCNNPlanningTest.class.getName());

	private final static String TEST_DIR = "functions/privacy/fedplanning/";
	private final static String TEST_NAME = "FederatedCNNPlanningTest";
	private final static String TEST_CLASS_DIR = TEST_DIR + FederatedCNNPlanningTest.class.getSimpleName() + "/";
	private static File TEST_CONF_FILE;

	private final static int blocksize = 1024;
	public final int rows = 1000; // Number of images
	public final int cols = 784; // 28*28 flattened images
	...
	public void runCNNFOUTTest() {
		String[] expectedHeavyHitters = new String[] { "fed_fedinit", "fed_conv2d", "fed_maxpooling", "fed_ba+*" };
		setTestConf("SystemDS-config-fout.xml");
		loadAndRunTest(expectedHeavyHitters, TEST_NAME);
	}

	@Test
	public void runCNNHeuristicTest() {
	...
	@Test
	public void runCNNCostBasedTestPrivate() {
	...
	@Test
	public void runCNNCostBasedTestPublic() {
		String[] expectedHeavyHitters = new String[] {};
		setTestConf("SystemDS-config-cost-based.xml");
	...
	private void writeInputMatricesWithPrivacyConstraints(String privacyConstraints) {
		...
		writeOneHotLabels("Y", 85, privacyConstraints);
	}
	...
	private void writeOneHotLabels(String matrixName, long seed) {
		double[][] labels = getRandomMatrix(rows, classes, 0, 1, 1, seed);
		// Convert to one-hot encoded labels for CNN classification
	...
	private void writeOneHotLabels(String matrixName, long seed, String privacyConstraints) {
		double[][] labels = getRandomMatrix(rows, classes, 0, 1, 1, seed);
		// Convert to one-hot encoded labels for CNN classification
		for(int i = 0; i < rows; i++) {
			int maxIdx = 0;
	...
	private void writeStandardMatrix(String matrixName, long seed, int numRows) {
		// Generate MNIST-like image data (normalized 0-1)
