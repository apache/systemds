...
public class FederatedLinearRegressionPlanningTest extends AutomatedTestBase {
	private static final Log LOG = LogFactory.getLog(FederatedLinearRegressionPlanningTest.class.getName());

	private final static String TEST_DIR = "functions/privacy/fedplanning/";
	private final static String TEST_NAME = "FederatedLinearRegressionPlanningTest";
	private final static String TEST_CLASS_DIR = TEST_DIR + FederatedLinearRegressionPlanningTest.class.getSimpleName() + "/";
	...
	@Test
	public void runLinearRegressionFOUTTest() {
		String[] expectedHeavyHitters = new String[] { "fed_fedinit", "fed_tsmm", "fed_ba+*" };
		setTestConf("SystemDS-config-fout.xml");
		loadAndRunTest(expectedHeavyHitters, TEST_NAME);
	}

	@Test
	public void runLinearRegressionHeuristicTest() {
	...
	@Test
	public void runLinearRegressionCostBasedTestPrivate() {
	...
	@Test
	public void runLinearRegressionCostBasedTestPrivateAggregate() {
		String[] expectedHeavyHitters = new String[] {};
		setTestConf("SystemDS-config-cost-based.xml");
	...
	@Test
	public void runLinearRegressionCostBasedTestPublic() {
		String[] expectedHeavyHitters = new String[] {};
		setTestConf("SystemDS-config-cost-based.xml");
	...
	private void writeInputMatricesWithPrivacyConstraints(String privacyConstraints) {
		...
		writeTargetVector("Y", 85, privacyConstraints);
	}
	...
	private void writeTargetVector(String matrixName, long seed) {
		double[][] target = getRandomMatrix(rows, 1, 0, 100, 1, seed);
		MatrixCharacteristics mc = new MatrixCharacteristics(rows, 1, blocksize, rows);
		writeInputMatrixWithMTD(matrixName, target, false, mc);
	}

	private void writeTargetVector(String matrixName, long seed, String privacyConstraints) {
		double[][] target = getRandomMatrix(rows, 1, 0, 100, 1, seed);
		MatrixCharacteristics mc = new MatrixCharacteristics(rows, 1, blocksize, rows);
		writeInputMatrixWithMTD(matrixName, target, false, mc, privacyConstraints);
	}
