...
public class FederatedRowColAggregateTest extends AutomatedTestBase {
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][] {
            {10, 1000, false}, {100, 4, false},
            {36, 1000, true}, {1000, 10, true}, {4, 100, true}
        });
    }
    ...
    @Test
    public void testColMeanDenseMatrixCP() { runAggregateOperationTest(OpType.MEAN, InstType.COL, ExecMode.SINGLE_NODE); }

    @Test
    public void testColMaxDenseMatrixCP() { runAggregateOperationTest(OpType.MAX, InstType.COL, ExecMode.SINGLE_NODE); }

    @Test
    public void testRowSumDenseMatrixCP() { runAggregateOperationTest(OpType.SUM, InstType.ROW, ExecMode.SINGLE_NODE); }

    @Test
    public void testRowMeanDenseMatrixCP() { runAggregateOperationTest(OpType.MEAN, InstType.ROW, ExecMode.SINGLE_NODE); }

    @Test
    public void testRowMaxDenseMatrixCP() { runAggregateOperationTest(OpType.MAX, InstType.ROW, ExecMode.SINGLE_NODE); }

    @Test
    public void testRowMinDenseMatrixCP() { runAggregateOperationTest(OpType.MIN, InstType.ROW, ExecMode.SINGLE_NODE); }

    @Test
    public void testColMinDenseMatrixCP() { runAggregateOperationTest(OpType.MIN, InstType.COL, ExecMode.SINGLE_NODE); }
    ...
    private void runAggregateOperationTest( OpType type, InstType instr, ExecMode execMode)
    {
        ...
        switch( type )
        {
            case SUM: TEST_NAME = instr == InstType.COL ? TEST_NAME1 : TEST_NAME5; break;
            case MEAN: TEST_NAME = instr == InstType.COL ? TEST_NAME2 : TEST_NAME6; break;
            case MAX: TEST_NAME = instr == InstType.COL ? TEST_NAME3 : TEST_NAME7; break;
            case MIN: TEST_NAME = instr == InstType.COL ? TEST_NAME4 : TEST_NAME8; break;
        }
        ...
        switch( type )
        {
            case SUM: Assert.assertTrue(heavyHittersContainsString(fedInst.concat("k+"))); break;
            case MEAN: Assert.assertTrue(heavyHittersContainsString(fedInst.concat("mean"))); break;
            case MAX: Assert.assertTrue(heavyHittersContainsString(fedInst.concat("max"))); break;
            case MIN: Assert.assertTrue(heavyHittersContainsString(fedInst.concat("min"))); break;
        }
