...
public class FederatedCastToMatrixTest extends AutomatedTestBase {
	...
	public void federatedMultiply(Types.ExecMode execMode) {
		...
		try {
			...
			writer.writeFrameToHDFS(frame1.slice(0, halfRows - 1, 0, schema.length - 1, new FrameBlock()), input("X1"),
				halfRows, schema.length);

			DetectSchemaTest.initFrameDataString(frame2, X2, schema, halfRows, cols);
			writer.writeFrameToHDFS(frame2.slice(0, halfRows - 1, 0, schema.length - 1, new FrameBlock()), input("X2"),
				halfRows, schema.length);

			MatrixCharacteristics mc = new MatrixCharacteristics(X1.length, X1[0].length, OptimizerUtils.DEFAULT_BLOCKSIZE,
				-1);
