...
package org.apache.sysds.test.functions.federated.primitives.part4;
...
public class FederatedLogicalTest extends AutomatedTestBase {
	protected final static String SCALAR_TEST_NAME = "FederatedLogicalMatrixScalarTest";
	protected final static String MATRIX_TEST_NAME = "FederatedLogicalMatrixMatrixTest";
	protected final static String TEST_DIR = "functions/federated/binary/";
	protected final static String TEST_CLASS_DIR = TEST_DIR + FederatedLogicalTest.class.getSimpleName() + "/";

	protected final static String OUTPUT_NAME = "Z";
	protected final static double TOLERANCE = 0;
	protected final static int BLOCKSIZE = 1024;

	protected enum Type {
		GREATER, LESS, EQUALS, NOT_EQUALS, GREATER_EQUALS, LESS_EQUALS
	}

	protected enum FederationType {
		SINGLE_FED_WORKER, ROW_PARTITIONED, COL_PARTITIONED, FULL_PARTITIONED
	}

	protected enum YType {
		MATRIX, ROW_VEC, COL_VEC, FED_MAT, // federated matrix Y
	...
	@Override
	public void setUp() {
		addTestConfiguration(SCALAR_TEST_NAME,
			new TestConfiguration(TEST_CLASS_DIR, SCALAR_TEST_NAME, new String[] {OUTPUT_NAME}));
		addTestConfiguration(MATRIX_TEST_NAME,
			new TestConfiguration(TEST_CLASS_DIR, MATRIX_TEST_NAME, new String[] {OUTPUT_NAME}));
	}
	...
	public void federatedLogicalTest(String testname, Type op_type, ExecMode execMode) {
		...
		if(is_matrix_test) {
			...
			writeInputMatrixWithMTD("Y", Y_mat, false,
				new MatrixCharacteristics(y_rows, y_cols, BLOCKSIZE, y_rows * y_cols));
		}
		...
		try {
			...
			programArgs = new String[] {"-nvargs", "in_X1=" + input("X1"),
				"in_X2=" + (!single_fed_worker ? input("X2") : input("X1")), // not needed in case of a single federated
																									// worker
				"in_X3=" + (!single_fed_worker ? input("X3") : input("X1")), // not needed in case of a single federated
																									// worker
				"in_X4=" + (!single_fed_worker ? input("X4") : input("X1")), // not needed in case of a single federated
																									// worker
				"in_Y=" + (is_matrix_test ? input("Y") : Double.toString(Y_scal)),
				"in_fed_type=" + Integer.toString(fed_type.ordinal()), "in_y_type=" + Integer.toString(y_type.ordinal()),
				"in_op_type=" + Integer.toString(op_type.ordinal()), "out_Z=" + expected(OUTPUT_NAME)};
			...
			programArgs = new String[] {"-stats", "-nvargs", "in_X1=" + TestUtils.federatedAddress(port1, input("X1")),
				...
				"in_Y=" + (is_matrix_test ? input("Y") : Double.toString(Y_scal)),
				"in_fed_type=" + Integer.toString(fed_type.ordinal()), "in_y_type=" + Integer.toString(y_type.ordinal()),
				"in_op_type=" + Integer.toString(op_type.ordinal()), "rows=" + Integer.toString(fed_rows),
				"cols=" + Integer.toString(fed_cols), "out_Z=" + output(OUTPUT_NAME)};
			runTest(true, false, null, -1);
