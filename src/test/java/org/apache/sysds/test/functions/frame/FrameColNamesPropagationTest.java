...
public class FrameColNamesPropagationTest extends AutomatedTestBase {
	...
	private void writeInputFrame(String name, int rows, int cols, String prefix, long seed) throws IOException {
		String[] columnNames = genColnames(cols, prefix);

		Types.ValueType[] schema = Collections.nCopies(cols, Types.ValueType.FP64).toArray(new Types.ValueType[0]);

		FrameBlock frame = new FrameBlock(schema);
		frame.setColumnNames(columnNames);

		double[][] data = getRandomMatrix(rows, cols, -10, 10, 0.7, seed);

		TestUtils.initFrameData(frame, data, schema, rows);

		FrameWriter writer = FrameWriterFactory.createFrameWriter(FileFormat.CSV,
				new FileFormatPropertiesCSV(true, ",", false));

		writer.writeFrameToHDFS(frame, input(name), rows, cols);
	}
	...
	private void runPropagationCbindTest(Integer matrixDim, ExecType et) {
		...
		try {
			...
			programArgs = new String[]{"-args",
					input("X1_" + matrixDim), String.valueOf(matrixDim),
					String.valueOf(matrixDim),
					input("X2_" + matrixDim),
					Integer.toString(matrixDim),
					output("B")};

			writeInputFrame("X1_" + matrixDim, matrixDim, matrixDim, "A", 14123);

			writeInputFrame("X2_" + matrixDim, matrixDim, matrixDim, "B", 14124);
	...
	private void runPropagationSliceTest(Integer matrixDim, ExecType et) {
		...
		try {
			...
			programArgs = new String[]{"-args",
					input("X_" + matrixDim), String.valueOf(matrixDim),
					String.valueOf(matrixDim),
					output("B")};
	...
	private void runPropagationLeftIndexingTest(int matrixDim, ExecType et) {
		...
		try {
			...
			writeInputFrame("X_" + matrixDim, matrixDim, matrixDim, "A", 14123);
