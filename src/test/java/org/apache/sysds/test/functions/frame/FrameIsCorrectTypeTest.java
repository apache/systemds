...
public class FrameIsCorrectTypeTest extends AutomatedTestBase {
	private final static String TEST_NAME = "is_correct";
	private final static String TEST_DIR = "functions/frame/binary/";
	...
	private void runIsCorrectTest(ValueType[] schema, int rows, int cols, int badValues, int test,
		...
		try {
			...
			writer.writeFrameToHDFS(frame1.slice(0, rows - 1, 0, 1, new FrameBlock()), input("A"), rows, schema.length);
