...
class ParquetTestUtils {
	...
	static SparkSession sparkSession() {
		return AutomatedTestBase.createSystemDSSparkSession("parquet-frame-tests", "local[1]");
	}

	/**
	 * Reads a parquet file (or directory of part files) through Spark, into a FrameBlock with the given column layout,
	 * projecting the columns in the requested order.
	 *
	 * @param fname  parquet file or directory
	 * @param schema value types of the requested columns
	 * @param names  names of the requested columns
	 * @return the frame as read by Spark
	 */
	static FrameBlock sparkReadAsFrame(String fname, ValueType[] schema, String[] names) {
		Dataset<Row> df = sparkSession().read().parquet(fname).select(names[0],
			Arrays.copyOfRange(names, 1, names.length));
		return toFrameBlock(df.collectAsList(), schema, names);
	}

	/** Converts collected Spark rows into a FrameBlock */
	static FrameBlock toFrameBlock(List<Row> rows, ValueType[] schema, String[] names) {
		FrameBlock fb = new FrameBlock(schema, names);
		fb.ensureAllocatedColumns(rows.size());
		for(int r = 0; r < rows.size(); r++)
			for(int c = 0; c < schema.length; c++)
				fb.set(r, c, rows.get(r).get(c));
		return fb;
	}

	/**
	 * Generates the public test files (userdata1, alltypes_plain, all) with Spark's DataFrameWriter, covering all
	 * supported physical types and null values. One file is gzip-compressed to also cover a non-default codec on read;
	 * the others use Spark's default (snappy).
	 *
	...
	static Map<String, String> generatePublicTestFiles(File outDir) throws Exception {
		...
		files.put("userdata1", writeTestFile(spark, outDir, "userdata1", userdata1Rows(), userdata1Schema(), null));
		files.put("alltypes_plain",
			writeTestFile(spark, outDir, "alltypes_plain", alltypesPlainRows(), alltypesPlainSchema(), "gzip"));
		files.put("all", writeTestFile(spark, outDir, "all", allRows(), allSchema(), null));
		return files;
	}

	/**
	 * Writes a file containing a deprecated INT96 timestamp column (id, ts, name), forced via
	 * spark.sql.parquet.outputTimestampType so the encoding does not depend on the Spark version.
	 *
	 * @param outDir directory the file is written into
	 * @return the generated file path
	 */
	static String writeInt96TimestampFile(File outDir) throws IOException {
		SparkSession spark = sparkSession();
		StructType schema = new StructType(
			new StructField[] {new StructField("id", DataTypes.IntegerType, true, Metadata.empty()),
				new StructField("ts", DataTypes.TimestampType, true, Metadata.empty()),
				new StructField("name", DataTypes.StringType, true, Metadata.empty()),});
		List<Row> rows = Arrays.asList(RowFactory.create(1, Timestamp.valueOf("2016-02-03 07:55:29"), "Amanda"),
			RowFactory.create(2, Timestamp.valueOf("2016-02-03 17:04:03"), "Albert"),
			RowFactory.create(3, null, "Evelyn"));
		spark.conf().set("spark.sql.parquet.outputTimestampType", "INT96");
		try {
			return writeTestFile(spark, outDir, "int96", rows, schema, null);
		}
		finally {
	...
	private static StructType userdata1Schema() {
		return new StructType(new StructField[] {new StructField("id", DataTypes.IntegerType, true, Metadata.empty()),
	...
	private static List<Row> userdata1Rows() {
		return Arrays.asList(RowFactory.create(1, "Amanda", 49756.53), RowFactory.create(2, "Albert", 150280.17),
			RowFactory.create(3, "Evelyn", 144972.51), RowFactory.create(4, "Denise", null),
			RowFactory.create(5, "Carlos", 75500.34));
	}
	...
	private static List<Row> alltypesPlainRows() {
		return Arrays.asList(RowFactory.create(1, true, 1, 10, 100L, 1.5f, 2.25, "03/01/09", "row-1"),
			RowFactory.create(2, false, 2, 20, 200L, 2.5f, 4.5, "03/02/09", "row-2"),
			RowFactory.create(3, true, 3, 30, 300L, 3.5f, 6.75, "03/03/09", "row-3"),
			RowFactory.create(4, false, 4, 40, 400L, 4.5f, 9.0, "03/04/09", "row-4"),
			RowFactory.create(5, true, 5, 50, 500L, 5.5f, 11.25, "03/05/09", "row-5"),
			RowFactory.create(6, false, 6, 60, 600L, 6.5f, 13.5, "03/06/09", "row-6"),
			RowFactory.create(7, true, 7, 70, 700L, 7.5f, 15.75, "03/07/09", "row-7"),
			RowFactory.create(8, false, 8, 80, 800L, 8.5f, 18.0, "03/08/09", "row-8"));
	}

	private static StructType allSchema() {
		return new StructType(
			new StructField[] {new StructField("PassengerId", DataTypes.IntegerType, true, Metadata.empty()),
	...
	private static List<Row> allRows() {
		return Arrays.asList(RowFactory.create(1, 0, 3, "Braund, Mr. Owen Harris", "male", 22.0, 7.25, "S"),
	...
	private static String writeTestFile(SparkSession spark, File outDir, String name, List<Row> rows, StructType schema,
		String codec) throws IOException {
		Dataset<Row> df = spark.createDataFrame(rows, schema);
		File tmpDir = new File(outDir, "_tmp_" + name);
		DataFrameWriter<Row> writer = df.coalesce(1).write().mode(SaveMode.Overwrite);
		if(codec != null)
			writer = writer.option("compression", codec);
		writer.parquet(tmpDir.getPath());
