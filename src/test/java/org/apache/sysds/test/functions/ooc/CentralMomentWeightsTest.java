...
public class CentralMomentWeightsTest extends AutomatedTestBase {
	...
	private void runCentralMomentTest(int order, boolean sparse) {
		...
		try {
			...
			programArgs = new String[] {"-explain", "-stats", "-ooc", "-args", input(INPUT_NAME), input(INPUT_NAME_W),
			...
			programArgs = new String[] {"-explain", "-stats", "-args", input(INPUT_NAME), input(INPUT_NAME_W),
				Integer.toString(order), output(OUTPUT_NAME + "_target")};
			runTest(true, false, null, -1);
