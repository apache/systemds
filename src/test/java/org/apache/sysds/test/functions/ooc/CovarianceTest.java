...
public class CovarianceTest extends AutomatedTestBase {
    private final static String TEST_NAME1 = "Covariance";
    ...
    @Test
    public void testCovarianceDense() {
        runCovarianceTest(false);
    }

    @Test
    public void testCovarianceSparse() {
        runCovarianceTest(true);
    }

    private void runCovarianceTest(boolean sparse) {
        ...
        try {
            ...
            programArgs = new String[] {"-explain", "-stats", "-ooc", "-args", input(INPUT_NAME_A),
                    input(INPUT_NAME_B), output(OUTPUT_NAME)};

            // 1. Generate the data in-memory as MatrixBlock objects like 1-d column vectors;
