...
import org.apache.sysds.test.functions.binary.matrix.MatrixMultiplicationTest;
import org.junit.Assert;
import org.junit.Test;

import java.io.IOException;

public class MatrixMatrixBinaryMultiplicationTest extends AutomatedTestBase {
	private final static String TEST_NAME1 = "MatrixMatrixMultiplication";
	private final static String TEST_DIR = "functions/ooc/";
	private final static String TEST_CLASS_DIR = TEST_DIR + MatrixMatrixBinaryMultiplicationTest.class.getSimpleName() + "/";
	...
	@Test
	public void testMMBinaryMultiplication1() {
		runMatrixMatrixMultiplicationTest(cols_wide, false);
	}

	private void runMatrixMatrixMultiplicationTest(int cols, boolean sparse )
	{
		...
		try
		{
			...
			double[][] B_data = getRandomMatrix(rows2, cols2, 0, 1, 1.0, 10);
			...
			// 5. Write vector B to a binary SequenceFile
			writer.writeMatrixToHDFS(B_mb, input(INPUT_NAME2), rows2, cols2, 1000, B_mb.getNonZeros());
			HDFSTool.writeMetaDataFile(input(INPUT_NAME2 + ".mtd"), Types.ValueType.FP64,
					new MatrixCharacteristics(rows2, cols2, 1000, B_mb.getNonZeros()), Types.FileFormat.BINARY);
			...
			MatrixBlock ret2 = LibMatrixMult.matrixMult(A_mb, B_mb, k);
