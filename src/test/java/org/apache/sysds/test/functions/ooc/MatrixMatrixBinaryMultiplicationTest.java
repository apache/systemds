...
public class MatrixMatrixBinaryMultiplicationTest extends AutomatedTestBase {
	...
	private final static int cols_wide = 3000;
	...
	private void runMatrixVectorMultiplicationTest(int cols, boolean sparse )
	{
		...
		try
		{
			...
			HDFSTool.writeMetaDataFile(input(INPUT_NAME2 + ".mtd"), Types.ValueType.FP64,
					new MatrixCharacteristics(cols, rows, 1000, x_mb.getNonZeros()), Types.FileFormat.BINARY);
			...
			programArgs = new String[]{"-explain", "-stats",
				"-args", input(INPUT_NAME), input(INPUT_NAME2), output(OUTPUT_NAME + "_target")};
			runTest(true, exceptionExpected, null, -1);

			// compare matrices
			MatrixBlock ret1 = DataConverter.readMatrixFromHDFS(output(OUTPUT_NAME),
				Types.FileFormat.BINARY, rows, rows, 1000);
