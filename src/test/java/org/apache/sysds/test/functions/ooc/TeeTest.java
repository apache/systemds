...
public class TeeTest extends AutomatedTestBase 
{
	...
	public void testTeeOperation(boolean rewrite)
	{
		...
		try {
			...
			programArgs = new String[] {"-explain","hops", "-stats", "-ooc",
							"-args", input(INPUT_NAME), output(OUTPUT_NAME)};

			int rows = 2000, cols = 100;
			...
			//compare results and check ops
			double[][] C1 = readMatrix(output(OUTPUT_NAME), FileFormat.BINARY, rows, cols, 1000, 1000);
			double expected = mb.sum() + mb.sumSq();
			Assert.assertEquals( "result mismatch", expected, C1[0][0], eps);
			...
			Assert.assertTrue("OOC wasn't used for TEE",
							heavyHittersContainsString(Instruction.OOC_INST_PREFIX + "tee"));
		}
		catch(Exception ex) {
	...
	private static double[][] readMatrix( String fname, FileFormat fmt,
		long rows, long cols, int brows, int bcols ) throws IOException
	{
