	...
	private void runReverseTest(String testname, boolean matrix, boolean sparse, ExecType instType)
	{
		...
		try
		{
			...
			if( instType == ExecType.CP )
				Assert.assertTrue("Missing opcode: rev", Statistics.getCPHeavyHitterOpCodes().contains(Opcodes.REV.getName()));
			else if ( instType == ExecType.SPARK )
				Assert.assertTrue("Missing opcode: "+Instruction.SP_INST_PREFIX+Opcodes.REV.getName(), Statistics.getCPHeavyHitterOpCodes().contains(Instruction.SP_INST_PREFIX+"rev"));
		}
		finally
		{
			//reset flags
