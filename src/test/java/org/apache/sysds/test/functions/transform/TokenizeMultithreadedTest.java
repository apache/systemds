...
public class TokenizeMultithreadedTest extends AutomatedTestBase  {
    ...
    public void setUp()  {
        ...
        addTestConfiguration(this.getClass().getSimpleName(), new TestConfiguration(TEST_CLASS_DIR, this.getClass().getSimpleName(), new String[] { "R" }) );
    }

    @Test
    public void testTokenizeSplitCountLong() {
        runTokenizeTest(ExecMode.SINGLE_NODE, TokenizerBuilder.WHITESPACE_SPLIT,TokenizerApplier.COUNT,  2000, false, null, count_out_params0);
    }
    ...
    private void runTokenizeTest(ExecMode rt, TokenizerBuilder builder, TokenizerApplier applier, int max_tokens, boolean format_wide, JsonObject algo_params, JsonObject out_params) {
        try{
            ...
            TestUtils.compareFrames(DataConverter.convertToStringFrame(outS), DataConverter.convertToStringFrame(outM),
                    outS.getNumRows(), outS.getNumColumns());
