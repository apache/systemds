...
public class ElementwiseBitwLogicalTest extends AutomatedTestBase{
	...
	private void runBitwLogic(String testname, boolean sparse, ExecType et) {
		...
		try {
			...
			//get a random matrix of values with [1,999]
			double[][] A = getRandomMatrix(rows, cols, 1, 999, sparse ? sparsity1 : sparsity2, 1234);
			double[][] B = getRandomMatrix(rows, cols, 1, 999, sparse ? sparsity1 : sparsity2, 5678);
