...
x=matrix(seq(1, numImg*numChannels*imgSize*imgSize), numImg, numChannels*imgSize*imgSize)
w=matrix(seq(1, numFilters*numChannels*filterSize*filterSize), numFilters, numChannels*filterSize*filterSize)
  ...
  for (hout in 1:Hout) {  # all output rows
    ...
    for (wout in 1:Wout) {  # all output columns
      ...
      for (c in 1:C) {  # all channels
        img_slice = matrix(img[c,], Hin, Win)  # reshape
        img_patch[c,] = matrix(img_slice[hin:hin+Hf-1, win:win+Wf-1], 1, Hf*Wf)
      }
      img_cols[,(hout-1)*Wout + wout] = matrix(img_patch, C*Hf*Wf, 1)  # reshape
    }
  }
  img_cols
}
...
conv2d <- function(X, W, C, Hin, Win, Hf, Wf, strideh, stridew, padh, padw) {
  ...
  for (n in 1:N) {  # all examples
    ...
    out[n,] = matrix(outn, 1, F*Hout*Wout)  # reshape
  }
